...
namespace smo::runtime {
    ...
    public:
        ...
        {
            return {Category::Validation, 1000, msg, false, false};
        }
        static RuntimeError not_found(const std::string& msg) { return {Category::NotFound, 1001, msg, false, false}; }
        static RuntimeError unauthorized(const std::string& msg)
        {
            return {Category::Unauthorized, 1002, msg, false, true};
        }
        static RuntimeError policy_denied(const std::string& msg)
        {
            return {Category::PolicyDenied, 1003, msg, false, true};
        }
        ...
        static RuntimeError compensation(const std::string& msg)
        {
            return {Category::Compensation, 1009, msg, false, true};
        }
    };
    ...
    {
    public:
        using Value = std::variant<std::monostate, bool, int64_t, double, std::string, Bytes,
            ...
            return std::visit(
                [](auto&& arg) -> std::string {
                    ...
                    if constexpr (std::is_same_v<T, std::monostate>)
                        ...
                        return "[bytes:" + std::to_string(arg.size()) + "]";
        ...
        Result<ContextValue> operator[](const std::string& key) const
        {
            if (is_map())
            {
                ...
                return Result<ContextValue>(Error(
                    ErrorCode{ErrorCategory::Internal, 1001, Severity::Error, RetryClass::NoRetry, Recovery::None},
                    "key not found: " + key));
            }
            return Result<ContextValue>(
                Error(ErrorCode{ErrorCategory::Internal, 1002, Severity::Error, RetryClass::NoRetry, Recovery::None},
                      "not a map"));
        }
    ...
    struct ContractInput
    {
        ...
        static ContractInput method_only(const std::string& method) { return {method, ContextValue()}; }
        static ContractInput with_map(const std::string& method,
                                      const std::unordered_map<std::string, std::string>& map)
        {
            return {method, ContextValue(map)};
        }
        static ContractInput with_string(const std::string& method, const std::string& data)
        {
    ...
    struct Variables
    {
        ...
        void set(const std::string& key, ContextValue value) const { store[key] = std::move(value); }

        Result<ContextValue> get(const std::string& key) const
        {
            auto it = store.find(key);
            if (it == store.end())
                return Result<ContextValue>(static_cast<Error>(RuntimeError::not_found("variable not found: " + key)));
            return it->second;
        }
    ...
    using NextAction = std::variant<ActionDispatchContract, ActionDispatchMessage, ActionScheduleRetry, ActionEmitEvent,
                                    ActionStoreContext, ActionSpawnPlan, ActionNotify, ActionCompensate, ActionAbort>;
    ...
    struct ContractResult
    {
        ...
        static ContractResult ok(std::string data = "") { return {Status::Success, std::move(data), {}, {}, {}}; }
        ...
        Result<void> validate() const
        {
            ...
            for (size_t i = 0; i < steps.size(); ++i)
            {
                if (id_to_idx.count(steps[i].step_id))
                    return Result<void>(
                        static_cast<Error>(RuntimeError::validation("duplicate step id: " + steps[i].step_id)));
            ...
            for (const auto& step : steps)
            {
                for (const auto& dep : step.depends_on)
                {
                    if (!id_to_idx.count(dep))
                        return Result<void>(static_cast<Error>(RuntimeError::validation(
                            "step '" + step.step_id + "' depends on unknown step '" + dep + "'")));
                }
            }
            ...
            if (visited != steps.size())
                return Result<void>(static_cast<Error>(RuntimeError::validation("cycle detected in execution plan")));
            return {};
        }
    ...
    inline NextAction dispatch_message(std::string opcode, std::vector<uint8_t> data, std::string target = "",
                                       uint64_t timeout = 5'000'000'000)
    {
        return ActionDispatchMessage{std::move(opcode), std::move(data), std::move(target), timeout};
    }
    inline NextAction schedule_retry(uint64_t delay_ns = 1'000'000'000, uint64_t max_retries = 3, double backoff = 2.0)
    {
        return ActionScheduleRetry{delay_ns, max_retries, backoff};
    }
    inline NextAction emit_event(std::string type, std::string payload = "", EventPriority prio = EventPriority::Normal)
    {
    ...
    inline NextAction spawn_plan(std::string plan_id, std::unordered_map<std::string, std::string> params = {},
                                 ExecutionMode mode = ExecutionMode::Sequential)
    {
