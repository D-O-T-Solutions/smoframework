...
namespace smo::runtime {
    ...
    Result<void> MiddlewarePipeline::process(PacketContext& ctx)
    {
        for (auto& mw : middlewares_)
        {
            ...
            {
                return Error(
                    ErrorCode(ErrorCategory::Session, 507, Severity::Error, RetryClass::NoRetry, Recovery::None),
                    ctx.deny_reason.empty() ? "denied by " + mw->name() : ctx.deny_reason, __FILE__, __LINE__);
            }
        }
        return {};
    }
