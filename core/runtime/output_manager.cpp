...
namespace smo::runtime {
    ...
    void OutputManager::add_result(const std::string& node_id, const std::string& contract_id,
                                   const std::string& execution_id, const std::string& status,
    ...
    OutputManager::DetailedOutput OutputManager::drill_down(const DrillFilter& filter) const
    {
        ...
        {
            bool match = true;
            if (!filter.contract_id.empty() && r.contract_id != filter.contract_id)
                match = false;
            if (!filter.node_id.empty() && r.node_id != filter.node_id)
                match = false;
            if (!filter.execution_id.empty() && r.execution_id != filter.execution_id)
                match = false;
            if (!filter.status_filter.empty() && r.status != filter.status_filter)
                match = false;

            if (match)
            {
                detail.results.push_back({.node_id = r.node_id,
