...
namespace smo {
    ...
    struct EventStore::Impl
    {
        ...
        Result<void> open()
        {
            ...
            int rc3 = sqlite3_prepare_v2(db, "SELECT event_hash FROM events ORDER BY sequence DESC LIMIT 1", -1, &stmt,
                                         nullptr);
            if (rc3 == SQLITE_OK)
            {
                if (sqlite3_step(stmt) == SQLITE_ROW)
                {
                    ...
                    if (hash)
                        last_prev_hash = reinterpret_cast<const char*>(hash);
                }
                sqlite3_finalize(stmt);
            }
    ...
    Result<uint64_t> EventStore::append(const EventRecord& event)
    {
        ...
        auto stmt = impl_->prepare("INSERT INTO events (sequence, type, timestamp_ns, contract_id, execution_id, "
        ...
        if (rc != SQLITE_DONE)
        {
            return SMO_ERR_STORAGE(904, Error, RetrySafe, RetryOperation, "Failed to insert event");
        }
    ...
    {
        auto stmt = impl_->prepare("SELECT sequence, type, timestamp_ns, contract_id, execution_id, trace_id, "
    ...
    {
        auto stmt = impl_->prepare("INSERT OR REPLACE INTO executions "
    ...
    {
        auto stmt = impl_->prepare("SELECT execution_id, contract_id, trace_id, requester_id, responder_id, "
                                   "witness_ids, selected_nodes, intent_hash, policy_name, control_level, "
                                   "scope, created_ns, started_ns, completed_ns, timeout_ns, "
                                   "retry_count, max_retries, priority, status, result_hash, error_message "
