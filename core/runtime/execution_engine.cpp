    ...
    struct ExecutionEngine::Impl
    {
        ...
        void execute_task(std::unique_ptr<Task> task)
        {
            ...
            task->promise.set_value(ExecutionResult{
                ...
                .error_message = "",
                .created_ns =
                    std::chrono::duration_cast<std::chrono::nanoseconds>(task->created.time_since_epoch()).count(),
                .started_ns =
                    std::chrono::duration_cast<std::chrono::nanoseconds>(task->started.time_since_epoch()).count(),
                .completed_ns =
                    std::chrono::duration_cast<std::chrono::nanoseconds>(task->completed.time_since_epoch()).count(),
