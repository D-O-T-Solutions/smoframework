...
namespace smo::runtime {
    ...
    public:
        ...
        PlanExecutor(const ExecutionPlan& plan, StepExecutor executor) : plan_(plan), executor_(std::move(executor)) {}
    ...
    private:
                ...
                if (status.state == StepState::Completed)
                {
                    ...
                    for (const auto& step : plan_.steps)
                    {
                        if (step.step_id == id && !step.compensation_id.empty())
                        {
                            ...
                            auto* comp_contract =
                                ctx.dispatcher ? ctx.dispatcher->get_contract(step.compensation_id) : nullptr;
                            if (comp_contract)
                            {
        ...
        {
            std::visit(overloaded{[&](const ActionDispatchContract& a) {
                                  ...
                                  [&](const ActionStoreContext& a) { ctx.context[a.key] = a.value.to_string(); },
                                  ...
                                  [&](const ActionCompensate& a) { trigger_compensation(ctx); },
