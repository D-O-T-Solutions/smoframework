...
namespace smo::runtime {
    ...
    std::string LogEntry::to_json() const
    {
        ...
        if (!node_id.empty())
            j += R"(,"node_id":")" + escape_json(node_id) + '"';
        if (!mesh_id.empty())
            j += R"(,"mesh_id":")" + escape_json(mesh_id) + '"';
        if (!trace_id.empty())
            j += R"(,"trace_id":")" + escape_json(trace_id) + '"';
        if (!span_id.empty())
            j += R"(,"span_id":")" + escape_json(span_id) + '"';
        if (!session_id.empty())
            j += R"(,"session_id":")" + escape_json(session_id) + '"';
        if (!component.empty())
            j += R"(,"component":")" + escape_json(component) + '"';
    ...
    LogEntry LogEntry::make(std::string component, LogLevel level, std::string message, std::string trace_id,
                            std::string span_id)
    {
        LogEntry e;
    ...
    StructuredLogger::StructuredLogger(Format fmt) : impl_(std::make_unique<Impl>())
    {
        impl_->format = fmt;
    }
    ...
    void StructuredLogger::set_format(Format fmt)
    {
        impl_->format = fmt;
    }
    void StructuredLogger::set_node_id(std::string n)
    {
        impl_->node_id = std::move(n);
    }
    void StructuredLogger::set_mesh_id(std::string m)
    {
        impl_->mesh_id = std::move(m);
    }
    void StructuredLogger::set_component(std::string c)
    {
        impl_->component = std::move(c);
    }
    void StructuredLogger::enable_trace_id(bool e)
    {
        impl_->trace_id_enabled = e;
    }
    std::string StructuredLogger::node_id() const noexcept
    {
