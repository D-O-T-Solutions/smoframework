...
namespace smo::runtime {
    ...
    JoinContract::JoinContract(const HashImpl& hash, const SignerImpl& signer, RngRef& rng)
        : NativeContract(default_metadata()), hash_(hash), signer_(signer), rng_(rng)
    {
    }

    Result<ContractResult> JoinContract::execute(const ContractInput& input, const RuntimeContext& ctx)
    {
    ...
    Result<ContractResult> JoinContract::handle_join(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        ...
        // Emit join event
        result.next_actions.push_back(emit_event("node.joined", "node=" + node_id + " mesh=" + token.mesh_id));

        return result;
    }

    // ── handle_leave ──────────────────────────────────────────────────────
    Result<ContractResult> JoinContract::handle_leave(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        ...
        result.next_actions.push_back(emit_event("node.left", "node=" + node_id_res.value()));

        return result;
    }

    // ── handle_info ──────────────────────────────────────────────────────
    Result<ContractResult> JoinContract::handle_info(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)input;
        (void)ctx;
