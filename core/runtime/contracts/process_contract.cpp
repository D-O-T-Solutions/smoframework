...
namespace smo::runtime {
    ...
    ProcessContract::ProcessContract() : NativeContract(default_metadata()) {}

    Result<ContractResult> ProcessContract::execute(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        if (input.method == "exec")
            return handle_exec(input, ctx);
        if (input.method == "kill")
            return handle_kill(input, ctx);
        if (input.method == "ps")
            return handle_ps(input, ctx);
        if (input.method == "top")
            return handle_top(input, ctx);
        if (input.method == "systemctl")
            return handle_systemctl(input, ctx);
        if (input.method == "service")
            return handle_service(input, ctx);
    ...
    // ── handle_exec ─────────────────────────────────────────────────────
    Result<ContractResult> ProcessContract::handle_exec(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto command = map_str(input.arguments, "command");
        if (!command)
            return ContractResult::denied(command.error().message);
        ...
        pid_t pid = fork();
        if (pid < 0)
            return ContractResult::denied("fork failed");
        ...
        while (std::chrono::steady_clock::now() < wait_until)
        {
            ...
            if (ret == pid)
            {
                exit_code = WIFEXITED(status) ? WEXITSTATUS(status) : WIFSIGNALED(status) ? -WTERMSIG(status) : -1;
                break;
            }
    ...
    // ── handle_kill ─────────────────────────────────────────────────────
    Result<ContractResult> ProcessContract::handle_kill(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto pid = map_int(input.arguments, "pid");
        if (!pid)
            return ContractResult::denied(pid.error().message);
    ...
    // ── handle_ps ───────────────────────────────────────────────────────
    Result<ContractResult> ProcessContract::handle_ps(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        ...
        if (!proc)
            return ContractResult::denied("cannot open /proc");
    ...
    // ── handle_top ──────────────────────────────────────────────────────
    Result<ContractResult> ProcessContract::handle_top(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
    ...
    // ── handle_systemctl ────────────────────────────────────────────────
    Result<ContractResult> ProcessContract::handle_systemctl(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto unit = map_str(input.arguments, "unit");
        if (!unit)
            return ContractResult::denied(unit.error().message);
        auto action = map_str(input.arguments, "action");
        if (!action)
            return ContractResult::denied(action.error().message);
    ...
    // ── handle_service ──────────────────────────────────────────────────
    Result<ContractResult> ProcessContract::handle_service(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto service = map_str(input.arguments, "service");
        if (!service)
            return ContractResult::denied(service.error().message);
        auto action = map_str(input.arguments, "action");
        if (!action)
            return ContractResult::denied(action.error().message);
    ...
    // ── handle_info ─────────────────────────────────────────────────────
    Result<ContractResult> ProcessContract::handle_info(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)input;
        (void)ctx;
