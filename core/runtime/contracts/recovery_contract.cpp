...
namespace smo::runtime {
    ...
    static Result<std::string> map_str(const ContextValue& args, const std::string& key)
    {
        if (!args.is_map())
        {
            return Error(ErrorCode{ErrorCategory::Runtime, 1002, Severity::Error, RetryClass::NoRetry, Recovery::None},
    ...
    static Result<int64_t> map_int(const ContextValue& args, const std::string& key)
    {
    ...
    static Result<uint64_t> map_uint(const ContextValue& args, const std::string& key)
    {
    ...
    ContractMetadata RecoveryContract::default_metadata()
    {
        ...
        meta.provides = {"assess", "start", "sign", "execute", "cancel", "crl_revoke", "crl_check", "crl_sync", "info"};
        meta.entry_point = "system.recovery";
        meta.has_validate = true;
        return meta;
    }

    RecoveryContract::RecoveryContract(smo::recovery::RecoveryEngine& engine, smo::recovery::CRL* crl,
                                       smo::GovernanceEngine& governance_engine)
        : NativeContract(default_metadata()), engine_(engine), crl_(crl), governance_engine_(governance_engine)
    {
    }

    Result<ContractResult> RecoveryContract::execute(const ContractInput& input, const RuntimeContext& ctx)
    {
        if (input.method == "assess")
            return handle_assess(input, ctx);
        if (input.method == "start")
            return handle_start(input, ctx);
        if (input.method == "sign")
            return handle_sign(input, ctx);
        if (input.method == "execute")
            return handle_execute(input, ctx);
        if (input.method == "cancel")
            return handle_cancel(input, ctx);
        if (input.method == "crl_revoke")
            return handle_crl_revoke(input, ctx);
        if (input.method == "crl_check")
            return handle_crl_check(input, ctx);
        if (input.method == "crl_sync")
            return handle_crl_sync(input, ctx);
    ...
    // ── handle_assess ───────────────────────────────────────────────────
    Result<ContractResult> RecoveryContract::handle_assess(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;

        auto total = map_uint(input.arguments, "total_authorities");
        if (!total)
            return ContractResult::denied(total.error().message);

        auto online = map_uint(input.arguments, "online_authorities");
        if (!online)
            return ContractResult::denied(online.error().message);

        auto quorum = map_int(input.arguments, "quorum_threshold");
        if (!quorum)
            return ContractResult::denied(quorum.error().message);

        auto mode = engine_.assess_mode(static_cast<uint32_t>(total.value()), static_cast<uint32_t>(online.value()),
                                        static_cast<int>(quorum.value()));

        const char* mode_str = "none";
        switch (mode)
        {
        case smo::recovery::RecoveryMode::Soft:
            mode_str = "soft";
            break;
        case smo::recovery::RecoveryMode::Hard:
            mode_str = "hard";
            break;
        default:
            break;
        ...
        result.metrics["mode"] = ContextValue(static_cast<int64_t>(static_cast<uint8_t>(mode)));
        result.data = std::string("{\"mode\":\"") + mode_str + "\"}";
        return result;
    }

    // ── handle_start ────────────────────────────────────────────────────
    Result<ContractResult> RecoveryContract::handle_start(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;

        auto mode_str = map_str(input.arguments, "mode");
        if (!mode_str)
            return ContractResult::denied(mode_str.error().message);

        auto mesh_id = map_str(input.arguments, "mesh_id");
        if (!mesh_id)
            return ContractResult::denied(mesh_id.error().message);

        auto root_node_id = map_str(input.arguments, "root_node_id");
        if (!root_node_id)
            return ContractResult::denied(root_node_id.error().message);

        auto epoch = map_uint(input.arguments, "current_epoch");
        if (!epoch)
            return ContractResult::denied(epoch.error().message);

        auto passphrase = map_str(input.arguments, "recovery_passphrase");
        if (!passphrase)
            return ContractResult::denied(passphrase.error().message);
        ...
        Result<smo::recovery::RecoverySession> session_res =
            (mode_str.value() == "hard")
                ? engine_.start_hard(mesh_id.value(), root_node_id.value(), epoch.value(), passphrase.value(), now_ns)
                : engine_.start_soft(mesh_id.value(), root_node_id.value(), epoch.value(), passphrase.value(), now_ns);
        ...
        result.metrics["mode"] = ContextValue(static_cast<int64_t>(static_cast<uint8_t>(session_res.value().mode)));
        result.metrics["state"] = ContextValue(static_cast<int64_t>(static_cast<uint8_t>(session_res.value().state)));
        result.metrics["session_id"] = ContextValue(session_res.value().session_id);
        result.metrics["new_epoch"] = ContextValue(static_cast<int64_t>(session_res.value().new_epoch));
        result.binary = ser.value();
        result.next_actions.push_back(
            emit_event("recovery.session_started", "session=" + session_res.value().session_id));
        return result;
    }

    // ── handle_sign ─────────────────────────────────────────────────────
    Result<ContractResult> RecoveryContract::handle_sign(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;

        auto session_hex = map_str(input.arguments, "session");
        if (!session_hex)
            return ContractResult::denied(session_hex.error().message);

        auto session_bytes = hex_to_bytes(session_hex.value());
        if (!session_bytes)
            return ContractResult::denied(session_bytes.error().message);

        auto session_res = smo::recovery::RecoverySession::deserialize(BytesView(session_bytes.value()));
        if (!session_res)
        {
        ...
        if (!sig_hex)
            return ContractResult::denied(sig_hex.error().message);

        auto sig_bytes = hex_to_bytes(sig_hex.value());
        if (!sig_bytes)
            return ContractResult::denied(sig_bytes.error().message);

        auto sig_res = smo::GovernanceSignature::deserialize(BytesView(sig_bytes.value()));
        if (!sig_res)
        {
        ...
        result.metrics["session_id"] = ContextValue(session_res.value().session_id);
        result.metrics["signatures"] = ContextValue(static_cast<int64_t>(session_res.value().signatures.size()));
        result.binary = ser.value();
        return result;
    }

    // ── handle_execute ──────────────────────────────────────────────────
    Result<ContractResult> RecoveryContract::handle_execute(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        ...
        result.metrics["session_id"] = ContextValue(session_res.value().session_id);
        result.metrics["new_state"] =
            ContextValue(static_cast<int64_t>(static_cast<uint8_t>(session_res.value().state)));
        result.binary = ser.value();
        return result;
    }

    // ── handle_cancel ───────────────────────────────────────────────────
    Result<ContractResult> RecoveryContract::handle_cancel(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
    ...
    Result<ContractResult> RecoveryContract::handle_crl_revoke(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;

        auto fingerprint = map_str(input.arguments, "cert_fingerprint");
        if (!fingerprint)
            return ContractResult::denied(fingerprint.error().message);

        auto node_id_hex = map_str(input.arguments, "node_id_hex");
        if (!node_id_hex)
            return ContractResult::denied(node_id_hex.error().message);

        auto reason = map_str(input.arguments, "reason");
        if (!reason)
            return ContractResult::denied(reason.error().message);
        ...
        auto ev = make_event(EventType::RecoveryProposalCreated, "recovery_contract",
                             std::to_string(id_res.value().value), "", "system.recovery", payload_oss.str());
        ...
        result.next_actions.push_back(
            emit_event("recovery.proposal_created",
                       "proposal_id=" + std::to_string(id_res.value().value) + ",fingerprint=" + fingerprint.value()));

        return result;
    }

    // ── handle_crl_check ────────────────────────────────────────────────
    Result<ContractResult> RecoveryContract::handle_crl_check(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;

        if (!crl_)
            return ContractResult::denied("CRL not available");
        ...
        result.data = revoked_res.value() ? std::string("{\"revoked\":true}") : std::string("{\"revoked\":false}");
        return result;
    }

    // ── handle_crl_sync ─────────────────────────────────────────────────
    Result<ContractResult> RecoveryContract::handle_crl_sync(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
    ...
    // ── handle_info ─────────────────────────────────────────────────────
    Result<ContractResult> RecoveryContract::handle_info(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)input;
        (void)ctx;
