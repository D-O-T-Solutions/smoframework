...
namespace smo::runtime {
    ...
    GovernanceContract::GovernanceContract(smo::GovernanceEngine& engine, smo::authority::MeshAuthority& authority)
        : NativeContract(default_metadata()), engine_(engine), authority_(authority)
    {
    }

    Result<ContractResult> GovernanceContract::execute(const ContractInput& input, const RuntimeContext& ctx)
    {
        if (input.method == "propose")
            return handle_propose(input, ctx);
        if (input.method == "vote")
            return handle_vote(input, ctx);
        if (input.method == "commit")
            return handle_commit(input, ctx);
        if (input.method == "list")
            return handle_list(input, ctx);
        if (input.method == "status")
            return handle_status(input, ctx);
    ...
    // ── handle_propose ──────────────────────────────────────────────────
    Result<ContractResult> GovernanceContract::handle_propose(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        ...
        result.metrics["action"] =
            ContextValue(static_cast<int64_t>(static_cast<uint8_t>(proposal_res.value().action)));
        result.next_actions.push_back(
            emit_event("governance.proposal_created", "id=" + std::to_string(id_res.value().value)));
        return result;
    }

    // ── handle_vote ────────────────────────────────────────────────────
    Result<ContractResult> GovernanceContract::handle_vote(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        ...
        auto sign_res =
            engine_.sign(smo::ProposalID{pid}, authority_id, sig_res.value().signature, now_ns / 1'000'000'000LL);
        if (!sign_res)
        {
        ...
        result.next_actions.push_back(emit_event("governance.vote_cast", "proposal=" + std::to_string(pid)));
        return result;
    }

    // ── handle_commit ──────────────────────────────────────────────────
    Result<ContractResult> GovernanceContract::handle_commit(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        ...
        auto res = engine_.commit(smo::ProposalID{static_cast<uint64_t>(pid_res.value())}, now_ns / 1'000'000'000LL);
        if (!res)
        {
        ...
        result.metrics["proposal_id"] = ContextValue(pid_res.value());
        result.next_actions.push_back(
            emit_event("governance.proposal_committed", "proposal=" + std::to_string(pid_res.value())));
        return result;
    }

    // ── handle_list ────────────────────────────────────────────────────
    Result<ContractResult> GovernanceContract::handle_list(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)input;
        (void)ctx;
    ...
    // ── handle_status ──────────────────────────────────────────────────
    Result<ContractResult> GovernanceContract::handle_status(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        ...
        ContractResult result = ContractResult::ok(oss.str());
        result.metrics["state"] = ContextValue(static_cast<int64_t>(static_cast<uint8_t>(p.state)));
        result.metrics["signatures"] = ContextValue(static_cast<int64_t>(p.signatures.size()));
        return result;
    }

    // ── handle_info ────────────────────────────────────────────────────
    Result<ContractResult> GovernanceContract::handle_info(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)input;
        (void)ctx;
