...
namespace smo::runtime {
    ...
    BootstrapContract::BootstrapContract(smo::MeshManager& mesh_mgr, smo::authority::MeshAuthority& authority,
                                         smo::GovernanceEngine* governance, smo::recovery::CRL* crl)
        : NativeContract(default_metadata()), mesh_mgr_(mesh_mgr), authority_(authority), governance_(governance),
          crl_(crl)
    {
    }

    Result<ContractResult> BootstrapContract::execute(const ContractInput& input, const RuntimeContext& ctx)
    {
    ...
    Result<ContractResult> BootstrapContract::handle_snapshot(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)input;
        (void)ctx;
        ...
        auto resp_res = smo::bootstrap::handle_bootstrap_request(req, mesh_mgr_, authority_, governance_, crl_);
        if (!resp_res)
        {
        ...
        result.metrics["authority_count"] = ContextValue(static_cast<int64_t>(response.snapshot.authorities.size()));

        result.next_actions.push_back(
            emit_event("bootstrap.snapshot_delivered", "epoch=" + std::to_string(response.snapshot.epoch)));
    ...
    Result<ContractResult> BootstrapContract::handle_request(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        ...
        auto resp_res =
            smo::bootstrap::handle_bootstrap_request(req_res.value(), mesh_mgr_, authority_, governance_, crl_);
        if (!resp_res)
        {
        ...
        result.next_actions.push_back(
            emit_event("bootstrap.request_handled", "epoch=" + std::to_string(resp_res.value().snapshot.epoch)));

        return result;
    }

    // ── handle_info ──────────────────────────────────────────────────────
    Result<ContractResult> BootstrapContract::handle_info(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)input;
        (void)ctx;
    ...
    Result<ContractResult> BootstrapContract::handle_bootstrap_sync(const ContractInput& input,
                                                                    const RuntimeContext& ctx)
    {
        ...
        if (req.manifest_revision < current_epoch)
        {
            ...
            man_enc.encode_uint(1);
            man_enc.encode_string(cfg.mesh_id);
            man_enc.encode_uint(2);
            man_enc.encode_uint(current_epoch);
            man_enc.encode_uint(3);
            man_enc.encode_string(cfg.display_name.empty() ? cfg.mesh_id : cfg.display_name);
        ...
        if (req.membership_revision < membership_revision)
        {
            ...
            if (nodes_res)
            {
                ...
                for (auto& node : nodes)
                {
                    ...
                    mem_enc.encode_uint(1);
                    mem_enc.encode_string(node.node_id_hex);
                    mem_enc.encode_uint(2);
                    mem_enc.encode_string(node.role);
                    mem_enc.encode_uint(3);
                    mem_enc.encode_string(node.status);
                    mem_enc.encode_uint(4);
                    mem_enc.encode_uint(node.epoch);
                    mem_enc.encode_uint(5);
                    mem_enc.encode_int(node.last_seen);
                }
        ...
        if (crl_ && req.crl_revision < crl_revision)
        {
            ...
            if (!entries.empty())
            {
                ...
                for (auto& entry : entries)
                {
                    ...
                    crl_enc.encode_uint(1);
                    crl_enc.encode_string(entry.cert_fingerprint);
                    crl_enc.encode_uint(2);
                    crl_enc.encode_string(entry.node_id_hex);
                    crl_enc.encode_uint(3);
                    crl_enc.encode_uint(entry.epoch);
                    crl_enc.encode_uint(4);
                    crl_enc.encode_int(entry.revoked_at);
                }
        ...
        if (req.policy_revision < policy_revision && governance_)
        {
            ...
            if (!pending.empty())
            {
                ...
                for (auto& prop : pending)
                {
                    ...
                    pol_enc.encode_uint(1);
                    pol_enc.encode_uint(prop.id.value);
                    pol_enc.encode_uint(2);
                    pol_enc.encode_int(prop.created_at);
                    pol_enc.encode_uint(3);
                    pol_enc.encode_uint(static_cast<uint64_t>(prop.tier));
                }
        ...
        result.next_actions.push_back(
            emit_event("bootstrap.sync_delivered", "manifest_revision=" + std::to_string(resp.manifest_revision)));
