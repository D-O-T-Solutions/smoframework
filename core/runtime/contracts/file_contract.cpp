...
namespace smo::runtime {
    ...
    static Result<bool> map_bool(const ContextValue& args, const std::string& key)
    {
        auto s = map_str(args, key);
        if (!s)
            return {s.error()};
    ...
    static bool opt_bool(const ContextValue& args, const std::string& key, bool def)
    {
        auto v = map_bool(args, key);
        return v ? v.value() : def;
    }

    static int64_t opt_int(const ContextValue& args, const std::string& key, int64_t def)
    {
    ...
    static std::string perm_str(mode_t mode)
    {
        ...
        p += (mode & S_IXOTH) ? 'x' : '-';
        if (mode & S_ISUID)
            p[2] = (mode & S_IXUSR) ? 's' : 'S';
        if (mode & S_ISGID)
            p[5] = (mode & S_IXGRP) ? 's' : 'S';
        if (mode & S_ISVTX)
            p[8] = (mode & S_IXOTH) ? 't' : 'T';
        return p;
    }
    ...
    ContractMetadata FileContract::default_metadata()
    {
        ...
        meta.provides = {"list",  "mkdir", "remove", "copy",    "move",     "stat",     "read",
                         "write", "chmod", "chown",  "symlink", "readlink", "realpath", "info"};
    ...
    FileContract::FileContract() : NativeContract(default_metadata()) {}

    Result<ContractResult> FileContract::execute(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        if (input.method == "list")
            return handle_list(input, ctx);
        if (input.method == "mkdir")
            return handle_mkdir(input, ctx);
        if (input.method == "remove")
            return handle_remove(input, ctx);
        if (input.method == "copy")
            return handle_copy(input, ctx);
        if (input.method == "move")
            return handle_move(input, ctx);
        if (input.method == "stat")
            return handle_stat(input, ctx);
        if (input.method == "read")
            return handle_read(input, ctx);
        if (input.method == "write")
            return handle_write(input, ctx);
        if (input.method == "chmod")
            return handle_chmod(input, ctx);
        if (input.method == "chown")
            return handle_chown(input, ctx);
        if (input.method == "symlink")
            return handle_symlink(input, ctx);
        if (input.method == "readlink")
            return handle_readlink(input, ctx);
        if (input.method == "realpath")
            return handle_realpath(input, ctx);
    ...
    // ── handle_list ─────────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_list(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto path = map_str(input.arguments, "path");
        if (!path)
            return ContractResult::denied(path.error().message);
        ...
        auto out_entry = [&](const fs::directory_entry& e) {
            ...
            if (!show_hidden && name.size() > 0 && name[0] == '.')
                return;
            if (!first)
                oss << ",";
            first = false;
    ...
    // ── handle_mkdir ────────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_mkdir(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto path = map_str(input.arguments, "path");
        if (!path)
        ...
        if (ec)
            return ContractResult::denied(ec.message());
    ...
    // ── handle_remove ───────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_remove(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto path = map_str(input.arguments, "path");
        if (!path)
    ...
    // ── handle_copy ─────────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_copy(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto src = map_str(input.arguments, "src");
        if (!src)
            return ContractResult::denied(src.error().message);
        auto dst = map_str(input.arguments, "dst");
        if (!dst)
            return ContractResult::denied(dst.error().message);
    ...
    // ── handle_move ─────────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_move(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto src = map_str(input.arguments, "src");
        if (!src)
    ...
    // ── handle_stat ─────────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_stat(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto path = map_str(input.arguments, "path");
        if (!path)
        ...
        oss << "{"
            ...
            << "\"modified_ns\":" << static_cast<int64_t>(st.st_mtim.tv_sec) * 1'000'000'000LL + st.st_mtim.tv_nsec
            << "}";
    ...
    // ── handle_read ─────────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_read(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto path = map_str(input.arguments, "path");
        if (!path)
        ...
        if (!file)
            return ContractResult::denied("cannot open: " + path.value());
        ...
        int64_t to_read = (length < 0) ? (file_size - offset) : std::min(length, file_size - offset);
    ...
    // ── handle_write ────────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_write(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto path = map_str(input.arguments, "path");
        if (!path)
        ...
        if (!file)
            return ContractResult::denied("cannot open for write: " + path.value());
    ...
    // ── handle_chmod ────────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_chmod(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto path = map_str(input.arguments, "path");
        if (!path)
    ...
    // ── handle_chown ────────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_chown(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto path = map_str(input.arguments, "path");
        if (!path)
    ...
    // ── handle_symlink ──────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_symlink(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto target = map_str(input.arguments, "target");
        if (!target)
            return ContractResult::denied(target.error().message);
        auto link_path = map_str(input.arguments, "link_path");
        if (!link_path)
            return ContractResult::denied(link_path.error().message);
    ...
    // ── handle_readlink ─────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_readlink(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto path = map_str(input.arguments, "path");
        if (!path)
    ...
    // ── handle_realpath ─────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_realpath(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)ctx;
        auto path = map_str(input.arguments, "path");
        if (!path)
    ...
    // ── handle_info ─────────────────────────────────────────────────────
    Result<ContractResult> FileContract::handle_info(const ContractInput& input, const RuntimeContext& ctx)
    {
        (void)input;
        (void)ctx;
        ContractResult result = ContractResult::ok();
