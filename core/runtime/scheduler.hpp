...
namespace smo::runtime {
    ...
    struct RetryPolicy
    {
        ...
        std::function<bool(uint16_t error_code, const std::string& error)> is_retryable;
    };
    ...
    public:
        ...
        bool is_retryable(uint16_t error_code, const std::string& error_msg) const;

        // Calculate next delay
        int64_t next_delay(int attempt, uint16_t error_code) const;

        // Execute with retry
        template <typename Func> Result<std::string> execute_with_retry(Func&& func, const std::string& context = "")
        {
            int attempt = 0;
            ...
            while (attempt < policy_.max_attempts)
            {
                ...
                if (!is_retryable(result.error().code.code, last_error))
                {
                    return result.error();
                }
                ...
                {
                    int64_t delay = next_delay(attempt, result.error().code.code);
