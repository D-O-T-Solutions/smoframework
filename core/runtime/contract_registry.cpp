...
namespace smo::runtime {
    ...
    std::vector<std::string> ContractRegistry::list_contracts() const
    {
        ...
        for (const auto& [id, _] : contracts_)
            ids.push_back(id);
        return ids;
    }
    ...
    Result<void> ContractManager::load(const std::string& id)
    {
        ...
        if (lc.state != ContractLifecycleState::Registered && lc.state != ContractLifecycleState::Idle &&
    ...
    Result<void> ContractManager::initialize(const std::string& id, const ContractConfig& config)
    {
        ...
        auto& lc = lifecycle_[id];
        if (lc.state != ContractLifecycleState::Loaded && lc.state != ContractLifecycleState::InitFailed)
        {
    ...
    Result<void> ContractManager::shutdown(const std::string& id)
    {
        ...
        auto& lc = lifecycle_[id];
        if (lc.state != ContractLifecycleState::Ready && lc.state != ContractLifecycleState::Initialized)
        {
    ...
    ContractLifecycleState ContractManager::get_state(const std::string& id) const
    {
        ...
        if (it == lifecycle_.end())
            return ContractLifecycleState::Registered;
    ...
    {
        auto s = get_state(id);
        return s == ContractLifecycleState::LoadFailed || s == ContractLifecycleState::InitFailed ||
               s == ContractLifecycleState::InitTimeout || s == ContractLifecycleState::CrashLoop;
    }
