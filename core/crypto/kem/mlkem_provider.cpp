...
namespace smo {
    namespace kem {
        ...
        KeypairResult MLKEMProvider::generate_keypair(RngRef& rng)
        {
            ...
            if (!kem)
                throw std::runtime_error("ML-KEM: algorithm not available");
        ...
        EncapsResult MLKEMProvider::encapsulate(BytesView pubkey, RngRef& rng)
        {
            ...
            OQS_STATUS ret = OQS_KEM_encaps(kem, ciphertext.data(), shared_secret.data(), pubkey.data());
        ...
        Bytes MLKEMProvider::decapsulate(BytesView privkey, BytesView ciphertext)
        {
            ...
            OQS_STATUS ret = OQS_KEM_decaps(kem, shared_secret.data(), ciphertext.data(), privkey.data());
