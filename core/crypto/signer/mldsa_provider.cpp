...
namespace smo {
    namespace signer {
        ...
        KeypairResult MLDSAProvider::generate_keypair(RngRef& rng)
        {
            ...
            if (!sig)
                throw std::runtime_error("ML-DSA: algorithm not available");
        ...
        Bytes MLDSAProvider::sign(BytesView msg, BytesView secret_key, RngRef& rng)
        {
            ...
            OQS_STATUS ret = OQS_SIG_sign(sig, signature.data(), &sig_len, msg.data(), msg.size(), secret_key.data());
        ...
        bool MLDSAProvider::verify(BytesView msg, BytesView signature, BytesView public_key)
        {
            ...
            OQS_STATUS ret =
                OQS_SIG_verify(sig, msg.data(), msg.size(), signature.data(), signature.size(), public_key.data());
