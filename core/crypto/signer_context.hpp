...
namespace smo::crypto {
    ...
    {
    public:
        SoftwareSignerContext(BytesView secret_key, SignerImpl signer, SignerMetadata meta = {})
        ...
        SoftwareSignerContext(SoftwareSignerContext&& other) noexcept
            : secret_key_(std::move(other.secret_key_)), signer_(other.signer_), meta_(std::move(other.meta_)),
              valid_(other.valid_)
        {
            other.valid_ = false;
        }
        ...
        Result<Bytes> sign(BytesView msg, RngRef& rng) override
        {
            if (!valid_)
            {
                return SMO_ERR_CRYPTO(110, Error, NoRetry, ManualIntervention, "SignerContext has been destroyed");
            }
            if (!signer_.sign)
            {
                return SMO_ERR_CRYPTO(110, Error, NoRetry, ManualIntervention, "SignerImpl::sign is null");
            }
    ...
    // ── Factory ───────────────────────────────────────────────────────────
    inline std::unique_ptr<SignerContext> make_software_signer_context(BytesView secret_key, SignerImpl signer,
                                                                       SignerMetadata meta = {})
    {
