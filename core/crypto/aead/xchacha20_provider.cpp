...
namespace smo {
    namespace aead {
        ...
        Bytes XChaCha20Provider::encrypt(BytesView plaintext, BytesView aad, BytesView key, BytesView nonce)
        {
            ...
            crypto_aead_lock(result.data(), mac, key.data(), nonce.data(), aad.data(), aad.size(), plaintext.data(),
                             plaintext.size());
        ...
        Bytes XChaCha20Provider::decrypt(BytesView ciphertext, BytesView aad, BytesView key, BytesView nonce)
        {
            if (key.size() != kKeySize)
                throw std::runtime_error("XChaCha20: invalid key size");
            if (nonce.size() != kNonceSize)
            ...
            int ret = crypto_aead_unlock(result.data(), ciphertext.data() + pt_len, key.data(), nonce.data(),
                                         aad.data(), aad.size(), ciphertext.data(), pt_len);
