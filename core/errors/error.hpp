...
#pragma pack(push, 1)
    struct ErrorCode
    {
        ...
        constexpr ErrorCode() noexcept
            : category(ErrorCategory::Internal), code(0), severity(Severity::Error), retry(RetryClass::NoRetry),
              recovery(Recovery::None)
        {
        }

        constexpr ErrorCode(ErrorCategory cat, uint16_t c, Severity sev, RetryClass r, Recovery rec) noexcept
        ...
        {
            return retry == RetryClass::RetrySafe || retry == RetryClass::RetryBackoff;
        }

        constexpr bool is_fatal() const noexcept { return severity >= Severity::Critical; }
    };
#pragma pack(pop)
    ...
    {
        static_assert(!std::is_same_v<T, Error>, "Result<Error> is not allowed. Use Result<void> instead.");
    ...
    public:
        ...
        Result(Result&& other) noexcept(std::is_nothrow_move_constructible_v<T>) : has_value_(other.has_value_)
        {
            if (has_value_)
            {
...
#define SMO_ERR(cat, code, sev, retry, rec, msg)                                                                       \
    smo::Error(                                                                                                        \
        smo::ErrorCode(smo::ErrorCategory::cat, code, smo::Severity::sev, smo::RetryClass::retry, smo::Recovery::rec), \
        msg, __FILE__, __LINE__)

#define SMO_ERR_CRYPTO(code, sev, retry, rec, msg) SMO_ERR(Crypto, code, sev, retry, rec, msg)
#define SMO_ERR_IDENTITY(code, sev, retry, rec, msg) SMO_ERR(Identity, code, sev, retry, rec, msg)
#define SMO_ERR_CERT(code, sev, retry, rec, msg) SMO_ERR(Certificate, code, sev, retry, rec, msg)
#define SMO_ERR_TRANSPORT(code, sev, retry, rec, msg) SMO_ERR(Transport, code, sev, retry, rec, msg)
#define SMO_ERR_DISCOVERY(code, sev, retry, rec, msg) SMO_ERR(Discovery, code, sev, retry, rec, msg)
#define SMO_ERR_SESSION(code, sev, retry, rec, msg) SMO_ERR(Session, code, sev, retry, rec, msg)
#define SMO_ERR_PROTOCOL(code, sev, retry, rec, msg) SMO_ERR(Protocol, code, sev, retry, rec, msg)
#define SMO_ERR_RUNTIME(code, sev, retry, rec, msg) SMO_ERR(Runtime, code, sev, retry, rec, msg)
#define SMO_ERR_GOVERNANCE(code, sev, retry, rec, msg) SMO_ERR(Governance, code, sev, retry, rec, msg)
#define SMO_ERR_ACL(code, sev, retry, rec, msg) SMO_ERR(Governance, code, sev, retry, rec, msg)
#define SMO_ERR_STORAGE(code, sev, retry, rec, msg) SMO_ERR(Storage, code, sev, retry, rec, msg)
#define SMO_ERR_INTERNAL(code, sev, retry, rec, msg) SMO_ERR(Internal, code, sev, retry, rec, msg)
#define SMO_ERR_COMPILER(code, sev, retry, rec, msg) SMO_ERR(Compiler, code, sev, retry, rec, msg)
#define SMO_ERR_TRUST(code, sev, retry, rec, msg) SMO_ERR(Trust, code, sev, retry, rec, msg)
