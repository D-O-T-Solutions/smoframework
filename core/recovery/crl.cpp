...
namespace smo::recovery {
    ...
    Result<void> CRL::revoke(const std::string& cert_fingerprint, const std::string& node_id_hex,
                             const std::string& reason, uint64_t epoch, int64_t now)
    {
        ...
        for (const auto& entry : entries_)
        {
            ...
            {
                return Error(ErrorCode(ErrorCategory::Trust, 207, Severity::Warn, RetryClass::NoRetry, Recovery::None),
                             "certificate already revoked: " + cert_fingerprint, __FILE__, __LINE__);
            }
        }
