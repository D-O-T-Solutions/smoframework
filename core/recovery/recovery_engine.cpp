...
namespace smo::recovery {
    ...
    RecoveryEngine::RecoveryEngine(RecoveryConfig config) : config_(std::move(config)) {}

    RecoveryMode RecoveryEngine::assess_mode(uint32_t total, uint32_t online, int quorum_threshold) const
    {
        if (online == 0)
            return RecoveryMode::Hard;
        if (static_cast<int>(online) < quorum_threshold)
            return RecoveryMode::Hard;
        if (online < total)
    ...
    Result<RecoverySession> RecoveryEngine::start_soft(const std::string& mesh_id, const std::string& root_node_id,
                                                       ...
                                                       int64_t now_ns)
    {
        ...
        if (!pkg_valid)
        {
            return SMO_ERR_RECOVERY(1503, Error, NoRetry, ManualIntervention, "recovery package passphrase mismatch");
        }
    ...
    Result<RecoverySession> RecoveryEngine::start_hard(const std::string& mesh_id, const std::string& root_node_id,
                                                       uint64_t current_epoch, const std::string& recovery_passphrase,
                                                       int64_t now_ns)
    {
        auto pkg_valid = verify_recovery_package(recovery_passphrase);
    ...
    Result<void> RecoveryEngine::add_signature(RecoverySession& session, const GovernanceSignature& sig, int64_t now_ns)
    {
        if (!session.is_valid(now_ns))
        {
            session.state = RecoveryState::Failed;
            return SMO_ERR_RECOVERY(1501, Warn, NoRetry, ManualIntervention, "recovery session expired");
        }
        if (session.state != RecoveryState::AwaitingRoot && session.state != RecoveryState::Signing)
        {
            return SMO_ERR_RECOVERY(1502, Error, NoRetry, ManualIntervention, "session not in signable state");
        }
    ...
    Result<void> RecoveryEngine::execute(RecoverySession& session, int64_t now_ns)
    {
        ...
        {
            return SMO_ERR_RECOVERY(1502, Error, NoRetry, ManualIntervention, "session not ready for execution");
        }
