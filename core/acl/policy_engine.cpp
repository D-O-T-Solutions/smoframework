...
namespace smo::acl {
    ...
    struct PolicyEngine::Impl
    {
        ...
        Impl(const Config& config) : config(config), cache_ttl_ns_(config.cache_ttl_ns) { load_builtin_presets(); }
        ...
        Result<void> load_policy_file(const std::filesystem::path& path)
        {
                ...
                if (doc["rules"])
                {
                    for (const auto& rule_node : doc["rules"])
                    {
                        ...
                        if (rule_node["effect"])
                        {
                            ...
                            if (effect == "allow")
                                rule.effect = PolicyDecision::Allow;
                            else if (effect == "deny")
                                rule.effect = PolicyDecision::Deny;
                            else if (effect == "conditional")
                                rule.effect = PolicyDecision::Conditional;
                            else if (effect == "audit")
                                rule.effect = PolicyDecision::Audit;
                            else if (effect == "sandbox")
                                rule.effect = PolicyDecision::Sandbox;
                            else if (effect == "ratelimit")
                                rule.effect = PolicyDecision::RateLimit;
                            else if (effect == "readonly")
                                rule.effect = PolicyDecision::ReadOnly;
                        }
                        if (rule_node["min_trust"])
                            rule.min_trust_score = rule_node["min_trust"].as<int32_t>();
                        if (rule_node["max_trust"])
                            rule.max_trust_score = rule_node["max_trust"].as<int32_t>();
            ...
            {
                return SMO_ERR_ACL(100, Error, NoRetry, None, "YAML parse error: " + std::string(e.what()));
            }
        ...
        Result<bool> check_capability(const std::string& actor_id, const std::string& capability,
        ...
        Result<std::vector<std::string>> get_effective_capabilities(const std::string& actor_id,
                                                                    const std::string& target_id)
        {
    ...
    private:
        ...
        {
            std::string key = context.session_id + "|" + context.request_contract_id + "|" + context.request_method +
                              "|" + context.session_mesh_id;
            return key;
        }
    };
...
namespace smo::acl {
    ...
    Result<bool> PolicyEngine::check_capability(const std::string& actor_id, const std::string& capability,
                                                const std::string& target_id)
    {
        return impl_->check_capability(actor_id, capability, target_id);
    }

    Result<std::vector<std::string>> PolicyEngine::get_effective_capabilities(const std::string& actor_id,
                                                                              const std::string& target_id)
    {
