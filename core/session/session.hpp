...
namespace smo {
    ...
    namespace SessionErrc {
        inline constexpr ErrorCode OpenFailed(ErrorCategory::Session, 500, Severity::Error, RetryClass::RetrySafe,
                                              Recovery::Reconnect);
        inline constexpr ErrorCode Closed(ErrorCategory::Session, 501, Severity::Info, RetryClass::RetrySafe,
                                          Recovery::Reconnect);
        inline constexpr ErrorCode Expired(ErrorCategory::Session, 502, Severity::Warn, RetryClass::RetrySafe,
                                           Recovery::Reconnect);
        inline constexpr ErrorCode Rejected(ErrorCategory::Session, 503, Severity::Error, RetryClass::NoRetry,
                                            Recovery::Reconnect);
        inline constexpr ErrorCode CapExceeded(ErrorCategory::Session, 504, Severity::Warn, RetryClass::NoRetry,
                                               Recovery::None);
        inline constexpr ErrorCode CapNotGranted(ErrorCategory::Session, 507, Severity::Error, RetryClass::NoRetry,
                                                 Recovery::None);
    } // namespace SessionErrc
    ...
    public:
        ...
        static Result<Session> create(SessionId id, NodeID peer_id, Certificate peer_cert, CapabilitySet capabilities,
        ...
        bool is_valid_at(int64_t now) const noexcept { return state_ != SessionState::Closed && now < expires_at_; }

        // Verify a capability is in the session's set
        bool has_capability(Capability cap) const noexcept { return capabilities_.test(static_cast<size_t>(cap)); }
