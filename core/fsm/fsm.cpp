...
namespace smo {
    ...
    Result<void> FsmInstance::on_event(FsmEvent event)
    {
        ...
        {
            return SMO_ERR_RUNTIME(701, Error, NoRetry, RestartFSM, "no transitions defined for current state");
        }

        auto event_it = state_it->second.find(event);
        if (event_it == state_it->second.end())
        {
            return SMO_ERR_RUNTIME(700, Error, NoRetry, RestartFSM, "invalid transition for current state");
        }
        ...
        for (size_t i = 0; i < 8; ++i)
        {
            hash[i] = static_cast<uint8_t>(
                (reinterpret_cast<const uint8_t*>(&current_state_)[i] ^ reinterpret_cast<const uint8_t*>(&event)[i] ^
                 reinterpret_cast<const uint8_t*>(&next_state)[i] ^ reinterpret_cast<const uint8_t*>(&now)[i]));
        }

        uint64_t elapsed = history_.empty() ? 0 : now - history_.back().timestamp_ns;

        history_.push_back({current_state_, event, next_state, elapsed, std::move(hash), static_cast<int64_t>(now)});
    ...
    Result<void> FsmInstance::on_timeout()
    {
        ...
        if (it == timeouts_.end())
        {
            return SMO_ERR_RUNTIME(701, Error, NoRetry, RestartFSM, "no timeout config for current state");
        }

        const auto& cfg = it->second;
        if (cfg.max_dwell_ns == 0)
        {
            return SMO_ERR_RUNTIME(701, Error, NoRetry, RestartFSM, "current state has no timeout");
        }
    ...
    Result<FsmInstance> FsmInstance::deserialize(BytesView data, const TransitionRule* rules, size_t rule_count,
                                                 const StateTimeout* timeouts, size_t timeout_count)
    {
        ...
        {
            return SMO_ERR_RUNTIME(702, Critical, NoRetry, RebootNode, "truncated FSM serialized state");
        }
        ...
        for (uint32_t i = 0; i < hist_count; ++i)
        {
            ...
            {
                return SMO_ERR_RUNTIME(702, Critical, NoRetry, RebootNode, "truncated transition record");
            }
            ...
            {
                return SMO_ERR_RUNTIME(702, Critical, NoRetry, RebootNode, "truncated state hash");
            }
