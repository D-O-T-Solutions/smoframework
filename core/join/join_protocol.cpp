...
namespace smo::join {
    ...
    Bytes JoinRequest::encode_cbor() const
    {
        ...
        enc.encode_uint(K_REQ_TOKEN);
        enc.encode_string(token);
        enc.encode_uint(K_REQ_CSR);
        enc.encode_string(csr_pem);
        if (timestamp > 0)
        {
            enc.encode_uint(K_REQ_TIMESTAMP);
        ...
        if (csr_hash.size() > 0)
        {
            enc.encode_uint(K_REQ_CSR_HASH);
            enc.encode_bytes(BytesView(csr_hash));
        }
        if (request_signature.size() > 0)
        {
            enc.encode_uint(K_REQ_SIG);
            enc.encode_bytes(BytesView(request_signature));
        }
        if (capability_bitmap > 0)
        {
    ...
    Result<JoinRequest> JoinRequest::decode_cbor(BytesView data)
    {
        ...
        for (size_t i = 0; i < map_sz.value(); ++i)
        {
            ...
            case K_REQ_CSR: {
                ...
                if (!v)
                    return v.error();
                req.csr_pem = std::move(v.value());
                break;
            }
            case K_REQ_TIMESTAMP: {
                auto v = dec.decode_int();
                if (!v)
                    return v.error();
                req.timestamp = v.value();
                break;
            }
            case K_REQ_NONCE: {
            ...
            case K_REQ_CSR_HASH: {
                ...
                if (!v)
                    return v.error();
                req.csr_hash = Bytes(v.value().begin(), v.value().end());
                break;
            }
            case K_REQ_SIG: {
                auto v = dec.decode_bytes();
                if (!v)
                    return v.error();
                req.request_signature = Bytes(v.value().begin(), v.value().end());
                break;
            }
            case K_REQ_PROTOCOL_VER: {
                ...
                if (!v)
                    return v.error();
                req.protocol_version = static_cast<uint8_t>(v.value());
                break;
            }
    ...
    {
        enc.encode_map(4);
        enc.encode_uint(K_SEED_ENDPOINT);
        enc.encode_string(si.endpoint);
        if (!si.region.empty())
        {
            enc.encode_uint(K_SEED_REGION);
            enc.encode_string(si.region);
        }
        enc.encode_uint(K_SEED_WEIGHT);
        enc.encode_uint(si.weight);
    ...
    static SeedInfo decode_seed_info(cbor::Decoder& dec)
    {
        ...
        for (size_t i = 0; i < map_sz.value(); ++i)
        {
            ...
            switch (key.value())
            {
            case K_SEED_ENDPOINT: {
                auto v = dec.decode_string();
                if (v)
                    si.endpoint = std::move(v.value());
                break;
            }
            case K_SEED_REGION: {
                auto v = dec.decode_string();
                if (v)
                    si.region = std::move(v.value());
                break;
            }
            case K_SEED_WEIGHT: {
                auto v = dec.decode_uint();
                if (v)
                    si.weight = static_cast<uint32_t>(v.value());
                break;
            }
            case K_SEED_HEALTH_SCORE: {
                auto v = dec.decode_uint();
                if (v)
                {
                    uint64_t f = v.value();
                    std::memcpy(&si.health_score, &f, sizeof(double));
                }
                break;
            }
            default: {
    ...
    Bytes JoinResponse::encode_cbor() const
    {
        ...
        if (!certificate_pem.empty())
        {
            enc.encode_uint(K_RESP_CERT);
            enc.encode_string(certificate_pem);
        }
        if (!mesh_id.empty())
        {
            enc.encode_uint(K_RESP_MESH_ID);
            enc.encode_string(mesh_id);
        }
        if (!bootstrap_ticket.empty())
        {
            enc.encode_uint(K_RESP_BOOT_TICKET);
            enc.encode_bytes(BytesView(bootstrap_ticket));
        }
        enc.encode_uint(K_RESP_NONCE);
        enc.encode_bytes(BytesView(nonce.data(), nonce.size()));
        if (server_time > 0)
        {
            enc.encode_uint(K_RESP_SERVER_TIME);
            enc.encode_int(server_time);
        }
        if (capability_bitmap > 0)
        {
    ...
    Result<JoinResponse> JoinResponse::decode_cbor(BytesView data)
    {
        ...
        for (size_t i = 0; i < map_sz.value(); ++i)
        {
            ...
            case K_RESP_CERT: {
                ...
                if (!v)
                    return v.error();
                resp.certificate_pem = std::move(v.value());
                break;
            }
            case K_RESP_MESH_ID: {
                auto v = dec.decode_string();
            ...
            case K_RESP_BOOT_TICKET: {
                ...
                if (!v)
                    return v.error();
                resp.bootstrap_ticket = Bytes(v.value().begin(), v.value().end());
                break;
            }
            case K_RESP_NONCE: {
                auto v = dec.decode_bytes();
    ...
    Bytes BootstrapSyncRequest::encode_cbor() const
    {
        ...
        if (policy_revision > 0)
            fields++;
        enc.encode_map(fields);
        enc.encode_uint(K_SYNC_REQ_PROTOCOL_VER);
        enc.encode_uint(protocol_version);
        if (!mesh_id.empty())
        {
            enc.encode_uint(K_SYNC_REQ_MESH_ID);
            enc.encode_string(mesh_id);
        }
        if (!node_id.empty())
        {
            enc.encode_uint(K_SYNC_REQ_NODE_ID);
            enc.encode_string(node_id);
        }
        if (!bootstrap_ticket.empty())
        {
            enc.encode_uint(K_SYNC_REQ_BOOT_TICKET);
            enc.encode_bytes(BytesView(bootstrap_ticket));
        }
        if (manifest_revision > 0)
        {
            enc.encode_uint(K_SYNC_REQ_MF_REV);
            enc.encode_uint(manifest_revision);
        }
        if (crl_revision > 0)
        {
            enc.encode_uint(K_SYNC_REQ_CRL_REV);
            enc.encode_uint(crl_revision);
        }
        if (membership_revision > 0)
        {
            enc.encode_uint(K_SYNC_REQ_MEMBER_REV);
            enc.encode_uint(membership_revision);
        }
        if (policy_revision > 0)
        {
            enc.encode_uint(K_SYNC_REQ_POLICY_REV);
            enc.encode_uint(policy_revision);
        }
        return enc.take();
    }
    ...
    Result<BootstrapSyncRequest> BootstrapSyncRequest::decode_cbor(BytesView data)
    {
        ...
        for (size_t i = 0; i < map_sz.value(); ++i)
        {
            ...
            case K_SYNC_REQ_BOOT_TICKET: {
                ...
                if (!v)
                    return v.error();
                req.bootstrap_ticket = Bytes(v.value().begin(), v.value().end());
                break;
            }
            ...
            case K_SYNC_REQ_MEMBER_REV: {
                ...
                if (!v)
                    return v.error();
                req.membership_revision = v.value();
                break;
            }
    ...
    Bytes BootstrapSyncResponse::encode_cbor() const
    {
        cbor::Encoder enc;
        int fields = 0;
        if (!manifest_delta.empty())
            fields++;
        if (!membership_delta.empty())
            fields++;
        if (!policy_delta.empty())
            fields++;
        if (!crl_delta.empty())
            fields++;
        if (manifest_revision > 0)
        ...
        if (!manifest_delta.empty())
        {
            enc.encode_uint(K_SYNC_RESP_MF_DELTA);
            enc.encode_bytes(BytesView(manifest_delta));
        }
        if (!membership_delta.empty())
        {
            enc.encode_uint(K_SYNC_RESP_MEMBER_DELTA);
            enc.encode_bytes(BytesView(membership_delta));
        }
        if (!policy_delta.empty())
        {
            enc.encode_uint(K_SYNC_RESP_POLICY_DELTA);
            enc.encode_bytes(BytesView(policy_delta));
        }
        if (!crl_delta.empty())
        {
            enc.encode_uint(K_SYNC_RESP_CRL_DELTA);
            enc.encode_bytes(BytesView(crl_delta));
        }
        if (manifest_revision > 0)
        {
            enc.encode_uint(K_SYNC_RESP_MF_REV);
            enc.encode_uint(manifest_revision);
        }
        if (membership_revision > 0)
        {
            enc.encode_uint(K_SYNC_RESP_MEMBER_REV);
            enc.encode_uint(membership_revision);
        }
        if (crl_revision > 0)
        {
            enc.encode_uint(K_SYNC_RESP_CRL_REV);
            enc.encode_uint(crl_revision);
        }
        if (policy_revision > 0)
        {
            enc.encode_uint(K_SYNC_RESP_POLICY_REV);
            enc.encode_uint(policy_revision);
        }
        if (!seeds.empty())
        {
            ...
            for (auto& si : seeds)
                encode_seed_info(enc, si);
        }
        if (server_time > 0)
        {
            enc.encode_uint(K_SYNC_RESP_SERVER_TIME);
            enc.encode_int(server_time);
        }
        return enc.take();
    }
    ...
    Result<BootstrapSyncResponse> BootstrapSyncResponse::decode_cbor(BytesView data)
    {
        ...
        for (size_t i = 0; i < map_sz.value(); ++i)
        {
            ...
            case K_SYNC_RESP_MF_DELTA: {
                ...
                if (!v)
                    return v.error();
                resp.manifest_delta = Bytes(v.value().begin(), v.value().end());
                break;
            }
            case K_SYNC_RESP_MEMBER_DELTA: {
                ...
                if (!v)
                    return v.error();
                resp.membership_delta = Bytes(v.value().begin(), v.value().end());
                break;
            }
            case K_SYNC_RESP_POLICY_DELTA: {
                ...
                if (!v)
                    return v.error();
                resp.policy_delta = Bytes(v.value().begin(), v.value().end());
                break;
            }
            case K_SYNC_RESP_CRL_DELTA: {
                ...
                if (!v)
                    return v.error();
                resp.crl_delta = Bytes(v.value().begin(), v.value().end());
                break;
            }
            case K_SYNC_RESP_MF_REV: {
                ...
                if (!v)
                    return v.error();
                resp.manifest_revision = v.value();
                break;
            }
            case K_SYNC_RESP_MEMBER_REV: {
                ...
                if (!v)
                    return v.error();
                resp.membership_revision = v.value();
                break;
            }
    ...
    std::vector<smo::TransitionRule> join_transition_table()
    {
        ...
        return {
            ...
            {static_cast<int64_t>(WAIT_RESPONSE), static_cast<int64_t>(RESPONSE_RCVD),
             static_cast<int64_t>(CERT_RECEIVED)},

            // CERT_VERIFY — new state per §5.21
            {static_cast<int64_t>(CERT_RECEIVED), static_cast<int64_t>(CERT_VERIFIED),
             static_cast<int64_t>(CERT_VERIFY)},
            {static_cast<int64_t>(CERT_RECEIVED), static_cast<int64_t>(CERT_INVALID), static_cast<int64_t>(FAILED)},

            // Bootstrap sync
            {static_cast<int64_t>(CERT_VERIFY), static_cast<int64_t>(SYNC_REQUESTED),
             static_cast<int64_t>(BOOTSTRAP_SYNC)},
            {static_cast<int64_t>(BOOTSTRAP_SYNC), static_cast<int64_t>(SYNC_COMPLETE),
             static_cast<int64_t>(WAIT_SYNC)},

            // Gossip sync
            {static_cast<int64_t>(WAIT_SYNC), static_cast<int64_t>(GOSSIP_STARTED), static_cast<int64_t>(GOSSIP_SYNC)},
            {static_cast<int64_t>(GOSSIP_SYNC), static_cast<int64_t>(GOSSIP_COMPLETE),
             static_cast<int64_t>(WAIT_GOSSIP)},
    ...
    namespace {
        ...
        struct NonceCache
        {
            ...
            bool check_and_insert(const std::string& mesh_id, const std::array<uint8_t, 8>& nonce, int64_t ttl_sec,
                                  int64_t now_sec)
            {
    ...
    Result<JoinResponse> process_join_request(const JoinRequest& req, MeshManager& mesh_mgr,
                                              authority::MeshAuthority& authority)
    {
        ...
        if (!token_result)
        {
            return SMO_ERR_CERT(213, Error, NoRetry, None, "Invalid join token: " + token_result.error().message);
        }
        ...
        {
            return SMO_ERR_CERT(214, Error, NoRetry, None, "JoinRequest timestamp outside ±30s window");
        }
        ...
        if (!g_nonce_cache.check_and_insert(token.mesh_id, req.nonce, token.expiry_unix_sec, now_sec))
        {
            return SMO_ERR_CERT(219, Error, NoRetry, None, "JoinRequest nonce replay detected");
        }
        ...
        // Used by BOOTSTRAP_SYNC to authenticate the node.
        {
            ...
            enc.encode_map(4);
            enc.encode_uint(1);
            enc.encode_string(token.mesh_id);
            enc.encode_uint(2);
    ...
    Result<BootstrapSyncResponse> process_bootstrap_sync(const BootstrapSyncRequest& req, MeshManager& mesh_mgr,
                                                         authority::MeshAuthority& authority, recovery::CRL* crl)
    {
        auto mesh_res = mesh_mgr.get_current_mesh();
        if (!mesh_res)
        {
            return SMO_ERR_PROTOCOL(900, Error, NoRetry, None, "No active mesh for bootstrap sync");
        }
        ...
        if (req.manifest_revision < current_epoch)
        {
            ...
            enc.encode_map(4);
            enc.encode_uint(1);
            enc.encode_string(cfg.mesh_id);
            enc.encode_uint(2);
            enc.encode_uint(current_epoch);
            enc.encode_uint(3);
            enc.encode_string(cfg.display_name.empty() ? cfg.mesh_id : cfg.display_name);
            enc.encode_uint(4);
        ...
        if (req.membership_revision < current_epoch)
        {
            ...
            if (nodes && !nodes.value().empty())
            {
                ...
                for (auto& n : nodes.value())
                {
                    enc.encode_map(5);
                    enc.encode_uint(1);
                    enc.encode_string(n.node_id_hex);
                    enc.encode_uint(2);
                    enc.encode_string(n.role);
                    enc.encode_uint(3);
                    enc.encode_string(n.status);
                    enc.encode_uint(4);
                    enc.encode_uint(n.epoch);
                    enc.encode_uint(5);
                    enc.encode_int(n.last_seen);
                }
        ...
        if (crl && req.crl_revision < current_epoch)
        {
            ...
            if (!entries.empty())
            {
                ...
                for (auto& e : entries)
                {
                    enc.encode_map(4);
                    enc.encode_uint(1);
                    enc.encode_string(e.cert_fingerprint);
                    enc.encode_uint(2);
                    enc.encode_string(e.node_id_hex);
                    enc.encode_uint(3);
                    enc.encode_uint(e.epoch);
                    enc.encode_uint(4);
                    enc.encode_int(e.revoked_at);
                }
