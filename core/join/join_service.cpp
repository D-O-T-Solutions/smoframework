...
namespace smo::join {
    ...
    public:
        ...
        void reset_fsm() { fsm_.reset(static_cast<int64_t>(JoinState::NEW)); }
        ...
        Result<void> load_fsm_state()
        {
            ...
            auto result =
                FsmInstance::deserialize(BytesView(data), rules.data(), rules.size(), timeouts.data(), timeouts.size());
            if (!result)
                return result.error();
        ...
        JoinState current_state() const { return static_cast<JoinState>(fsm_.current_state()); }
    ...
    JoinService::JoinService(MeshManager& mesh_mgr, authority::MeshAuthority& authority, Config config)
    ...
    Result<void> JoinService::initialize()
    {
    ...
    Result<void> JoinService::reset_fsm()
    {
        impl_->reset_fsm();
        return {};
    }
    Result<void> JoinService::save_fsm_state()
    {
        return impl_->save_fsm_state();
    }
    Result<void> JoinService::load_fsm_state()
    {
        return impl_->load_fsm_state();
    }
    JoinState JoinService::current_state() const
    {
