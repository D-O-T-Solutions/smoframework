...
namespace smo::contract::native {
    ...
    Result<ProcExecResponse> proc_exec(const ProcExecRequest& req)
    {
        ...
        if (pipe(stdout_pipe) != 0 || pipe(stderr_pipe) != 0 || (!req.stdin_data.empty() && pipe(stdin_pipe) != 0))
        {
        ...
        auto deadline = std::chrono::steady_clock::now() + std::chrono::nanoseconds(req.timeout_ns);
    ...
    Result<ProcPSResponse> proc_ps(const ProcPSRequest& req)
    {
        ...
        while ((entry = readdir(dir)) != nullptr)
        {
            ...
            if (!req.user.empty() && proc.user != req.user)
                continue;
        ...
        for (int i = 0; i < req.iterations; ++i)
        {
            ...
            for (const auto& line : lines)
            {
                if (line.rfind("cpu ", 0) == 0)
                {
                    ...
                    {
                        // Calculate CPU usage
                        snap.cpu_total = std::stod(parts[1]) + std::stod(parts[2]) + std::stod(parts[3]) +
                                         std::stod(parts[4]) + std::stod(parts[5]) + std::stod(parts[6]) +
                                         std::stod(parts[7]);
                    }
                    break;
                }
    ...
    Result<SystemdResponse> systemd_control(const SystemdRequest& req)
    {
        ...
        switch (req.action)
        {
        case SystemdRequest::Action::Start:
            cmd += "start ";
            break;
        case SystemdRequest::Action::Stop:
            cmd += "stop ";
            break;
        case SystemdRequest::Action::Restart:
            cmd += "restart ";
            break;
        case SystemdRequest::Action::Reload:
            cmd += "reload ";
            break;
        case SystemdRequest::Action::Enable:
            cmd += "enable ";
            break;
        case SystemdRequest::Action::Disable:
            cmd += "disable ";
            break;
        case SystemdRequest::Action::Status:
            cmd += "status ";
            break;
        case SystemdRequest::Action::IsActive:
            cmd += "is-active ";
            break;
        case SystemdRequest::Action::IsEnabled:
            cmd += "is-enabled ";
            break;
        }
    ...
    Result<ServiceResponse> service_control(const ServiceRequest& req)
    {
        ...
        switch (req.action)
        {
        case ServiceRequest::Action::Start:
            action = "start";
            break;
        case ServiceRequest::Action::Stop:
            action = "stop";
            break;
        case ServiceRequest::Action::Restart:
            action = "restart";
            break;
        case ServiceRequest::Action::Status:
            action = "status";
            break;
        }
        ...
        resp.output = output;
        resp.success = output.find("OK") != std::string::npos || output.find("running") != std::string::npos ||
                       output.find("active") != std::string::npos;
        resp.running = output.find("running") != std::string::npos || output.find("active") != std::string::npos;
