    ...
    public:
        ...
        Result<std::string> deploy(const WorkflowDeployRequest& req)
        {
            ...
            std::string instance_id =
                "wf_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
        ...
        Result<void> validate(const YAML::Node& doc)
        {
            ...
            {
                return SMO_ERR(Contract, 101, Error, NoRetry, None, "Missing required fields: name, version, steps");
            }

            if (!doc["steps"].IsSequence() || doc["steps"].size() == 0)
            {
                return SMO_ERR(Contract, 102, Error, NoRetry, None, "At least one step required");
            }
            ...
            for (const auto& step : doc["steps"])
            {
                if (!step["name"])
                {
                    return SMO_ERR(Contract, 103, Error, NoRetry, None, "Step missing name");
                }
                std::string name = step["name"].as<std::string>();
                if (step_names.count(name))
                {
                    return SMO_ERR(Contract, 104, Error, NoRetry, None, "Duplicate step name: " + name);
                }
        ...
        Result<std::string> stop(const WorkflowStopRequest& req) { return "stopped"; }

        Result<std::string> status(const WorkflowStatusRequest& req) { return "running"; }

        Result<std::string> logs(const WorkflowLogRequest& req) { return "logs"; }

        Result<std::string> scale(const WorkflowScaleRequest& req) { return "scaled"; }

        Result<std::string> rollback(const WorkflowRollbackRequest& req) { return "rolled_back"; }

        Result<std::string> upgrade(const WorkflowUpgradeRequest& req) { return "upgraded"; }

        Result<std::string> validate(const WorkflowValidateRequest& req) { return "valid"; }
