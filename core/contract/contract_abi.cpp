...
namespace smo {
    ...
    std::string ContractABI::to_canonical_json() const
    {
        ...
        for (auto& p : input_schema.properties)
        {
            ...
            if (p.default_value)
                os << ",\"default\":\"" << escape_json(*p.default_value) << "\"";
            os << "}";
        }
        os << "]},";
    ...
    bool AbiRegistry::verify_compatibility(const ContractID& id, const std::string& runtime_version) const
    {
        auto abi = get_abi(id);
        if (!abi)
            return false;
        return abi->min_runtime_version <= runtime_version && runtime_version <= abi->max_runtime_version;
    }
