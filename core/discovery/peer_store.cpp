...
namespace smo {
    ...
    static const char kPeerSchema[] = "CREATE TABLE IF NOT EXISTS peers ("
    ...
    static const char kEventSchema[] = "CREATE TABLE IF NOT EXISTS peer_events ("
    ...
    static const char kPeerIndexes[] = "CREATE INDEX IF NOT EXISTS idx_peers_display_name ON peers(display_name);"
    ...
    PeerRecord PeerStore::row_to_peer_record(const Statement& stmt) const
    {
        ...
        rec.rtt_ms = stmt.column_text(17).empty() ? 0.0 : std::stod(stmt.column_text(17));
        return rec;
    }

    void PeerStore::bind_peer_record(Statement& stmt, const PeerRecord& rec, int& idx)
    {
    ...
    {
        auto stmt = db_.prepare("INSERT OR REPLACE INTO peers "
    ...
    {
        auto stmt = db_.prepare("SELECT * FROM peers WHERE node_id = ?1;");
        if (!stmt)
            return stmt.error();
        ...
        {
            return SMO_ERR_DISCOVERY(400, Info, RetrySafe, None, "peer not found in store");
        }
    ...
    {
        auto stmt = db_.prepare("SELECT * FROM peers WHERE display_name = ?1 LIMIT 1;");
        if (!stmt)
            return stmt.error();
    ...
    Result<std::vector<PeerRecord>> PeerStore::peers_by_tag(std::string_view tag) const
    {
        ...
        auto stmt = db_.prepare("SELECT * FROM peers WHERE tags LIKE ?1 ORDER BY display_name;");
        if (!stmt)
            return stmt.error();
    ...
    {
        auto stmt = db_.prepare("SELECT * FROM peers WHERE platform = ?1 ORDER BY display_name;");
        if (!stmt)
            return stmt.error();
    ...
    {
        auto stmt = db_.prepare("SELECT * FROM peers WHERE arch = ?1 ORDER BY display_name;");
        if (!stmt)
            return stmt.error();
    ...
    {
        auto stmt = db_.prepare("SELECT * FROM peers WHERE mesh_name = ?1 ORDER BY display_name;");
        if (!stmt)
            return stmt.error();
    ...
    {
        auto stmt = db_.prepare("SELECT * FROM peers WHERE state = ?1 ORDER BY display_name;");
        if (!stmt)
            return stmt.error();
    ...
    Result<void> PeerStore::record_event(PeerEventType type, const NodeID& id, Bytes payload)
    {
        auto stmt = db_.prepare("INSERT INTO peer_events (node_id, event_type, payload, created_at)"
                                " VALUES (?1, ?2, ?3, ?4);");
        if (!stmt)
            return stmt.error();
    ...
    {
        auto stmt = db_.prepare("SELECT id, node_id, event_type, payload, created_at"
