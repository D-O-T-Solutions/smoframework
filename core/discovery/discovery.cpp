...
namespace smo {
    ...
    Result<PeerRecord> PeerRecord::deserialize(BytesView data)
    {
        ...
        if (off + 32 > data.size())
        {
            return SMO_ERR_DISCOVERY(400, Info, RetrySafe, None, "truncated node_id in PeerRecord");
        }
        ...
        if (off >= data.size())
        {
            return SMO_ERR_DISCOVERY(400, Info, RetrySafe, None, "truncated state in PeerRecord");
        }
        ...
        auto read_str = [&]() -> std::string {
            ...
            if (off + len > data.size())
                return {};
            std::string s(data.begin() + static_cast<ptrdiff_t>(off), data.begin() + static_cast<ptrdiff_t>(off + len));
            off += len;
            return s;
        };
    ...
    Result<void> MembershipTable::upsert(PeerRecord record)
    {
        ...
        {
            return SMO_ERR_DISCOVERY(405, Warn, NoRetry, None, "membership table at capacity");
        }
    ...
    Result<PeerRecord> MembershipTable::lookup(const NodeID& id) const
    {
        ...
        if (it == records_.end())
        {
            return SMO_ERR_DISCOVERY(400, Info, RetrySafe, None, "peer not found in membership table");
        }
        return it->second;
    }
    ...
    Result<PeerRecord> MembershipTable::lookup_by_name(const std::string& name) const
    {
        ...
        return SMO_ERR_DISCOVERY(400, Info, RetrySafe, None, "no peer with that name");
    }
    ...
    Result<void> MembershipTable::remove(const NodeID& id)
    {
        ...
        if (it == records_.end())
        {
            return SMO_ERR_DISCOVERY(400, Info, RetrySafe, None, "peer not found for removal");
        }
        records_.erase(it);
        return {};
    }
    ...
    void HealthMonitor::tick(MembershipTable& table, int64_t now, int64_t ping_timeout_ns, int max_misses)
    {
    ...
    Result<HelloMsg> HelloMsg::deserialize(BytesView data)
    {
        ...
        if (off + 32 > data.size())
        {
            return SMO_ERR_DISCOVERY(400, Info, RetrySafe, None, "truncated HelloMsg node_id");
        }
    ...
    Result<WelcomeMsg> WelcomeMsg::deserialize(BytesView data)
    {
        ...
        if (off + 32 > data.size())
        {
            return SMO_ERR_DISCOVERY(400, Info, RetrySafe, None, "truncated WelcomeMsg node_id");
        }
        std::memcpy(msg.node_id.value.data(), data.data() + off, 32);
        off += 32;
        uint32_t rec_len = read_u32(data, off);
        if (off + rec_len > data.size())
        {
            return SMO_ERR_DISCOVERY(400, Info, RetrySafe, None, "truncated WelcomeMsg peer_record");
        }
        auto rec = PeerRecord::deserialize(data.subspan(off, rec_len));
        if (!rec)
            return std::move(rec.error());
    ...
    Result<OfflineMsg> OfflineMsg::deserialize(BytesView data)
    {
        ...
        if (off + 32 > data.size())
        {
            return SMO_ERR_DISCOVERY(400, Info, RetrySafe, None, "truncated OfflineMsg node_id");
        }
        std::memcpy(msg.node_id.value.data(), data.data() + off, 32);
        off += 32;
    ...
    // Bootstrap
    // ===========================================================================
    Result<PeerRecord> Bootstrap::find_seed(const std::vector<Endpoint>& seeds, Transport& transport,
                                            const NodeID& local_id, int64_t now)
    {
        (void)local_id;
        (void)now;
        ...
        if (seeds.empty())
        {
            return SMO_ERR_DISCOVERY(406, Error, RetryBackoff, ManualIntervention, "no seed endpoints provided");
        }
        ...
        return SMO_ERR_DISCOVERY(406, Error, RetryBackoff, ManualIntervention, "no seed nodes responded");
    }
    ...
    Result<void> dispatch_discovery_datagram(BytesView data, DiscoveryEngine& engine, const Endpoint& from,
                                             int64_t now_ns)
    {
