...
namespace smo {
    ...
    Result<void> SeedStore::open(std::string_view base_path)
    {
        ...
        SMO_TRY(db_.exec("CREATE TABLE IF NOT EXISTS seeds ("
    ...
    Result<void> SeedStore::put(const SeedEntry& entry)
    {
        ...
        auto st = db_.prepare("INSERT OR REPLACE INTO seeds (endpoint, weight, mesh_id, added_at) "
                              "VALUES (?1, ?2, ?3, ?4)");
        if (!st)
            return st.error();
