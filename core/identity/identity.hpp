...
namespace smo {
    ...
    public:
        ...
        static Result<Identity> load(BytesView public_key, BytesView secret_key, CryptoSuiteID suite_id);

        // Load from file (JSON identity.json).
        static Result<Identity> load_from_file(const std::string& path, const CryptoProvider& crypto);
    ...
    private:
        Identity(NodeID node_id, CryptoSuiteID suite_id, Bytes public_key, Bytes secret_key,
                 IdentityState state) noexcept
            : node_id_(node_id), suite_id_(suite_id), public_key_(std::move(public_key)),
              secret_key_(std::move(secret_key)), state_(state)
        {
        }
