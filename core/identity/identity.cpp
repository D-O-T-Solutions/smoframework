...
namespace smo {
    ...
    Result<NodeID> node_id_from_public_key(BytesView pk, const HashImpl& hash)
    {
        if (!hash.hash)
        {
            return SMO_ERR_IDENTITY(100, Error, NoRetry, RetryOperation, "hash implementation is null");
        }
    ...
    bool is_valid_transition(IdentityState from, IdentityState to) noexcept
    {
        ...
        case IdentityState::Enrolled:
            return to == IdentityState::Active || to == IdentityState::Suspended || to == IdentityState::Retired;
        case IdentityState::Active:
            return to == IdentityState::Suspended || to == IdentityState::Retired ||
                   to == IdentityState::KeypairReady; // key rotation re-enters
        case IdentityState::Suspended:
            return to == IdentityState::Active || to == IdentityState::Retired;
    ...
    Result<Identity> Identity::create(const CryptoProvider& crypto, RngRef& rng)
    {
        ...
        {
            return SMO_ERR_IDENTITY(100, Error, NoRetry, RetryOperation, "crypto provider has no keygen");
        }
        if (!crypto.hash.hash)
        {
            return SMO_ERR_IDENTITY(100, Error, NoRetry, RetryOperation, "crypto provider has no hash");
        }
        ...
        return Identity(nid.value(), crypto.suite_id, std::move(pk), std::move(sk), IdentityState::KeypairReady);
    }

    Result<Identity> Identity::load(BytesView public_key, BytesView secret_key, CryptoSuiteID suite_id)
    {
        if (public_key.empty() || secret_key.empty())
        {
            return SMO_ERR_IDENTITY(100, Error, NoRetry, RetryOperation, "empty key material");
        }
        ...
        return Identity(id, suite_id, Bytes(public_key.begin(), public_key.end()),
                        Bytes(secret_key.begin(), secret_key.end()), IdentityState::KeypairReady);
    }
    ...
    Result<void> Identity::transition_to(IdentityState new_state) noexcept
    {
        ...
        {
            return SMO_ERR_IDENTITY(107, Error, NoRetry, RestartFSM, "invalid state transition");
        }
        state_ = new_state;
        return {};
    }
    ...
    Result<void> Identity::save_to_file(const std::string& path) const
    {
        ...
        if (!f)
        {
            return SMO_ERR_STORAGE(900, Error, NoRetry, None, "cannot open identity file for writing: " + path);
        }
        f << to_json();
        f.close();
        return {};
    ...
    Result<Identity> Identity::load_from_file(const std::string& path, const CryptoProvider& crypto)
    {
        std::ifstream f(path);
        if (!f)
        {
            return SMO_ERR_STORAGE(900, Error, NoRetry, None, "cannot open identity file: " + path);
        }
        ...
        {
            return SMO_ERR_STORAGE(901, Error, NoRetry, None, "identity file missing key material");
        }
        ...
        if (state_str == "Enrolled")
            state = IdentityState::Enrolled;
        else if (state_str == "Active")
            state = IdentityState::Active;
        else if (state_str == "CertificatePending")
            state = IdentityState::CertificatePending;

        Identity id(node_id, crypto.suite_id, std::move(public_key), std::move(secret_key), state);
        return id;
    }
