...
namespace smo {
    ...
    const char* to_string(GovernanceLevel l) noexcept
    {
        switch (l)
        {
        case GovernanceLevel::Local:
            return "Local";
        case GovernanceLevel::Authority:
            return "Authority";
        case GovernanceLevel::Policy:
            return "Policy";
        case GovernanceLevel::Critical:
            return "Critical";
        case GovernanceLevel::Genesis:
            return "Genesis";
        default:
            return "Unknown";
        }
    ...
    const char* to_string(GovernanceAction a) noexcept
    {
        ...
        case GovernanceAction::ChangeMaximum:
            return "ChangeMaximum";
        case GovernanceAction::ChangeMinimum:
            return "ChangeMinimum";
        ...
        case GovernanceAction::ChangeCipherSuite:
            return "ChangeCipherSuite";
        case GovernanceAction::ChangeGovernanceRules:
            return "ChangeGovernanceRules";
    ...
    const char* to_string(ProposalState s) noexcept
    {
        ...
        case ProposalState::Draft:
            return "Draft";
        case ProposalState::Signing:
            return "Signing";
        case ProposalState::Committed:
            return "Committed";
        case ProposalState::Rejected:
            return "Rejected";
        case ProposalState::Expired:
            return "Expired";
        case ProposalState::Conflicted:
            return "Conflicted";
        default:
            return "Unknown";
        }
    ...
    std::string MeshHealth::to_display() const
    {
        ...
        os << "Health:      " << level_str << "\n";
        os << "Authorities: " << online_authorities << "/" << total_authorities << " online (min=" << min_required
           << ", preferred=" << preferred << ", max=" << maximum << ")\n";
        os << "Offline:     " << offline_authorities << "\n";
        os << "Quorum:      " << current_quorum << "/" << required_quorum << " (need " << required_quorum
           << " to operate)\n";
    ...
    Result<GovernanceSignature> GovernanceSignature::deserialize(BytesView data)
    {
        ...
        if (off + 32 > data.size())
        {
            return SMO_ERR_GOVERNANCE(810, Error, NoRetry, None, "truncated authority_id in signature");
        }
        ...
        {
            return SMO_ERR_GOVERNANCE(810, Error, NoRetry, None, "truncated signature data");
        }
        gs.signature =
            Bytes(data.begin() + static_cast<ptrdiff_t>(off), data.begin() + static_cast<ptrdiff_t>(off + sig_len));
        off += sig_len;
    ...
    Result<GovernanceProposal> GovernanceProposal::deserialize(BytesView data)
    {
        ...
        if (off + 4 > data.size())
        {
            return SMO_ERR_GOVERNANCE(810, Error, NoRetry, None, "truncated proposal header");
        }
        ...
        {
            return SMO_ERR_GOVERNANCE(810, Error, NoRetry, None, "truncated proposal payload");
        }
        prop.payload =
            Bytes(data.begin() + static_cast<ptrdiff_t>(off), data.begin() + static_cast<ptrdiff_t>(off + payload_len));
        off += payload_len;
    ...
    {
        if (static_cast<int>(proposal.level) < 0 || proposal.level > GovernanceLevel::Genesis)
        {
            return SMO_ERR_GOVERNANCE(811, Error, NoRetry, None, "invalid governance level");
        }
        if (proposal.payload.empty())
        {
            return SMO_ERR_GOVERNANCE(810, Error, NoRetry, None, "proposal payload cannot be empty");
        }
        ...
        if (proposal.expires_at == 0)
        {
            ...
            int64_t ttl = (proposal.tier >= GovernanceTier::Constitution) ? 172800000000000LL : 86400000000000LL;
    ...
    Result<void> GovernanceEngine::sign(ProposalID id, NodeID authority, Bytes signature, int64_t now)
    {
        auto it = proposals_.find(id.value);
        if (it == proposals_.end())
        {
            return SMO_ERR_GOVERNANCE(800, Error, NoRetry, GovernanceVote, "proposal not found");
        }
        auto& prop = it->second;

        if (prop.state != ProposalState::Signing)
        {
            return SMO_ERR_GOVERNANCE(810, Error, NoRetry, None, "proposal is not in Signing state");
        }
        if (prop.is_expired(now))
        {
            prop.state = ProposalState::Expired;
            return SMO_ERR_GOVERNANCE(810, Error, NoRetry, None, "proposal has expired");
        }
    ...
    Result<void> GovernanceEngine::commit(ProposalID id, int64_t now)
    {
        ...
        {
            return SMO_ERR_GOVERNANCE(803, Warn, NoRetry, GovernanceVote, "signature threshold not met");
        }
