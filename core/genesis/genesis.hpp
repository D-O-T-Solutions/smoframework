...
namespace smo::genesis {
    ...
    public:
        ...
        Result<GenesisResult> run_stage_0(const std::string& mesh_id, const std::string& root_node_id,
        ...
        Result<Bytes> run_stage_1_claim_slot(GenesisResult& result, const std::string& role,
                                             const std::string& node_public_key, const std::string& join_token_id,
                                             uint64_t now_ns);

        // Finalize slot fulfillment after Root signs CSR
        Result<void> run_stage_1_fulfill(GenesisResult& result, uint32_t slot_index, const std::string& signed_csr,
                                         uint64_t now_ns);
