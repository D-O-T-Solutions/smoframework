...
namespace smo::genesis {
    ...
    Result<GenesisManifest> GenesisManifest::deserialize(BytesView data)
    {
        ...
        if (m.mesh_id.empty())
        {
            return SMO_ERR_GENESIS(1401, Error, NoRetry, ManualIntervention, "mesh_id is required");
        }
        if (m.root_public_key.empty())
        {
            return SMO_ERR_GENESIS(1401, Error, NoRetry, ManualIntervention, "root_public_key is required");
        }
