...
namespace smo::genesis {
    ...
    inline Result<DeploymentProfile> deployment_profile_from_string(const std::string& s)
    {
        ...
        if (s == "startup")
            return DeploymentProfile::Startup;
        if (s == "enterprise")
            return DeploymentProfile::Enterprise;
        if (s == "air-gapped")
            return DeploymentProfile::AirGapped;
        if (s == "custom")
            return DeploymentProfile::Custom;
        return SMO_ERR_GENESIS(100, Error, NoRetry, ManualIntervention, "unknown deployment profile: " + s);
    }
    ...
    struct AuthorityRange
    {
        ...
        bool valid() const { return minimum >= 1 && minimum <= preferred && preferred <= maximum; }
    };
