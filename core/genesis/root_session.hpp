...
namespace smo::genesis {
    ...
    struct SessionPolicy
    {
        ...
        {
            SessionPolicy p;
            p.caps = {Capability::SignJoinToken, Capability::SignBootstrapCSR,  Capability::SignRecovery,
                      Capability::IssueRecovery, Capability::RotateRoot,        Capability::EmergencyLockdown,
    ...
    public:
        ...
        RootSession(RootSession&& other) noexcept
            : session_id(std::move(other.session_id)), state_(other.state_),
              root_node_id(std::move(other.root_node_id)), root_public_key(std::move(other.root_public_key)),
              signer(std::move(other.signer)), policy(std::move(other.policy)), audit_sink(std::move(other.audit_sink)),
              created_at_(other.created_at_), expires_at_(other.expires_at_), consumed_at_(other.consumed_at_),
        ...
        {
            return state_ == RootSessionState::Active && (expires_at_ == 0 || now_ns <= expires_at_);
        }

        // ── Core API ────────────────────────────────────────────────────
        // 1. Activate — called by RootSessionManager or after unlock()
        //    If ctx is null the existing signer (from unlock) is kept.
        void activate(const std::string& sid, const std::string& node_id, const std::string& pubkey,
                      std::unique_ptr<crypto::SignerContext> ctx, SessionPolicy pol, AuditSink sink, uint64_t now_ns,
                      uint64_t ttl_ns);
    ...
    struct RootSessionManager
    {
        ...
        Result<std::string> start_session(const std::string& root_node_id, const std::string& root_public_key,
                                          std::unique_ptr<crypto::SignerContext> signer, SessionPolicy policy,
                                          AuditSink audit_sink, uint64_t now_ns);

        Result<void> validate_session(const std::string& session_id, uint64_t now_ns);
