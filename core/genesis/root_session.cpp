...
namespace smo::genesis {
    ...
    void RootSession::activate(const std::string& sid, const std::string& node_id, const std::string& pubkey,
                               std::unique_ptr<crypto::SignerContext> ctx, SessionPolicy pol, AuditSink sink,
                               uint64_t now_ns, uint64_t ttl_ns)
    {
        session_id = sid;
    ...
    Result<void> RootSession::consume(uint64_t now_ns)
    {
        ...
        {
            return SMO_ERR_GENESIS(1406, Error, NoRetry, ManualIntervention, "session not active");
        }
    ...
    Result<RootResult> RootSession::execute(const RootRequest& req, RngRef& rng, uint64_t now_ns)
    {
        if (state_ != RootSessionState::Active)
        {
            return SMO_ERR_GENESIS(1406, Error, NoRetry, ManualIntervention, "RootSession not active");
        }
        if (!is_valid(now_ns))
        {
            ...
            if (signer)
                signer->destroy();
            signer.reset();
            return SMO_ERR_GENESIS(1405, Warn, NoRetry, ManualIntervention, "RootSession expired");
        }
        if (!signer || !signer->valid())
        {
            return SMO_ERR_GENESIS(1406, Error, NoRetry, ManualIntervention, "RootSession signer context invalid");
        }
        ...
        auto sign_result = [&]() -> Result<Bytes> {
            ...
            case RootOperation::ConstitutionChange:
                return SMO_ERR_GENESIS(1400, Error, NoRetry, ManualIntervention, "RootOperation not yet implemented");
            }
            return SMO_ERR_GENESIS(1400, Error, NoRetry, ManualIntervention, "Unknown RootOperation");
        }();
        ...
        // ── 3. Audit success ────────────────────────────────────────────
        {
            ...
            ev.success = true;
            if (!req.mesh_id.empty())
                ev.details = "mesh=" + req.mesh_id;
    ...
    Result<std::string> RootSessionManager::start_session(const std::string& root_node_id,
                                                          const std::string& root_public_key,
                                                          std::unique_ptr<crypto::SignerContext> signer,
                                                          SessionPolicy policy, AuditSink audit_sink, uint64_t now_ns)
    {
        ...
        session.activate(sid, root_node_id, root_public_key, std::move(signer), std::move(policy),
                         std::move(audit_sink), now_ns, ttl_ns);

        return session.session_id;
    }

    Result<void> RootSessionManager::validate_session(const std::string& session_id, uint64_t now_ns)
    {
        if (session.session_id != session_id)
        {
            return SMO_ERR_GENESIS(1406, Error, NoRetry, ManualIntervention, "session ID mismatch");
        }
        ...
        if (!session.is_valid(now_ns))
        {
            ...
            return SMO_ERR_GENESIS(1405, Warn, NoRetry, ManualIntervention, "root session expired");
        }
        return {};
    }
