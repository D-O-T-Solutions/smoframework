...
namespace smo::genesis {
    ...
    // ── SlotRing mutators ────────────────────────────────────────────────
    Result<uint32_t> SlotRing::claim_slot(const std::string& role, const std::string& node_public_key,
                                          const std::string& join_token_id, uint64_t now_ns)
    {
        for (size_t i = 0; i < slots.size(); ++i)
        {
            auto& s = slots[i];
            if (s.status == SlotStatus::Vacant || (s.status == SlotStatus::Claimed && s.is_expired(now_ns)))
            {
                s.index = (uint32_t)i;
        ...
        return SMO_ERR_GENESIS(1402, Error, NoRetry, ManualIntervention, "all bootstrap slots exhausted");
    }
    ...
    Result<void> SlotRing::fulfill_slot(uint32_t index, const std::string& csr, uint64_t now_ns)
    {
        if (index >= slots.size())
        {
            return SMO_ERR_GENESIS(1401, Error, NoRetry, ManualIntervention, "slot index out of range");
        }
        auto& s = slots[index];
        if (s.status != SlotStatus::Claimed)
        {
            return SMO_ERR_GENESIS(1401, Error, NoRetry, ManualIntervention, "slot not in claimed state");
        }
    ...
    Result<void> SlotRing::revoke_slot(uint32_t index)
    {
        ...
        {
            return SMO_ERR_GENESIS(1401, Error, NoRetry, ManualIntervention, "slot not in claimable state");
        }
    ...
    uint32_t SlotRing::fulfilled_count() const
    {
        ...
        for (const auto& s : slots)
        {
            if (s.status == SlotStatus::Fulfilled)
                ++cnt;
        }
        return cnt;
    }
