...
namespace smo {
    ...
    namespace {
        ...
        Bytes read_bytes(BytesView data, size_t& off)
        {
            ...
            if (off + len > data.size())
                return {};
            Bytes b(data.begin() + static_cast<ptrdiff_t>(off), data.begin() + static_cast<ptrdiff_t>(off + len));
            off += len;
            return b;
        }
    } // anonymous namespace
    ...
    double compute_composite(const TrustComponents& c) noexcept
    {
        ...
        double s = (c.citizen * w_citizen) + (c.execution * w_execution) + (c.witness * w_witness) +
    ...
    void TrustManager::record_success(NodeID node, double weight, int64_t now)
    {
        ...
        if (ts.components.execution > 1.0)
            ts.components.execution = 1.0;
        if (ts.components.execution < 0.0)
            ts.components.execution = 0.0;
    ...
    void TrustManager::record_offline(NodeID node, int64_t now)
    {
        ...
        if (ts.components.citizen < 0.0)
            ts.components.citizen = 0.0;
        ts.composite = compute_composite(ts.components);
        ts.last_updated = now;
    }
    ...
    {
        auto it =
            std::remove_if(anchors_.begin(), anchors_.end(), [&](const TrustAnchor& a) { return a.node_id == node; });
        if (it == anchors_.end())
            return false;
