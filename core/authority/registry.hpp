...
namespace smo::authority {
    ...
    // ---------------------------------------------------------------------------
    namespace AliasErrc {
        inline constexpr ErrorCode DisplayNameAlreadyExists(ErrorCategory::Certificate, 220, Severity::Warn,
                                                            RetryClass::RetrySafe, Recovery::None);
        inline constexpr ErrorCode InvalidDisplayName(ErrorCategory::Certificate, 221, Severity::Warn,
                                                      RetryClass::NoRetry, Recovery::None);
        inline constexpr ErrorCode DisplayNameTooLong(ErrorCategory::Certificate, 222, Severity::Warn,
                                                      RetryClass::NoRetry, Recovery::None);
    } // namespace AliasErrc
    ...
    public:
        ...
        Result<EnrollResult> enroll_node(const std::string& node_id_hex,
                                         const std::string& display_name, // will be normalized internally
                                         const std::string& mesh_id, const std::string& role,
                                         const std::string& cert_fingerprint, const std::string& cert_issuer_pubkey_hex,
