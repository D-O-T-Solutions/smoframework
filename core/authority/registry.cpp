...
namespace smo::authority {
    ...
    Result<void> validate_display_name(const std::string& name) noexcept
    {
        if (name.empty())
        {
            return SMO_ERR_CERT(221, Warn, NoRetry, None, "display name cannot be empty");
        }
        if (name.size() > 128)
        {
            return SMO_ERR_CERT(222, Warn, NoRetry, None, "display name too long (max 128 chars)");
        }
        // Must start with alphanumeric
        if (!std::isalnum(static_cast<unsigned char>(name[0])))
        {
            return SMO_ERR_CERT(221, Warn, NoRetry, None, "display name must start with alphanumeric character");
        }
    ...
    template <typename T = void> static Result<T> map_sqlite_error(int rc, sqlite3* db, const char* context)
    {
        ...
        if (rc == SQLITE_CONSTRAINT)
        {
            ...
            return SMO_ERR_CERT(220, Warn, RetrySafe, None, std::string("constraint violation (") + context + ")");
        }

        return SMO_ERR_STORAGE(904, Error, RetrySafe, RetryOperation, std::string(context) + ": " + sqlite3_errmsg(db));
    }
    ...
    Result<void> NodeRegistry::open(const std::string& db_path)
    {
        ...
        char* err = nullptr;
        sqlite3_exec(db_, "PRAGMA journal_mode = WAL; PRAGMA foreign_keys = ON;", nullptr, nullptr, &err);
        if (err)
        {
            sqlite3_free(err);
        }
    ...
    Result<EnrollResult> NodeRegistry::enroll_node(const std::string& node_id_hex, const std::string& display_name,
                                                   const std::string& mesh_id, const std::string& role,
                                                   const std::string& cert_fingerprint,
                                                   const std::string& cert_issuer_pubkey_hex,
                                                   const std::string& cert_subject_pubkey_hex, uint64_t epoch,
                                                   int64_t cert_issued_at, int64_t cert_expires_at)
    {
        if (!db_)
        {
        ...
        // 3. Insert alias (so display_name is also claimed as an alias)
        {
            ...
            if (rc != SQLITE_DONE)
            {
                ...
                if (rc == SQLITE_CONSTRAINT)
                {
                    return SMO_ERR_CERT(220, Warn, RetrySafe, None, "display name already taken: " + normalized);
                }
    ...
    Result<std::vector<NodeRecord>> NodeRegistry::list_nodes(const std::string& mesh_id) const
    {
        ...
        const char* sql =
            mesh_id.empty() ? "SELECT node_id_hex, display_name, mesh_id, role, "
                            ...
                            : "SELECT node_id_hex, display_name, mesh_id, role, "
                              "cert_fingerprint, status, epoch, enrolled_at, last_seen FROM nodes WHERE mesh_id = ? "
    ...
    Result<void> NodeRegistry::register_alias(const std::string& alias, const std::string& node_id_hex)
    {
        ...
        if (rc == SQLITE_CONSTRAINT)
        {
            return SMO_ERR_CERT(220, Warn, RetrySafe, None, "alias already exists: " + normalized);
        }
    ...
    Result<std::vector<AliasRecord>> NodeRegistry::list_aliases_for_node(const std::string& node_id_hex) const
    {
        ...
        const char* sql =
            "SELECT alias, node_id_hex, created_at FROM node_aliases WHERE node_id_hex = ? ORDER BY alias";
        sqlite3_stmt* stmt = nullptr;
        if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK)
        {
    ...
    Result<std::vector<EnrollmentRecord>> NodeRegistry::list_pending_enrollments() const
    {
        ...
        if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK)
        {
            return map_sqlite_error<std::vector<EnrollmentRecord>>(SQLITE_ERROR, db_,
                                                                   "prepare list_pending_enrollments");
        }
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
