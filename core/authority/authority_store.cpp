...
namespace smo::authority {
    ...
    Result<std::vector<AuthorityInfo>> AuthorityStore::list_by_role(const std::string& role,
    ...
    Result<std::vector<AuthorityInfo>> AuthorityStore::list_authorities(const std::string& mesh_id) const
    {
        return list_by_role("Authority", mesh_id);
    }

    Result<std::vector<AuthorityInfo>> AuthorityStore::list_roots(const std::string& mesh_id) const
    {
        return list_by_role("Root", mesh_id);
    }

    Result<bool> AuthorityStore::is_trusted(const std::string& node_id_hex, const std::string& mesh_id) const
    {
    ...
    Result<size_t> AuthorityStore::count(const std::string& mesh_id) const
    {
        ...
        for (auto& n : nodes.value())
        {
            if (n.role == "Authority" || n.role == "Root")
                ++c;
        }
        return c;
    }
