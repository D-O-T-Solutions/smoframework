...
namespace smo::authority {
    ...
    namespace {
        ...
        static bool parse_http_request(const std::string& raw, HttpRequest& req)
        {
            ...
            if (cl_pos != std::string::npos)
            {
                ...
                while (cl_pos < headers_section.size() && headers_section[cl_pos] == ' ')
                    ++cl_pos;
                size_t cl_end = cl_pos;
                while (cl_end < headers_section.size() && headers_section[cl_end] >= '0' &&
                       headers_section[cl_end] <= '9')
                    ++cl_end;
        ...
        static std::string json_response(const std::string& status, int code, const std::string& message,
    ...
    public:
                           ...
                           const HashImpl& hash)
        {
            ...
            if (server_fd_ < 0)
            {
                return SMO_ERR_STORAGE(900, Error, RetrySafe, None, "Failed to create enroll server socket");
            }
            ...
            if (::listen(server_fd_, 5) < 0)
            {
                ::close(server_fd_);
                server_fd_ = -1;
                return SMO_ERR_STORAGE(900, Error, RetrySafe, None, "Failed to listen on enroll server socket");
            }
    ...
    private:
        ...
        void handle_enroll(const HttpRequest& req, HttpResponse& resp)
        {
            ...
            auto node_id_result = node_id_from_public_key(cert_result.value().subject_pubkey, *hash_);
    ...
    Result<void> EnrollServer::start(uint16_t port, MeshAuthority& authority, const std::string& hmac_secret_hex,
                                     const HashImpl& hash)
    {
    ...
    bool EnrollServer::is_running() const
    {
