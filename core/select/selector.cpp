    ...
    namespace select {
        ...
        namespace {
            ...
            public:
                ...
                Token peek()
                {
                    if (lookahead_.type != TokenType::Eof)
                        return lookahead_;
                    lookahead_ = advance();
                    return lookahead_;
                }
            ...
            public:
                ...
                std::string get_field(const std::string& name) const
                {
                    ...
                    if (name == "arch")
                        return rec.arch;
                    if (name == "os" || name == "platform")
                        return rec.platform;
                    if (name == "version")
                        return rec.version;
                    if (name == "mesh")
                        return rec.mesh_name;
                    if (name == "name")
                        return rec.display_name;
                    if (name == "state")
                        return to_string(rec.state);
                    if (name == "location")
                        return rec.location;
                    if (name == "hostname")
                        return rec.hostname;
            ...
            public:
                ...
                bool parse(const EvalContext& ctx) { return or_expr(ctx); }
                ...
                bool primary(const EvalContext& ctx)
                {
                    ...
                    if (tok.type == TokenType::Ident)
                    {
                        ...
                        if (op.type != TokenType::Eq && op.type != TokenType::Neq && op.type != TokenType::Gt &&
                            op.type != TokenType::Lt && op.type != TokenType::Ge && op.type != TokenType::Le)
                        {
                            return false;
                        }
                        ...
                        if (field.value == "trust")
                        {
                            ...
                            if (op.type == TokenType::Lt)
                                return fnum < vnum;
                            if (op.type == TokenType::Ge)
                                return fnum >= vnum;
                            if (op.type == TokenType::Le)
                                return fnum <= vnum;
                        }

                        // String comparisons for other fields
                        if (op.type == TokenType::Gt)
                            return fval > val.value;
                        if (op.type == TokenType::Lt)
                            return fval < val.value;
                        if (op.type == TokenType::Ge)
                            return fval >= val.value;
            ...
            bool evaluate_expression(const PeerRecord& rec, double trust, const std::string& expr)
            {
                Lexer lex(expr);
        ...
        Result<NodeSet> Selector::select(const SelectQuery& query) const
        {
            ...
            if (!query.node_id && !query.name && !query.role && query.tags.empty() && !query.cap_mask &&
                !query.trust_min && !query.trust_max && !query.os && !query.arch && !query.version_constraint &&
                !query.mesh_name && !query.where_expr && !query.state)
            {
                return SMO_ERR(Discovery, 450, Info, NoRetry, None, "empty selection query — no filter specified");
            }

            auto peers = table_.peers();
            if (peers.empty())
            {
                return SMO_ERR(Discovery, 451, Info, RetrySafe, None, "no nodes in membership table");
            }
            ...
            for (const auto& rec : peers)
            {
                ...
                if (!matches_tags(rec, query.tags))
                    continue;

                // OS match
                if (query.os && rec.platform != query.os.value())
                    continue;

                // Arch match
                if (query.arch && rec.arch != query.arch.value())
                    continue;

                // Version constraint
                if (query.version_constraint && !matches_version(rec.version, query.version_constraint.value()))
                    continue;

                // Mesh name match
                if (query.mesh_name && rec.mesh_name != query.mesh_name.value())
                    continue;

                // State match
                if (query.state && rec.state != query.state.value())
                    continue;
            ...
            if (candidates.empty())
            {
                return SMO_ERR(Discovery, 451, Info, RetrySafe, None, "no nodes match selection criteria");
            }

            // Sort by composite score descending
            std::sort(candidates.begin(), candidates.end(),
                      [](const ScoredPeer& a, const ScoredPeer& b) { return a.score > b.score; });
