...
namespace smo::mesh {
    ...
    std::vector<StateTimeout> MeshFsm::default_timeouts()
    {
        return {
            ...
            {(int64_t)MeshState::Bootstrap, uint64_t(30) * 60 * 1'000'000'000ULL,
             (int64_t)MeshState::Recovery},                              // 30 min
            {(int64_t)MeshState::Online, 0, (int64_t)MeshState::Online}, // unlimited
            {(int64_t)MeshState::Maintenance, uint64_t(24) * 60 * 60 * 1'000'000'000ULL,
             (int64_t)MeshState::Recovery}, // 24h
            {(int64_t)MeshState::Recovery, uint64_t(72) * 60 * 60 * 1'000'000'000ULL,
             (int64_t)MeshState::Archived}, // 72h
        };
    }
