...
namespace smo {
    ...
    static std::string generate_mesh_id(const MeshConfig& config)
    {
        std::string canonical =
            config.root_pubkey + "|" + std::to_string(config.created_at) + "|" + std::to_string(std::rand());
        ...
        std::shared_ptr<MeshContext> load_mesh_context(const std::string& mesh_id) const
        {
            ...
            if (sqlite3_step(stmt) == SQLITE_ROW)
            {
                ...
                if (json_cstr)
                {
                    ...
                    ctx->config.cipher_suite_id =
                        static_cast<CryptoSuiteID>(json_read_int(config_json, "cipher_suite_id", kSuitePurePQC));
                    ...
                    ctx->config.bootstrap_configured = json_read_string(config_json, "bootstrap_configured") == "true";
    ...
    MeshManager::MeshManager(const Config& config) : impl_(std::make_unique<Impl>(config)) {}

    MeshManager::~MeshManager() = default;

    Result<void> MeshManager::initialize()
    {
        return impl_->open();
    }
    ...
    Result<void> MeshManager::join_mesh(const std::string& mesh_id, const std::string& seed_address)
    {
        ...
        {
            return SMO_ERR_STORAGE(905, Error, NoRetry, None, "JoinService not available");
        }
    ...
    namespace {
        ...
        int64_t parse_duration(const std::string& dur)
        {
            ...
            try
            {
                num = std::stoll(dur.substr(0, dur.size() - 1));
            }
            catch (...)
            {
                return 3600;
            ...
            case 'd':
                return num * 86400;
            default:
                return num * 3600; // assume hours if no unit
            }
        }

    } // anonymous namespace

    Result<std::string> MeshManager::generate_invite(const std::string& mesh_id, const std::string& role,
                                                     ...
                                                     const std::vector<std::string>& bootstrap_endpoints)
    {
        ...
        auto token_result =
            enroll::generate_token_hmac(config.mesh_id, config.epoch, static_cast<int>(config.cipher_suite_id),
    ...
    // ---------------------------------------------------------------------------
    Result<void> MeshManager::publish_mesh(const std::string& mesh_id, const std::string& listen_address,
                                           ...
                                           const std::vector<std::string>& bootstrap_endpoints)
    {
        ...
        if (!mf)
        {
            return SMO_ERR_STORAGE(905, Error, NoRetry, None, "Failed to write mesh.json at " + paths.mesh_json);
        }
    ...
    Result<MeshConfig> MeshManager::load_mesh_config(const std::string& mesh_dir)
    {
        ...
        if (!f)
        {
            return SMO_ERR_STORAGE(404, Info, NoRetry, None, "mesh.json not found: " + path);
        }
