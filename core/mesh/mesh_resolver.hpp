...
namespace smo {
    namespace mesh {
        ...
        inline Result<std::string> read_current_mesh()
        {
            ...
            if (!f)
            {
                return SMO_ERR_STORAGE(404, Info, NoRetry, None, "context.json not found: " + path);
            }
        ...
        inline Result<void> write_current_mesh(const std::string& name)
        {
            ...
            if (!f)
            {
                return SMO_ERR_STORAGE(500, Error, NoRetry, None, "cannot write context.json: " + path);
            }
            f << "{\n";
            f << "  \"current_mesh\": \"" << name << "\"\n";
            f << "}\n";
        ...
        inline Result<MeshResolution> resolve_mesh(const std::string& explicit_mesh_dir,
        ...
        // ---------------------------------------------------------------------------
        inline Result<MeshResolution> require_mesh(const std::string& explicit_mesh_dir,
                                                   const std::string& explicit_mesh_name)
        {
