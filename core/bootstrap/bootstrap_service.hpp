...
namespace smo::bootstrap {
    ...
    {
    public:
        BootstrapService(MeshManager& mesh_mgr, authority::MeshAuthority& authority, recovery::CRL* crl = nullptr)
            : mesh_mgr_(mesh_mgr), authority_(authority), crl_(crl)
        {
        }

        // Process a BootstrapSyncRequest — builds delta sync response
        Result<join::BootstrapSyncResponse> handle_sync(const join::BootstrapSyncRequest& req);
