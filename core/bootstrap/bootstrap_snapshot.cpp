...
namespace smo::bootstrap {
    ...
    Bytes BootstrapSnapshot::encode_cbor() const
    {
        ...
        {
            enc.encode_map(4);
            enc.encode_uint(K_SNAP_SEED_ENDPOINT);
            enc.encode_string(s.endpoint);
            if (!s.region.empty())
            {
                enc.encode_uint(K_SNAP_SEED_REGION);
                enc.encode_string(s.region);
            }
            enc.encode_uint(K_SNAP_SEED_WEIGHT);
            enc.encode_uint(s.weight);
    ...
    Result<BootstrapSnapshot> BootstrapSnapshot::decode_cbor(BytesView data)
    {
            ...
            case K_SEEDS: {
                ...
                for (size_t j = 0; j < arr_sz.value(); ++j)
                {
                    ...
                    for (size_t k = 0; k < map_sz.value(); ++k)
                    {
                        ...
                        switch (sk.value())
                        {
                        case K_SNAP_SEED_ENDPOINT: {
                            auto v = dec.decode_string();
                            if (v)
                                si.endpoint = std::move(v.value());
                            break;
                        }
                        case K_SNAP_SEED_REGION: {
                            auto v = dec.decode_string();
                            if (v)
                                si.region = std::move(v.value());
                            break;
                        }
                        case K_SNAP_SEED_WEIGHT: {
                            auto v = dec.decode_uint();
                            if (v)
                                si.weight = static_cast<uint32_t>(v.value());
                            break;
                        }
                        case K_SNAP_SEED_HEALTH: {
                            auto v = dec.decode_uint();
                            if (v)
                            {
                                uint64_t f = v.value();
        ...
        {
            return SMO_ERR_PROTOCOL(900, Error, NoRetry, None, "BootstrapSnapshot CBOR missing mesh_id");
        }
    ...
    std::string BootstrapSnapshot::to_json() const
    {
        ...
        for (size_t i = 0; i < authorities.size(); ++i)
        {
            if (i > 0)
                oss << ",";
            oss << "{\"node_id\":\"" << json_esc(authorities[i].node_id) << "\",\"endpoint\":\""
                << json_esc(authorities[i].endpoint) << "\"}";
        }
        oss << "],";
