...
namespace smo {
    namespace enroll {
        ...
        static Result<FsmInstance> load_join_state(const std::string& data_dir)
        {
            ...
            return FsmInstance::deserialize(BytesView(data), rules.data(), rules.size(), timeouts.data(),
                                            timeouts.size());
        }
        ...
        static Result<Bytes> tcp_cbor_exchange(int fd, BytesView send_cbor)
        {
            ...
            while (total < sizeof(header))
            {
                ...
                if (n <= 0)
                    return SMO_ERR_TRANSPORT(400, Error, RetrySafe, None, "Failed to send CBOR header");
            ...
            while (total < send_cbor.size())
            {
                ...
                if (n <= 0)
                    return SMO_ERR_TRANSPORT(400, Error, RetrySafe, None, "Failed to send CBOR payload");
                total += static_cast<size_t>(n);
            }
            ...
            while (total < sizeof(resp_header))
            {
                ...
                if (n <= 0)
                    return SMO_ERR_TRANSPORT(400, Error, RetrySafe, None, "Failed to receive response header");
                total += static_cast<size_t>(n);
            }
            uint32_t resp_len = (static_cast<uint32_t>(resp_header[0]) << 24) |
                                (static_cast<uint32_t>(resp_header[1]) << 16) |
                                (static_cast<uint32_t>(resp_header[2]) << 8) | static_cast<uint32_t>(resp_header[3]);
            ...
            while (total < resp_len)
            {
                ...
                if (n <= 0)
                    return SMO_ERR_TRANSPORT(400, Error, RetrySafe, None, "Failed to receive response payload");
                total += static_cast<size_t>(n);
            }
            ::close(fd);
            return resp;
        ...
        static Result<void> ensure_dir(const std::string& path)
        {
            ...
            if (ec)
            {
                return SMO_ERR_CERT(200, Error, RetrySafe, None, "Failed to create directory: " + ec.message());
            }
            return {};
        }
                                            ...
                                            const std::string& node_name, uint16_t port, const std::string& mesh_dir)
        {
            ...
            if (current_state() <= join::JoinState::CERT_RECEIVED)
            {
                ...
                if (!cert.is_valid_at(now_sec))
                {
                    ...
                    save_join_state(fsm, actual_data_dir);
                    return SMO_ERR_CERT(216, Error, NoRetry, None, "Certificate not valid at current time");
                }
                ...
                if (!verify_cert || !verify_cert.value())
                {
                    (void)fsm.on_event(static_cast<int64_t>(join::JoinEvent::CERT_INVALID));
                    save_join_state(fsm, actual_data_dir);
                    return SMO_ERR_CERT(216, Error, NoRetry, None, "Certificate signature invalid");
                }
                ...
                // 4. Save authority public key for manifest verification
                {
                    ...
                    if (f)
                    {
                        f.write(reinterpret_cast<const char*>(cert.issuer_pubkey.data()), cert.issuer_pubkey.size());
                    }
                }
            ...
            // ── Step: Bootstrap sync ───────────────────────────────────────
            if (current_state() >= join::JoinState::CERT_VERIFY && current_state() <= join::JoinState::BOOTSTRAP_SYNC)
            {
                ...
                for (const auto& endpoint : endpoints)
                {
                    ...
                    auto decode_result = join::BootstrapSyncResponse::decode_cbor(BytesView(enc_resp.value()));
                    if (!decode_result)
                    {
                        std::printf("FAIL (invalid response: %s)\n", decode_result.error().message.c_str());
                        last_error = "Invalid CBOR response";
                    ...
                    std::printf("OK (mf=%llu mem=%llu crl=%llu pol=%llu seeds=%zu)\n",
                                ...
                                (unsigned long long)sync_resp.policy_revision, sync_resp.seeds.size());
                    ...
                    if (!sync_resp.manifest_delta.empty())
                    {
                        ...
                        else
                        {
                            ...
                            if (enroll::cbor::decode_map(BytesView(sync_resp.manifest_delta), off) >= 3)
                            {
                                ...
                                for (size_t i = 0; i < 3; ++i)
                                {
                                    ...
                                    {
                                        manifest_data =
                                            enroll::cbor::decode_bytes(BytesView(sync_resp.manifest_delta), off);
                                    }
                                    else if (key == 2)
                                    {
                                        manifest_sig =
                                    ...
                                    {
                                        manifest_epoch =
                                            enroll::cbor::decode_uint(BytesView(sync_resp.manifest_delta), off);
                                    }
                                }
                        ...
                        if (mf_f)
                        {
                            mf_f.write(reinterpret_cast<const char*>(sync_resp.manifest_delta.data()),
                    ...
                    if (!sync_resp.membership_delta.empty())
                    {
                        ...
                        if (mem_f)
                        {
                            mem_f.write(reinterpret_cast<const char*>(sync_resp.membership_delta.data()),
                    ...
                    if (!sync_resp.crl_delta.empty())
                    {
                        ...
                        if (crl_f)
                        {
                            crl_f.write(reinterpret_cast<const char*>(sync_resp.crl_delta.data()),
                    ...
                    if (!sync_resp.policy_delta.empty())
                    {
                        ...
                        if (pol_f)
                        {
                            pol_f.write(reinterpret_cast<const char*>(sync_resp.policy_delta.data()),
            ...
            if (current_state() == join::JoinState::READY)
            {
                ...
                std::string config_path = actual_data_dir + "/config.json";
                {
                    ...
                    if (f)
                    {
                        ...
                        f << "  \"profile\": \""
                          << (token.admission.profile.empty() ? "server" : token.admission.profile) << "\",\n";
                        f << "  \"listen_port\": " << port << ",\n";
