...
namespace smo::enroll {
    ...
    namespace cbor {
        ...
        static uint64_t read_int(BytesView data, size_t& off, uint8_t ib)
        {
            ...
            if (extra <= 23)
                return extra;
            if (off + (1ULL << (extra - 24)) > data.size())
                return 0;
            uint64_t val = 0;
    ...
    static Admission deserialize_admission(BytesView data, size_t& off)
    {
        ...
        for (size_t i = 0; i < pairs && off < data.size(); ++i)
        {
            ...
            {
            case 1:
                a.role = decode_text(data, off);
                break;
            case 2:
                a.profile = decode_text(data, off);
                break;
            case 3:
                a.slot = static_cast<int>(decode_uint(data, off));
                break;
            default: {
    ...
    // =========================================================================
    Result<JoinToken> generate_token(const std::string& mesh_id, int64_t mesh_epoch, int cipher_suite_id,
                                     const std::vector<std::string>& bootstrap_endpoints, const Admission& admission,
                                     int64_t expiry_unix_sec, const std::string& issuer, const SignerImpl& signer,
                                     BytesView issuer_secret_key, RngRef& rng)
    {
        JoinToken token;
        token.version = 1;
        ...
        if (!sig_result)
        {
            return SMO_ERR_CERT(212, Error, NoRetry, RetryOperation, "Join Token signing failed");
        }
    ...
    // =========================================================================
    Result<JoinToken> generate_token_hmac(const std::string& mesh_id, int64_t mesh_epoch, int cipher_suite_id,
                                          const std::vector<std::string>& bootstrap_endpoints, const std::string& role,
                                          int64_t expiry_unix_sec, const Bytes& hmac_secret, const HashImpl& hash)
    {
        JoinToken token;
        token.version = 1;
        token.mesh_id = mesh_id;
        ...
        if (!hmac_result)
        {
            return SMO_ERR_CERT(212, Error, NoRetry, RetryOperation, "HMAC computation failed");
        }
    ...
    // =========================================================================
    Result<void> validate_token(const JoinToken& token, const SignerImpl& signer, BytesView issuer_public_key,
                                const HashImpl& hash)
    {
        if (token.expiry_unix_sec > 0)
        {
            ...
            {
                return SMO_ERR_CERT(213, Warn, NoRetry, ManualIntervention, "Join Token has expired");
            }
        }

        if (token.issuer.empty())
        {
            return SMO_ERR_CERT(212, Error, NoRetry, ManualIntervention, "Join Token v2 requires issuer field");
        }
        if (token.signature.empty())
        {
            return SMO_ERR_CERT(212, Error, NoRetry, ManualIntervention, "Join Token has no signature");
        }
        ...
        if (!verify_result)
        {
            return SMO_ERR_CERT(212, Error, NoRetry, ManualIntervention, "Signature verification failed");
        }
        if (!verify_result.value())
        {
            return SMO_ERR_CERT(212, Error, NoRetry, ManualIntervention, "Join Token signature mismatch");
        }
    ...
    Result<void> validate_token_v1(const JoinToken& token, const Bytes& hmac_secret, const HashImpl& hash)
    {
        ...
        {
            return SMO_ERR_CERT(212, Error, NoRetry, ManualIntervention, "Join Token HMAC mismatch");
        }
