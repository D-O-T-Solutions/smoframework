...
namespace smo {
    ...
    static bool read_u32(BytesView data, size_t& offset, uint32_t& out)
    {
        if (offset + 4 > data.size())
            return false;
        out = (static_cast<uint32_t>(data[offset]) << 24) | (static_cast<uint32_t>(data[offset + 1]) << 16) |
    ...
    {
        uint32_t hi, lo;
        if (!read_u32(data, offset, hi))
            return false;
        if (!read_u32(data, offset, lo))
            return false;
    ...
    static bool read_bytes(BytesView data, size_t& offset, Bytes& out)
    {
        ...
        if (offset + len > data.size())
            return false;
        out.assign(data.begin() + static_cast<ptrdiff_t>(offset), data.begin() + static_cast<ptrdiff_t>(offset + len));
        offset += len;
        return true;
    }
    ...
    Result<Certificate> Certificate::deserialize(BytesView data)
    {
        ...
        if (!read_bytes(data, offset, cert.mesh_id))
            return SMO_ERR_CERT(203, Error, NoRetry, Reenroll, "failed to parse mesh_id");
        if (!read_bytes(data, offset, cert.subject_pubkey))
            return SMO_ERR_CERT(203, Error, NoRetry, Reenroll, "failed to parse subject_pubkey");
        if (!read_bytes(data, offset, cert.issuer_pubkey))
            return SMO_ERR_CERT(203, Error, NoRetry, Reenroll, "failed to parse issuer_pubkey");

        if (offset >= data.size())
            return SMO_ERR_CERT(203, Error, NoRetry, Reenroll, "truncated: missing role");
        cert.role = static_cast<Role>(data[offset]);
        offset += 1;

        Bytes dn;
        if (!read_bytes(data, offset, dn))
            return SMO_ERR_CERT(203, Error, NoRetry, Reenroll, "failed to parse display_name");
        cert.display_name = std::string(dn.begin(), dn.end());

        if (!read_bytes(data, offset, cert.capabilities))
            return SMO_ERR_CERT(203, Error, NoRetry, Reenroll, "failed to parse capabilities");
        if (!read_u64(data, offset, cert.epoch))
            return SMO_ERR_CERT(203, Error, NoRetry, Reenroll, "failed to parse epoch");

        uint64_t nb, na;
        if (!read_u64(data, offset, nb))
            return SMO_ERR_CERT(203, Error, NoRetry, Reenroll, "failed to parse not_before");
        if (!read_u64(data, offset, na))
            return SMO_ERR_CERT(203, Error, NoRetry, Reenroll, "failed to parse not_after");
        ...
        if (offset < data.size())
        {
            if (!read_bytes(data, offset, cert.signature))
                return SMO_ERR_CERT(203, Error, NoRetry, Reenroll, "failed to parse signature");
        }
    ...
    Result<bool> Certificate::verify(const SignerImpl& signer) const
    {
        if (signature.empty())
        {
            return SMO_ERR_CERT(204, Alert, NoRetry, None, "certificate has no signature");
        }
        auto body = serialize();
    ...
    Result<void> CertificateChain::verify(const CryptoProvider& crypto, BytesView root_pubkey) const
    {
        if (certs.empty())
        {
    ...
    Result<CertificateSigningRequest> CertificateSigningRequest::deserialize(BytesView data)
    {
        CertificateSigningRequest csr;
        size_t offset = 0;

        if (!read_bytes(data, offset, csr.new_public_key))
            return SMO_ERR_CERT(209, Alert, NoRetry, None, "failed to parse CSR new_public_key");
        if (!read_bytes(data, offset, csr.mesh_id))
            return SMO_ERR_CERT(209, Alert, NoRetry, None, "failed to parse CSR mesh_id");
        if (!read_bytes(data, offset, csr.old_cert_hash))
            return SMO_ERR_CERT(209, Alert, NoRetry, None, "failed to parse CSR old_cert_hash");

        Bytes dn;
        if (!read_bytes(data, offset, dn))
            return SMO_ERR_CERT(209, Alert, NoRetry, None, "failed to parse CSR display_name");
        csr.display_name = std::string(dn.begin(), dn.end());

        Bytes plat;
        if (!read_bytes(data, offset, plat))
            return SMO_ERR_CERT(209, Alert, NoRetry, None, "failed to parse CSR platform");
        csr.platform = std::string(plat.begin(), plat.end());

        Bytes ver;
        if (!read_bytes(data, offset, ver))
            return SMO_ERR_CERT(209, Alert, NoRetry, None, "failed to parse CSR version");
        csr.version = std::string(ver.begin(), ver.end());

        uint64_t ts;
        if (!read_u64(data, offset, ts))
            return SMO_ERR_CERT(209, Alert, NoRetry, None, "failed to parse CSR timestamp");
        csr.timestamp = static_cast<int64_t>(ts);

        // Read signature
        if (!read_bytes(data, offset, csr.signature))
            return SMO_ERR_CERT(209, Alert, NoRetry, None, "failed to parse CSR signature");

        return csr;
    }

    Result<bool> CertificateSigningRequest::verify(const SignerImpl& signer, BytesView signer_pubkey) const
    {
        if (signature.empty())
        {
            return SMO_ERR_CERT(209, Alert, NoRetry, None, "CSR has no signature");
        }
        auto body = serialize_body();
        return signer.verify(body, signature, signer_pubkey);
    }

    Result<void> CertificateSigningRequest::sign(const SignerImpl& signer, BytesView secret_key, RngRef& rng)
    {
        auto body = serialize_body();
