...
void MerkleTree::rebuild() {
    ...
    for (auto& bucket : buckets) {
        if (!bucket.data.empty()) {
            ...
            std::memcpy(bucket.hash.data(), h.data(),
                        std::min(h.size(), size_t(32)));
        } else {
            bucket.hash.fill(0);
            bucket.epoch = 0;
        }
    ...
    std::memcpy(root_hash.data(), root.data(),
                std::min(root.size(), size_t(32)));
}

bool MerkleTree::operator==(const MerkleTree& o) const {
    return id == o.id && epoch == o.epoch
        && version_vector == o.version_vector
...
Bytes MerkleNode::serialize() const {
    ...
    out.insert(out.end(), reinterpret_cast<const uint8_t*>(&epoch),
               reinterpret_cast<const uint8_t*>(&epoch) + 8);
...
Result<MerkleNode> MerkleNode::deserialize(BytesView data) {
    ...
    MerkleNode n;
    std::memcpy(n.hash.data(), data.data() + pos, 32); pos += 32;
    std::memcpy(&n.epoch, data.data() + pos, 8); pos += 8;
    uint32_t dlen;
...
Bytes MerkleTree::serialize() const {
    ...
    out.insert(out.end(), reinterpret_cast<const uint8_t*>(&vv_len),
               reinterpret_cast<const uint8_t*>(&vv_len) + 4);
    ...
    out.insert(out.end(), reinterpret_cast<const uint8_t*>(&bcount),
               reinterpret_cast<const uint8_t*>(&bcount) + 4);
    for (const auto& b : buckets) {
...
Result<MerkleTree> MerkleTree::deserialize(BytesView data) {
    ...
    MerkleTree t;
    t.id = static_cast<TreeID>(data[pos]); pos += 1;
    std::memcpy(&t.epoch, data.data() + pos, 8); pos += 8;
    uint32_t vv_len;
    std::memcpy(&vv_len, data.data() + pos, 4); pos += 4;
    if (pos + vv_len > data.size()) {
        return Error(SyncErrc::Truncated, "MerkleTree: version_vector truncated");
    }
    auto vv_r = VersionVector::deserialize(
        BytesView(data.data() + pos, vv_len));
    if (!vv_r) return vv_r.error();
    t.version_vector = std::move(vv_r.value());
    pos += vv_len;
    if (pos + 32 + 4 > data.size()) {
        return Error(SyncErrc::Truncated, "MerkleTree: root_hash truncated");
    }
    std::memcpy(t.root_hash.data(), data.data() + pos, 32); pos += 32;
    uint32_t bcount;
    std::memcpy(&bcount, data.data() + pos, 4); pos += 4;
    for (uint32_t i = 0; i < bcount; ++i) {
        auto br = MerkleNode::deserialize(
            BytesView(data.data() + pos, data.size() - pos));
