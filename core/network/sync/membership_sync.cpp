...
namespace smo::network::sync {
    ...
    MembershipSync::MembershipSync(MembershipTable& table, HealthMonitor& health) : membership_(table), health_(health)
    {
    }
    ...
    void MembershipSync::unsubscribe(uint64_t id)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        subscribers_.erase(
            std::remove_if(subscribers_.begin(), subscribers_.end(), [id](const auto& p) { return p.first == id; }),
            subscribers_.end());
    }
    ...
    void MembershipSync::emit_peer_renamed(const NodeID& id, const std::string& old_name, const std::string& new_name)
    {
        MembershipEvent ev;
    ...
    void MembershipSync::emit_state_changed(const NodeID& id, PeerState old_state, PeerState new_state, int misses)
    {
        MembershipEvent ev;
    ...
    namespace {
        ...
        void serialize_event_payload(Bytes& payload, const MembershipEvent& ev)
        {
            ...
            case MembershipEventType::CapabilityChange:
                ...
                for (auto& s : ev.added_caps)
                    put_str(payload, s);
                for (auto& s : ev.removed_caps)
                    put_str(payload, s);
                break;
        ...
        for (uint32_t i = 0; i < count; ++i)
        {
            ...
            case MembershipEventType::CertificateRotate: {
                ...
                if (cert_len > 0 && cert_len <= payload.size())
                {
                    ...
                    if (cert)
                        ev.new_cert = std::move(cert.value());
                }
                break;
            }
            }
