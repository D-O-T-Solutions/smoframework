...
namespace smo::network::udp {
    ...
    UdpSession::UdpSession(int fd, Endpoint remote) : fd_(fd), remote_(std::move(remote)), open_(true) {}
    ...
    Result<std::unique_ptr<TransportSession>> UdpListener::accept()
    {
        ...
        ssize_t n = ::recvfrom(fd_, buf, sizeof(buf), MSG_PEEK, reinterpret_cast<struct sockaddr*>(&peer), &addrlen);
        if (n < 0)
        {
        ...
        if (connected_fd < 0)
        {
            return SMO_ERR_TRANSPORT(306, Error, NoRetry, RestartFSM, "failed to create connected UDP socket");
        }
        ...
        if (::connect(connected_fd, reinterpret_cast<struct sockaddr*>(&peer_addr), sizeof(peer_addr)) < 0)
        {
            ::close(connected_fd);
        ...
        return std::unique_ptr<TransportSession>(new UdpSession(connected_fd, remote_ep));
    }
