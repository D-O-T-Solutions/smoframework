...
namespace smo {
    ...
    static Result<void> write_all(int fd, const void* data, size_t len)
    {
        ...
        while (remaining > 0)
        {
            ...
            if (n < 0)
            {
                if (errno == EINTR)
                    continue;
                return SMO_ERR_TRANSPORT(304, Error, RetrySafe, Reconnect, "secure write failed");
            }
            ptr += n;
    ...
    static Result<void> read_all(int fd, void* data, size_t len)
    {
        ...
        while (remaining > 0)
        {
            ...
            if (n < 0)
            {
                if (errno == EINTR)
                    continue;
                return SMO_ERR_TRANSPORT(305, Error, NoRetry, Reconnect, "secure read failed");
            }
            if (n == 0)
            {
                return SMO_ERR_TRANSPORT(303, Error, NoRetry, None, "connection closed during secure read");
            }
            ptr += n;
            remaining -= static_cast<size_t>(n);
        }
    ...
    SecureSession::SecureSession(SecureSession&& other) noexcept
        : fd_(other.fd_), secure_(other.secure_), config_(std::move(other.config_)), crypto_(other.crypto_),
          peer_pk_(std::move(other.peer_pk_)), peer_cert_(std::move(other.peer_cert_)),
    ...
    void SecureSession::derive_keys(BytesView ss1, BytesView ss2, BytesView pk_client, BytesView pk_server)
    {
        Bytes ikm;
        ...
        Bytes material =
            kdf::hkdf(BytesView(kSalt, sizeof(kSalt) - 1), BytesView(ikm), BytesView(kInfo, sizeof(kInfo) - 1), 96);
    ...
    Result<void> SecureSession::client_handshake()
    {
        ...
        auto sig_ok =
            crypto_.signer.verify(BytesView(sig_msg), BytesView(signature), BytesView(cert.value().subject_pubkey));
        if (!sig_ok || !sig_ok.value())
        {
            return SMO_ERR_CERT(216, Error, NoRetry, None, "Server handshake signature invalid");
        }

        // 5. Decapsulate ct → ss1
        auto ss1_res = crypto_.kem.decapsulate(BytesView(local_sk_), BytesView(ct));
        if (!ss1_res)
            return ss1_res.error();
        ...
        derive_keys(BytesView(ss1_res.value()), BytesView(enc2.value().shared_secret), BytesView(local_pk_),
                    BytesView(peer_pk_));
    ...
    Result<void> SecureSession::server_handshake()
    {
        ...
        auto sig_res = crypto_.signer.sign(BytesView(sig_msg), BytesView(config_.signing_secret_key), rng);
        if (!sig_res)
            return sig_res.error();
    ...
    Result<void> SecureSession::send(BytesView plaintext)
    {
        if (!secure_)
        {
            return SMO_ERR_TRANSPORT(312, Error, NoRetry, Reconnect, "send before handshake");
        }

        uint8_t nonce_bytes[kSecureNonceLen];
        build_nonce(tx_nonce_pre_, ++tx_counter_, BytesMutView(nonce_bytes, kSecureNonceLen));

        auto encrypted =
            crypto_.aead.encrypt(plaintext, BytesView{}, BytesView(tx_key_), BytesView(nonce_bytes, kSecureNonceLen));
        if (!encrypted)
    ...
    Result<Bytes> SecureSession::recv()
    {
        if (!secure_)
        {
            return SMO_ERR_TRANSPORT(312, Error, NoRetry, Reconnect, "recv before handshake");
        }

        uint8_t hdr[4];
        SMO_TRY(read_all(fd_, hdr, 4));
        uint32_t total_len = (static_cast<uint32_t>(hdr[0]) << 24) | (static_cast<uint32_t>(hdr[1]) << 16) |
                             (static_cast<uint32_t>(hdr[2]) << 8) | static_cast<uint32_t>(hdr[3]);

        if (total_len > 1024 * 1024)
        {
            return SMO_ERR_TRANSPORT(400, Error, NoRetry, None, "encrypted payload too large");
        }
        ...
        {
            return SMO_ERR_TRANSPORT(400, Error, NoRetry, None, "truncated encrypted payload");
        }

        BytesView wire_nonce(enc.data(), kSecureNonceLen);
        BytesView ciphertext(enc.data() + kSecureNonceLen, enc.size() - kSecureNonceLen);

        // Verify expected nonce
        uint8_t expected[kSecureNonceLen];
        build_nonce(rx_nonce_pre_, rx_counter_ + 1, BytesMutView(expected, kSecureNonceLen));
        if (std::memcmp(wire_nonce.data(), expected, kSecureNonceLen) != 0)
        {
            return SMO_ERR_TRANSPORT(314, Error, NoRetry, Reconnect, "nonce mismatch");
        }
        ++rx_counter_;
