...
namespace smo {
    ...
    static Result<int> create_tcp_socket()
    {
        ...
        if (fd < 0)
        {
            return SMO_ERR_TRANSPORT(306, Error, NoRetry, RestartFSM, "failed to create TCP socket");
        }
        return fd;
    }
    ...
    static Result<sockaddr_in> resolve_endpoint(const Endpoint& ep)
    {
        ...
        else
        {
            ...
            {
                return SMO_ERR_TRANSPORT(308, Error, NoRetry, Reconnect, "DNS resolution failed for " + ep.host);
            }

            std::memcpy(&addr.sin_addr, &((struct sockaddr_in*)result->ai_addr)->sin_addr, sizeof(addr.sin_addr));
            ::freeaddrinfo(result);
        }
    ...
    Result<void> TcpSession::send(BytesView data)
    {
        if (!open_)
        {
            return SMO_ERR_TRANSPORT(303, Error, NoRetry, None, "session is closed");
        }
        ...
        while (remaining > 0)
        {
            ...
            if (n < 0)
            {
                ...
                if (ec == 302)
                    open_ = false; // connection reset
                return SMO_ERR_TRANSPORT(ec, Error, RetrySafe, Reconnect, "TCP send failed");
            }
            ptr += n;
    ...
    Result<Bytes> TcpSession::recv(size_t max_bytes)
    {
        ...
        {
            ssize_t n = ::read(fd_, reinterpret_cast<uint8_t*>(&hdr) + hdr_read, sizeof(FrameHeader) - hdr_read);
            if (n < 0)
            {
                if (errno == EINTR)
                    continue;
                ...
                if (ec == 302)
                    open_ = false;
                return SMO_ERR_TRANSPORT(ec, Error, RetrySafe, Reconnect, "TCP recv header failed");
            }
            if (n == 0)
            {
                open_ = false;
                return SMO_ERR_TRANSPORT(303, Info, NoRetry, None, "connection closed by peer");
            }
        ...
        {
            return SMO_ERR_TRANSPORT(310, Warn, NoRetry, RestartFSM, "invalid frame magic");
        }
        ...
        {
            ssize_t n = ::read(fd_, payload.data() + payload_read, payload_len - payload_read);
            if (n < 0)
            {
                if (errno == EINTR)
                    continue;
                ...
                if (ec == 302)
                    open_ = false;
                return SMO_ERR_TRANSPORT(ec, Error, RetrySafe, Reconnect, "TCP recv payload failed");
            }
            if (n == 0)
            {
                open_ = false;
                return SMO_ERR_TRANSPORT(303, Info, NoRetry, None, "connection closed during payload read");
            }
    ...
    TcpListener::TcpListener(int fd, Endpoint local) : fd_(fd), local_(std::move(local)) {}
    ...
    Result<SessionPtr> TcpListener::accept()
    {
        ...
        if (client_fd < 0)
        {
            ...
            return SMO_ERR_TRANSPORT(304, Error, RetrySafe, RestartFSM, "TCP accept failed");
        }
        ...
        return std::unique_ptr<TransportSession>(new TcpSession(client_fd, std::move(remote)));
    }
    ...
    Result<ListenerPtr> TcpTransport::listen(const Endpoint& ep)
    {
        ...
        if (::bind(fd, (struct sockaddr*)&addr.value(), sizeof(addr.value())) < 0)
        {
            int ec = socket_error_to_code();
            ::close(fd);
            return SMO_ERR_TRANSPORT(ec, Error, NoRetry, RestartFSM, "TCP bind failed");
        }

        if (::listen(fd, 128) < 0)
        {
            ::close(fd);
            return SMO_ERR_TRANSPORT(306, Error, NoRetry, RestartFSM, "TCP listen failed");
        }
        ...
        return std::unique_ptr<TransportListener>(new TcpListener(fd, std::move(actual)));
    }
