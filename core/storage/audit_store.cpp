...
struct AuditStore::Impl {
    ...
    Result<void> open() {
        ...
        rc = sqlite3_prepare_v2(db, "SELECT record_hash FROM audit_records ORDER BY sequence DESC LIMIT 1", -1, &stmt, nullptr);
        if (rc == SQLITE_OK) {
            if (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    Result<AuditQueryResult> exec_query(const AuditQuery& q) const {
        ...
        if (q.actor_id)    sql += " AND actor_id = ?";
        if (q.target_id)   sql += " AND target_id = ?";
        if (q.contract_id) sql += " AND contract_id = ?";
        if (q.execution_id) sql += " AND execution_id = ?";
        if (q.type)        sql += " AND type = ?";
        if (q.from_ns)     sql += " AND timestamp_ns >= ?";
        if (q.to_ns)       sql += " AND timestamp_ns <= ?";
        ...
        int idx = 1;
        if (q.actor_id)    sqlite3_bind_text(stmt, idx++, q.actor_id->c_str(), -1, SQLITE_TRANSIENT);
        if (q.target_id)   sqlite3_bind_text(stmt, idx++, q.target_id->c_str(), -1, SQLITE_TRANSIENT);
        if (q.contract_id) sqlite3_bind_text(stmt, idx++, q.contract_id->c_str(), -1, SQLITE_TRANSIENT);
        if (q.execution_id) sqlite3_bind_text(stmt, idx++, q.execution_id->c_str(), -1, SQLITE_TRANSIENT);
        if (q.type)        sqlite3_bind_int(stmt, idx++, static_cast<int>(*q.type));
        if (q.from_ns)     sqlite3_bind_int64(stmt, idx++, *q.from_ns);
        if (q.to_ns)       sqlite3_bind_int64(stmt, idx++, *q.to_ns);
        ...
        while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            std::string js = R"({"sequence":)" + std::to_string(seq)
                + R"(,"type":)" + std::to_string(typ)
                + R"(,"timestamp_ns":)" + std::to_string(ts)
                + R"(,"actor_id":")" + escape(act) + '"'
                + R"(,"target_id":")" + escape(tgt) + '"'
                + R"(,"contract_id":")" + escape(cid) + '"'
                + R"(,"execution_id":")" + escape(eid) + '"'
                + R"(,"trace_id":")" + escape(tid) + '"'
                + R"(,"details":")" + escape(det) + '"'
                + R"(,"prev_hash":")" + escape(phs) + '"'
                + R"(,"record_hash":")" + escape(rhs) + '"'
                + R"(,"signature":")" + escape(sig) + "\"}";
...
Result<void> AuditStore::open() { return impl_->open(); }
void AuditStore::close() { impl_->close(); }
...
Result<std::vector<std::string>> AuditStore::get_contract_history(
    const std::string& contract_id, uint64_t limit) const {
    AuditQuery q;
...
Result<std::vector<std::string>> AuditStore::get_execution_history(
...
Result<std::vector<std::string>> AuditStore::get_actor_history(
    const std::string& actor_id, uint64_t limit) const {
    AuditQuery q;
    q.actor_id = actor_id;
    q.limit = limit;
    auto r = impl_->exec_query(q);
...
Result<std::vector<std::string>> AuditStore::get_timeline(
...
Result<size_t> AuditStore::count() const {
    ...
    if (rc != SQLITE_OK) return size_t(0);
    size_t n = 0;
...
void AuditStore::on_recovery_approved(const runtime::Event& ev) {
    ...
    r.details = "Certificate revoked: " + reason + " (fingerprint=" + fingerprint + ", epoch=" + std::to_string(epoch) + ")";
    r.sequence = ++impl_->sequence_counter;
    r.prev_hash = impl_->last_hash;
    r.record_hash = impl_->compute_hash(r);
