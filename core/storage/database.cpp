...
namespace smo {
    ...
    Result<void> DatabaseHandle::open(const std::string& path)
    {
        if (db_)
        {
            return SMO_ERR_STORAGE(900, Critical, NoRetry, RebootNode, "database already open");
        }
    ...
    Result<Statement> DatabaseHandle::prepare(const char* sql) const
    {
        ...
        if (rc != SQLITE_OK)
        {
            return SMO_ERR_STORAGE(905, Error, RetrySafe, RetryOperation, sqlite3_errmsg(db_));
        }
    ...
    Result<int> Statement::step()
    {
        if (!stmt_)
        {
            return SMO_ERR_STORAGE(905, Error, RetrySafe, RetryOperation, "statement not prepared");
        }
    ...
    Result<void> Statement::bind_blob(int index, const void* data, int size)
    {
        ...
        if (rc != SQLITE_OK)
        {
            return SMO_ERR_STORAGE(904, Error, RetrySafe, RetryOperation, sqlite3_errmsg(db_));
        }
        return {};
    }
    ...
    {
        int rc = sqlite3_bind_text(stmt_, index, val.c_str(), static_cast<int>(val.size()), SQLITE_TRANSIENT);
        if (rc != SQLITE_OK)
        {
            return SMO_ERR_STORAGE(904, Error, RetrySafe, RetryOperation, sqlite3_errmsg(db_));
        }
    ...
    Bytes Statement::column_blob(int index) const
    {
        ...
        if (size > 0)
            std::memcpy(out.data(), data, static_cast<size_t>(size));
        return out;
    }
