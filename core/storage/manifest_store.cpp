...
namespace smo {
    ...
    Result<void> ManifestStore::open(std::string_view base_path)
    {
        ...
        if (ec)
        {
            return SMO_ERR_STORAGE(900, Error, RetrySafe, None, "Failed to create manifests dir: " + ec.message());
        }
        opened_ = true;
        return {};
    }
    ...
    Result<void> ManifestStore::update_latest_symlink(uint64_t epoch)
    {
        ...
        if (ec)
        {
            ...
            if (!f)
            {
                return SMO_ERR_STORAGE(900, Error, RetrySafe, None, "Failed to create LATEST marker");
            }
            f << target << "\n";
        }
        return {};
    ...
    Result<void> ManifestStore::put(const ManifestRecord& rec)
    {
        ...
        if (!f)
        {
            return SMO_ERR_STORAGE(900, Error, RetrySafe, None, "Failed to write manifest to " + path);
        }
        f.write(reinterpret_cast<const char*>(rec.manifest_cbor.data()), rec.manifest_cbor.size());
    ...
    Result<ManifestRecord> ManifestStore::get(uint64_t epoch) const
    {
        ...
        if (!std::filesystem::exists(path))
        {
            return SMO_ERR_STORAGE(404, Info, NoRetry, None, "Manifest epoch " + std::to_string(epoch) + " not found");
        }

        std::ifstream f(path, std::ios::binary | std::ios::ate);
        if (!f)
        {
            return SMO_ERR_STORAGE(900, Error, RetrySafe, None, "Failed to read manifest " + path);
        }
    ...
    Result<std::vector<ManifestRecord>> ManifestStore::since(uint64_t since_epoch) const
    {
        ...
        for (auto ep : epochs.value())
        {
            if (ep > since_epoch)
            {
                auto rec = get(ep);
                if (rec)
                    result.push_back(std::move(rec.value()));
            }
        }
        return result;
    }
