...
namespace smo {
    ...
    Result<int> MigrationRunner::current_version()
    {
        ...
        {
            return SMO_ERR_STORAGE(905, Error, RetrySafe, RetryOperation, "PRAGMA user_version returned no row");
        }

        return static_cast<int>(stmt.value().column_int64(0));
    }

    Result<void> MigrationRunner::ensure_schema(const StoreInfo& info, const char* create_sql)
    {
        ...
        {
            return SMO_ERR_STORAGE(900, Critical, NoRetry, RebootNode, "database schema is newer than this build");
        }

        if (ver.value() < info.schema_version)
        {
            return SMO_ERR_STORAGE(901, Critical, NoRetry, RebootNode, "database requires migration");
        }
