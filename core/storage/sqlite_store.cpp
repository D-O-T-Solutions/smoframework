...
namespace smo {
    ...
    {
        auto stmt = db_.prepare("INSERT OR REPLACE INTO kv (key, value) VALUES (?1, ?2);");
        if (!stmt)
            return stmt.error();
        ...
        {
            return SMO_ERR_STORAGE(904, Error, RetrySafe, RetryOperation, "unexpected SQLite result");
        }
        return {};
    }
    ...
    {
        auto stmt = db_.prepare("SELECT value FROM kv WHERE key = ?1;");
        if (!stmt)
            return stmt.error();
        ...
        return SMO_ERR_STORAGE(902, Info, RetrySafe, None, "key not found in store");
    }

    Result<void> SqliteStore::del(BytesView key)
    {
        auto stmt = db_.prepare("DELETE FROM kv WHERE key = ?1;");
        if (!stmt)
            return stmt.error();
    ...
    {
        auto stmt = db_.prepare("SELECT key FROM kv WHERE key >= ?1;");
        if (!stmt)
            return stmt.error();
    ...
    Result<void> SqliteStore::backup(const std::string& dest_path)
    {
        if (!db_.is_open())
        {
            return SMO_ERR_STORAGE(912, Error, RetrySafe, RetryOperation, "database not open");
        }
        ...
        if (rc != SQLITE_OK)
        {
            ...
            return SMO_ERR_STORAGE(912, Error, RetrySafe, RetryOperation, "cannot open backup destination");
        }

        sqlite3_backup* backup = sqlite3_backup_init(dest_db, "main", db_.handle(), "main");

        if (!backup)
        {
            sqlite3_close(dest_db);
            return SMO_ERR_STORAGE(912, Error, RetrySafe, RetryOperation, "backup init failed");
        }
    ...
    Result<void> SqliteStore::restore(const std::string& src_path)
    {
        ...
        if (ec)
        {
            return SMO_ERR_STORAGE(913, Critical, NoRetry, ManualIntervention, ec.message().c_str());
        }
