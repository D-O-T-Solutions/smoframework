...
namespace smo {
    ...
    static std::string identity_key()
    {
        return "id:self";
    }
    ...
    static bool identity_deserialize(const std::string& data, NodeStore::Identity& id)
    {
        ...
        if (!std::getline(is, id.node_id, '\0'))
            return false;
        for (auto& b : id.public_key.value)
        {
            int c = is.get();
    ...
    {
        std::ostringstream os;
        os << e.node_id << '\0' << e.address << '\0' << e.port << '\0' << e.last_seen << '\0' << e.trust_score;
        return os.str();
    }
    ...
    static bool route_deserialize(const std::string& data, NodeStore::RouteEntry& e)
    {
        std::istringstream is(data);
        std::string field;
        if (!std::getline(is, e.node_id, '\0'))
            return false;
        if (!std::getline(is, e.address, '\0'))
            return false;
    ...
    void NodeStore::close() noexcept
    {
        if (store_)
            store_->close();
    }
    std::error_code NodeStore::flush() noexcept
    {
        return {};
    }
    ...
    std::error_code NodeStore::put_identity(const Identity& id)
    {
        if (!store_)
            return std::error_code(Errc::STORE_UNAVAILABLE);
        auto res = store_->put(to_bytes(identity_key()), to_bytes(identity_serialize(id)));
        if (!res)
    ...
    std::optional<NodeStore::Identity> NodeStore::get_identity()
    {
        ...
        if (!res)
            return std::nullopt;
        Identity id;
        if (!identity_deserialize(to_str(res.value()), id))
            return std::nullopt;
        return id;
    }
    ...
    std::error_code NodeStore::put_route(const RouteEntry& entry)
    {
        if (!store_)
            return std::error_code(Errc::STORE_UNAVAILABLE);
        auto res = store_->put(to_bytes(route_key(entry.node_id)), to_bytes(route_serialize(entry)));
        if (!res)
            return std::error_code(Errc::STORE_CORRUPTION);
        return {};
    }
    ...
    std::optional<NodeStore::RouteEntry> NodeStore::get_route(const std::string& node_id)
    {
        ...
        if (!res)
            return std::nullopt;
        RouteEntry e;
        if (!route_deserialize(to_str(res.value()), e))
            return std::nullopt;
        return e;
    }
