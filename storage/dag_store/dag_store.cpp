...
namespace smo {
    ...
    static std::string dag_serialize(const DagStore::DagRecord& r)
    {
        ...
        os << r.graph_id << '\0' << r.intent_id << '\0' << r.dag_json << '\0' << r.dag_hash << '\0' << r.compiled_at;
        return os.str();
    }
    ...
    static bool dag_deserialize(const std::string& data, DagStore::DagRecord& r)
    {
        ...
        if (!std::getline(is, r.graph_id, '\0'))
            return false;
        if (!std::getline(is, r.intent_id, '\0'))
            return false;
        if (!std::getline(is, r.dag_json, '\0'))
            return false;
        if (!std::getline(is, r.dag_hash, '\0'))
            return false;
        std::string field;
    ...
    void DagStore::close() noexcept
    {
        if (store_)
            store_->close();
    }
    std::error_code DagStore::flush() noexcept
    {
        return {};
    }
    ...
    std::error_code DagStore::put(const DagRecord& dag)
    {
        if (!store_)
            return std::error_code(Errc::STORE_UNAVAILABLE);
        auto res = store_->put(to_bytes(dag_key(dag.graph_id)), to_bytes(dag_serialize(dag)));
        if (!res)
    ...
    std::optional<DagStore::DagRecord> DagStore::get(const std::string& graph_id)
    {
        ...
        if (!res)
            return std::nullopt;
        DagRecord r;
        if (!dag_deserialize(to_str(res.value()), r))
            return std::nullopt;
        return r;
    }
