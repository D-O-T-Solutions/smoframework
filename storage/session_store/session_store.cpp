...
namespace smo {
    ...
    static std::string session_serialize(const Session& s)
    {
        ...
        os << s.requester << '\0' << s.responder << '\0' << s.capabilities.to_ulong() << '\0' << s.created_at << '\0'
           << s.expires_at << '\0' << (s.active ? 1 : 0);
        return os.str();
    }
    ...
    void SessionStore::close() noexcept
    {
        if (store_)
            store_->close();
    }
    std::error_code SessionStore::flush() noexcept
    {
        return {};
    }
    ...
    std::error_code SessionStore::put(const Session& session)
    {
        if (!store_)
            return std::error_code(Errc::STORE_UNAVAILABLE);
        auto res = store_->put(to_bytes(session_key(session.id)), to_bytes(session_serialize(session)));
        if (!res)
    ...
    std::optional<Session> SessionStore::get(const SessionId& id)
    {
        ...
        if (!res)
            return std::nullopt;
        Session s;
        if (!session_deserialize(to_str(res.value()), s))
            return std::nullopt;
        s.id = id;
        return s;
    }
    ...
    std::error_code SessionStore::expire_before(int64_t timestamp)
    {
        ...
        for (auto& key : list_res.value())
        {
            ...
            if (!val_res)
                continue;
            Session s;
            if (!session_deserialize(to_str(val_res.value()), s))
                continue;
