...
namespace smo {
    ...
    static std::string record_serialize(const TrustStore::Record& r)
    {
        ...
        os << r.node_id << '\0' << r.citizen_score << '\0' << r.execution_score << '\0' << r.witness_score << '\0'
           << r.consistency_score << '\0' << r.composite << '\0' << r.updated_at;
        return os.str();
    }
    ...
    void TrustStore::close() noexcept
    {
        if (store_)
            store_->close();
    }
    std::error_code TrustStore::flush() noexcept
    {
        return {};
    }
    ...
    std::error_code TrustStore::put(const Record& rec)
    {
        if (!store_)
            return std::error_code(Errc::STORE_UNAVAILABLE);
        auto res = store_->put(to_bytes(trust_key(rec.node_id)), to_bytes(record_serialize(rec)));
        if (!res)
