...
namespace smo {
    ...
    static std::string record_serialize(const PolicyStore::Record& r)
    {
        ...
        os << r.name << '\0' << r.description << '\0' << r.version << '\0' << r.yaml_content << '\0' << r.created_at
           << '\0' << r.created_by << '\0' << r.updated_at;
        return os.str();
    }
    ...
    static bool record_deserialize(const std::string& data, PolicyStore::Record& r)
    {
        ...
        std::string field;
        if (!std::getline(is, r.name, '\0'))
            return false;
        if (!std::getline(is, r.description, '\0'))
            return false;
        if (!std::getline(is, r.version, '\0'))
            return false;
        if (!std::getline(is, r.yaml_content, '\0'))
            return false;
        if (!std::getline(is, field, '\0'))
            return false;
        r.created_at = std::stoll(field);
        if (!std::getline(is, r.created_by, '\0'))
            return false;
    ...
    std::error_code PolicyStore::open() noexcept
    {
        ...
        if (!res)
            return std::error_code(Errc::STORE_UNAVAILABLE);
        return {};
    }

    void PolicyStore::close() noexcept
    {
        if (store_)
            store_->close();
    }
    std::error_code PolicyStore::flush() noexcept
    {
        return {};
    }
    ...
    std::error_code PolicyStore::put(const Record& rec)
    {
        if (!store_)
            return std::error_code(Errc::STORE_UNAVAILABLE);
        auto res = store_->put(to_bytes(policy_key(rec.name)), to_bytes(record_serialize(rec)));
        if (!res)
            return std::error_code(Errc::STORE_CORRUPTION);
        store_version_++;
        return {};
    }
    ...
    std::optional<PolicyStore::Record> PolicyStore::get(const std::string& name)
    {
        ...
        if (!res)
            return std::nullopt;
        Record r;
        if (!record_deserialize(to_str(res.value()), r))
            return std::nullopt;
        return r;
    }
