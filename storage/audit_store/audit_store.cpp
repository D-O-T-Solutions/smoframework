...
namespace smo {
    ...
    {
        std::ostringstream os;
        os << e.sequence << '\0' << e.contract_id << '\0' << e.from_state << '\0' << e.to_state << '\0' << e.trigger
           << '\0' << e.timestamp << '\0' << e.signature;
        return os.str();
    }
    ...
    static bool entry_deserialize(const std::string& data, AuditStore::Entry& e)
    {
        ...
        std::string field;
        if (!read_field(is, field))
            return false;
        e.sequence = std::stoll(field);
        if (!read_field(is, e.contract_id))
            return false;
        if (!read_field(is, e.from_state))
            return false;
        if (!read_field(is, e.to_state))
            return false;
        if (!read_field(is, e.trigger))
            return false;
        if (!read_field(is, field))
            return false;
    ...
    void AuditStore::close() noexcept
    {
        if (store_)
            store_->close();
    }
    std::error_code AuditStore::flush() noexcept
    {
        return {};
    }
    ...
    std::error_code AuditStore::append(const Entry& entry)
    {
        ...
        auto res = store_->put(to_bytes(entry_key(e.contract_id, e.sequence)), to_bytes(entry_serialize(e)));
        if (!res)
    ...
    std::optional<AuditStore::Entry> AuditStore::last(const std::string& contract_id)
    {
        ...
        Bytes prefix = to_bytes("aud:" + contract_id + ":");
        auto list_res = store_->list(prefix);
        if (!list_res || list_res.value().empty())
            return std::nullopt;
        ...
        if (!last_key)
            return std::nullopt;
        Entry e;
        if (!entry_deserialize(to_str(last_key.value()), e))
            return std::nullopt;
        return e;
    }
