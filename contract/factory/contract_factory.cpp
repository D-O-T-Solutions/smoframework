...
namespace smo {
    ...
    ContractFactory::ContractFactory(ContractRegistry& registry, OpcodeRegistry& opcodes)
    ...
    Result<ContractDefinition> ContractFactory::resolve(const Intent& intent)
    {
        ...
        if (!op_res)
        {
            return SMO_ERR_CRYPTO(8, Error, NoRetry, None, "unknown opcode in intent");
        }
        ...
        if (!def)
        {
            return SMO_ERR_CRYPTO(8, Error, NoRetry, None, "no active contract for opcode");
        }

        return def.value();
    }

    Result<ContractDefinition> ContractFactory::resolve_by_id(const ContractID& id)
    {
