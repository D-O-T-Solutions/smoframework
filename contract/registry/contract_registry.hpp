...
namespace smo {
    ...
    public:
        ...
        ContractID publish(const std::string& canonical_json, const std::string& signature,
                           const std::string& publisher_id);

        ContractID register_native(std::string_view canonical_json);

        Result<ContractDefinition> resolve(std::string_view opcode, std::string_view version_constraint = "");
        ...
        Result<void> deprecate(const ContractID& id, std::string_view reason, std::string_view publisher_signature);

        std::vector<ContractDefinition> list(std::string_view opcode = "", std::string_view category = "",
