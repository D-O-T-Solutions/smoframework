...
namespace smo {
    ...
    static std::string make_native_json(const char* opcode, const char* name, const char* params_schema,
                                        const char* cap, uint32_t cap_level, bool idempotent, uint32_t timeout)
    {
        ...
        os << R"("compiler_hints":{"max_parallelism":1,"timeout_sec":)" << timeout << R"(,"idempotent":)"
           << (idempotent ? "true" : "false") << R"(},)";
        os << R"("signature":null)";
        os << R"(})";
        return os.str();
    }
    ...
    {
        // ls
        registry.register_native(make_native_json("ls", "List Directory",
                                                  R"([{"name":"path","type":"string","required":true}])",
                                                  "filesystem_read", 1, true, 30));

        // put
        registry.register_native(make_native_json("put", "Write File",
                                                  R"([{"name":"source","type":"string","required":true},)"
                                                  R"({"name":"target","type":"string","required":true}])",
                                                  "filesystem_write", 1, true, 60));

        // get
        registry.register_native(make_native_json("get", "Read File",
                                                  R"([{"name":"path","type":"string","required":true}])",
                                                  "filesystem_read", 1, true, 60));

        // exec
        registry.register_native(make_native_json("exec", "Execute Command",
                                                  R"([{"name":"command","type":"string","required":true},)"
                                                  R"({"name":"args","type":"array","required":false}])",
                                                  "process_exec", 1, false, 300));

        // quarantine
        registry.register_native(make_native_json("quarantine", "Quarantine Node",
