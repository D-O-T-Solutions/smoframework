...
namespace smo {
    ...
    ContractRegistry::ContractRegistry(std::string base_path) : base_path_(std::move(base_path)) {}
    ...
    Result<void> ContractRegistry::open()
    {
        ...
        auto r1 = db.exec("CREATE TABLE IF NOT EXISTS contract_registry ("
        ...
        if (!r1)
            return r1;
        auto r2 = db.exec("CREATE INDEX IF NOT EXISTS idx_registry_opcode "
                          "ON contract_registry(opcode)");
        if (!r2)
            return r2;
        return db.exec("CREATE INDEX IF NOT EXISTS idx_registry_publisher "
                       "ON contract_registry(publisher)");
    }

    ContractID ContractRegistry::publish(const std::string& canonical_json, const std::string& signature,
                                         const std::string& publisher_id)
    {
        ...
        auto& db = store_->database();
        auto stmt_res = db.prepare("INSERT OR IGNORE INTO contract_registry "
    ...
    ContractID ContractRegistry::register_native(std::string_view canonical_json)
    {
        auto def_res = ContractDefinition::from_canonical_json(canonical_json);
        if (!def_res)
            return {};
    ...
    Result<ContractDefinition> ContractRegistry::resolve(std::string_view opcode, std::string_view)
    {
        auto& db = store_->database();
        auto stmt_res = db.prepare("SELECT canonical_json FROM contract_registry "
    ...
    Result<ContractDefinition> ContractRegistry::resolve_by_name(std::string_view name)
    {
        auto& db = store_->database();
        auto stmt_res = db.prepare("SELECT canonical_json FROM contract_registry "
    ...
    Result<ContractDefinition> ContractRegistry::get(std::string_view contract_id_hex)
    {
        auto& db = store_->database();
        auto stmt_res = db.prepare("SELECT canonical_json FROM contract_registry WHERE contract_id = ?");
        if (!stmt_res)
    ...
    Result<void> ContractRegistry::deprecate(const ContractID& id, std::string_view reason, std::string_view)
    {
        auto& db = store_->database();
        auto stmt_res = db.prepare("UPDATE contract_registry SET status = 'deprecated', "
    ...
    std::vector<ContractDefinition> ContractRegistry::list(std::string_view opcode, std::string_view category,
                                                           std::string_view status)
    {
        ...
        while (true)
        {
            ...
            if (def)
                results.push_back(std::move(def.value()));
        }
        return results;
    }
