...
namespace smo::hl {
    ...
    static Result<sockaddr_in> resolve(const std::string& host, uint16_t port)
    {
        ...
        else
        {
            ...
            {
                return SMO_ERR_TRANSPORT(308, Error, NoRetry, Reconnect, "DNS resolution failed for " + host);
            }
    ...
    struct TcpTransport::Impl
    {
        ...
        void accept_loop()
        {
            while (running && listen_fd_ >= 0)
            {
                ...
                int client_fd = ::accept(listen_fd_, reinterpret_cast<sockaddr*>(&peer_addr), &addrlen);
                if (client_fd < 0)
                {
                    if (errno == EINTR)
                        continue;
                ...
                reader_threads.emplace_back([this, key, remote]() { reader_loop(key, remote); });
            }
        }
        ...
        void reader_loop(const std::string& key, const Endpoint& remote)
        {
            ...
            while (running && s->fd >= 0)
            {
                ...
                {
                    ssize_t n =
                        ::read(s->fd, reinterpret_cast<uint8_t*>(&hdr) + hdr_read, sizeof(FrameHeader) - hdr_read);
                    if (n < 0)
                    {
                        if (errno == EINTR)
                            continue;
                        if (on_error)
                            on_error(errc_from_code(sock_err_to_code()), remote);
                        return;
                    }
                    if (n == 0)
                        return;
                ...
                {
                    if (on_error)
                        on_error(errc_from_code(310), remote);
                    return;
                }
                ...
                {
                    ssize_t n = ::read(s->fd, payload.data() + payload_read, payload_len - payload_read);
                    if (n < 0)
                    {
                        if (errno == EINTR)
                            continue;
                ...
                {
                    if (on_error)
                        on_error(errc_from_code(static_cast<int>(pkt.error().code.code)), remote);
                    continue;
                }
    ...
    TcpTransport::TcpTransport() noexcept : impl_(std::make_unique<Impl>()) {}

    TcpTransport::~TcpTransport() noexcept = default;

    std::error_code TcpTransport::listen(const Endpoint& ep, PacketHandler on_packet, ErrorHandler on_error)
    {
