#include "suite1_classical_provider.hpp"
...
namespace smo {
    namespace providers {
        ...
        namespace {
            ...
            Result<Bytes> sha256_hash_fn(BytesView data)
            {
                return hash::Sha256Provider().hash(data);
            }

            Result<Bytes> sha256_hmac_fn(BytesView key, BytesView data)
            {
