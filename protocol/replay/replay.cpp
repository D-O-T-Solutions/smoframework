...
namespace smo {
    ...
    ReplayProtector::ReplayProtector(ReplayConfig cfg) noexcept : impl_(std::make_unique<Impl>(Impl{cfg, {}, {}})) {}
    ...
    bool ReplayProtector::accept(std::array<uint8_t, 8> nonce, int64_t timestamp) noexcept
    {
        ...
        if (delta < 0)
            delta = -delta;
        if (delta > impl_->cfg.max_time_delta_ms)
            return false;

        uint64_t val = nonce_to_u64(nonce);

        // Nonce seen before?
        if (impl_->seen.count(val))
            return false;
    ...
    bool ReplayProtector::accept(std::array<uint8_t, 8> nonce, int64_t timestamp, int64_t now) noexcept
    {
        int64_t delta = now - timestamp;
        if (delta < 0)
            delta = -delta;
