...
namespace smo {
    ...
    void keypair_generate(PublicKey& pk, SecretKey& sk)
    {
        ...
        std::memcpy(pk.value.data(), kp.public_key.data(), std::min(pk.value.size(), kp.public_key.size()));
        std::memcpy(sk.value.data(), kp.secret_key.data(), std::min(sk.value.size(), kp.secret_key.size()));
    }
    ...
    Signature sign(std::span<const uint8_t> msg, const SecretKey& sk)
    {
        ...
        std::memcpy(sig.value.data(), sig_bytes.data(), std::min(sig.value.size(), sig_bytes.size()));
        return sig;
    }
