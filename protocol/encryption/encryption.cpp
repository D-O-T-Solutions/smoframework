...
namespace smo {
    ...
    std::vector<uint8_t> encrypt(std::span<const uint8_t> plaintext, const SymmetricKey& key, const Nonce& nonce)
    {
    ...
    std::vector<uint8_t> decrypt(std::span<const uint8_t> ciphertext, const SymmetricKey& key, const Nonce& nonce)
    {
        auto& reg = CryptoRegistry::instance();
        auto prov_res = reg.get_suite(kSuiteClassical);
        if (!prov_res)
