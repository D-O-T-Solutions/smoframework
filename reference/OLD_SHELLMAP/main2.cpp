...
void send_secure_json(string peer_key, uint8_t op_code, json j_payload)
{
    ...
    memcpy(hdr.magic, "SM", 2);
    hdr.op_code = op_code;
    hdr.flags = PacketFlag::ENCRYPTED;
    memcpy(hdr.sender_id, my_identity.node_id.c_str(), 16);
    hdr.seq = 0;
...
void handle_incoming_packet(PacketHeader& hdr, vector<uint8_t>& payload, sockaddr_in& sender)
{
    ...
    {
        if (payload.size() < sizeof(Payload_Iam_Kyber512))
            return;
    ...
    {
        if (payload.size() < sizeof(Payload_Hello))
            return;
        ...
        string kyber_ss;
        if (!CryptoCore::kyber_decaps(my_identity.kyber_priv, kyber_ct, kyber_ss))
            return;
        ...
        string ecdh_ss;
        if (!CryptoCore::compute_x25519_secret(my_ecdh_priv, client_ecdh_pub, ecdh_ss))
            return;
    ...
    else if (hdr.op_code == OpCode::WELCOME)
    {
        ...
        string ecdh_ss;
        if (!CryptoCore::compute_x25519_secret(sess.temp_ecdh_priv, server_ecdh_pub, ecdh_ss))
            return;
    ...
    {
        if (payload.size() < sizeof(Payload_PubKey))
            return;
        ...
        // 2. Verify chữ ký cũ
        if (sess.temp_signature.empty() || sess.handshake_data.empty())
            return;
        bool is_valid = CryptoCore::falcon_verify(sess.peer_falcon_pub, sess.handshake_data, sess.temp_signature);
    ...
    else if (hdr.flags & PacketFlag::ENCRYPTED)
    {
        ...
        if (CryptoCore::aes_gcm_decrypt(sess.aes_key, sess.aes_nonce, ciphertext, tag, aad, plaintext))
        {
            ...
            try
            {
                ...
                {
                    if (!dl_state.is_active)
                        return; // Nếu user đã hủy tải thì bỏ qua
                    ...
                    cout << "\n\r[DOWNLOADING] " << percent << "% (" << dl_state.current_offset << "/"
                         << dl_state.total_size << ")" << flush;
                    ...
                    else
                    {
                        ...
                        while (up_state.current_offset < up_state.total_size)
                        {
                            ...
                            vector<uint8_t> chunk =
                                file_mgr.read_file_chunk(up_state.filename, up_state.current_offset, 1024);
                            ...
                            if (up_state.current_offset % (100 * 1024) == 0)
                            { // Cứ 100KB in 1 lần cho đỡ lag console
                                cout << "." << flush;
                            }
                        }
    ...
    {
        if (payload.size() < sizeof(Payload_FindNode))
            return; // Chặn crash
        ...
        else if (route_table.get_route(target_id, t_ip, t_port))
        {
            ...
            if (!is_relay_mode)
            {
                ...
                PacketHeader as_hdr;
                memset(&as_hdr, 0, sizeof(as_hdr));
                memcpy(as_hdr.magic, "SM", 2);
                as_hdr.op_code = OpCode::P2P_ASSIST_REQ;
                as_hdr.payload_len = sizeof(Payload_NodeInfo);
                memcpy(as_hdr.sender_id, my_identity.node_id.c_str(), 16);
                vector<uint8_t> pl2(sizeof(Payload_NodeInfo));
                memcpy(pl2.data(), &assist_info, sizeof(Payload_NodeInfo));
    ...
    else if (hdr.op_code == OpCode::P2P_FIND_NODE_RES)
    {
        ...
        PacketHeader hello_hdr;
        memset(&hello_hdr, 0, sizeof(hello_hdr));
        memcpy(hello_hdr.magic, "SM", 2);
    ...
    else if (hdr.op_code == OpCode::P2P_ASSIST_REQ)
    {
        ...
        PacketHeader punch_hdr;
        memset(&punch_hdr, 0, sizeof(punch_hdr));
        memcpy(punch_hdr.magic, "SM", 2);
    ...
    else if (hdr.op_code == OpCode::RELAY_REQ)
    { // RELAY_REQ
        ...
        try
        {
            ...
            cout << "[MIDDLEMAN] Dang ship hang tu " << clean_id(hdr.sender_id, 16).substr(0, 8) << " -> toi "
                 << target_id.substr(0, 8) << endl;
...
void handle_user_input()
{
    ...
    else if (cmd == "list")
    {
        ...
        for (auto const& [key, sess] : session_table)
        {
            ...
            cout << marker << " " << key << " | ID: " << sess.node_id << "... | " << status << endl;
        }
    }
    else if (cmd == "mode")
    {
...
int main()
{
    ...
    while (true)
    {
        ...
        if (upnp_status == 0)
        {
            ...
            cout << "\n>>> NETWORK INFO:\n";
            cout << "    [LAN IP]    \033[1;33m" << upnp.local_ip << "\033[0m : " << my_identity.port
                 << " (Use for Local/Wifi)\n";
            cout << "    [PUBLIC IP] \033[1;32m" << upnp.public_ip << "\033[0m : " << my_identity.port
                 << " (Use for Internet)\n";
