...
uint64_t VerificationManager::initiate_verification(const std::string& node_id, const std::string& declared_ip,
...
bool VerificationManager::handle_probe_ack(const std::string& node_id, uint64_t probe_id, uint32_t checksum)
{
    auto it = verifications_.find(node_id);
    if (it == verifications_.end())
        return false;

    auto req = it->second;
    if (req->probe_id != probe_id)
        return false;
    if (req->probe_checksum != checksum)
        return false;
