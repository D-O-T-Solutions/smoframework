...
CommandHandler::CommandHandler(std::shared_ptr<SessionManager> session_mgr,
                               std::shared_ptr<VerificationManager> verify_mgr, std::shared_ptr<Web3Manager> web3_mgr,
                               FileManager* file_mgr, NetworkEngine& net_engine, RoutingTable& route_table,
                               std::shared_ptr<ProtocolManager> proto_mgr, NodeContext* node_ctx)
    : session_mgr_(session_mgr), verify_mgr_(verify_mgr), web3_mgr_(web3_mgr), file_mgr_(file_mgr),
      net_engine_(net_engine), route_table_(route_table), proto_mgr_(proto_mgr), node_ctx_(node_ctx)
{
...
bool CommandHandler::process_command(const std::string& cmd)
{
    ...
    if (command == "peers")
        return cmd_list_peers(args);
    if (command == "nodes")
        return cmd_list_nodes(args);
    if (command == "relay")
        return cmd_relay_mode(args);
    if (command == "verify")
    ...
    if (command == "cd")
        return cmd_cd(args);
    if (command == "myshare")
        return cmd_myshare(args);
    if (command == "mydownload")
        return cmd_mydownload(args);
    if (command == "bye")
        return cmd_bye(args);
    if (command == "return")
        return cmd_return(args);
    ...
    std::cout << COLOR_RED << "✗ Unknown command: " << command << ". Type 'help' for options." << COLOR_RESET
              << std::endl;
    return false;
}
...
bool CommandHandler::get_active_session()
{
    ...
    for (const auto& [node_id, sess] : all_sessions)
    {
        ...
        if (peer_addr == current_target_)
        {
            ...
            std::cout << COLOR_CYAN << "[DEBUG] Mapped " << current_target_ << " -> " << node_id.substr(0, 8) << "..."
...
bool CommandHandler::cmd_connect(const std::vector<std::string>& args)
{
    ...
    std::cout << COLOR_YELLOW << "[CONNECT] Initiating handshake with " << target_ip << ":" << target_port << "..."
              << COLOR_RESET << std::endl;
    ...
    std::cout << COLOR_CYAN << "[CONNECT] Handshake in progress (async). Monitor with 'peers' command." << COLOR_RESET
              << std::endl;
...
bool CommandHandler::cmd_list_peers(const std::vector<std::string>& args)
{
    ...
    // Separator line
    std::cout << "|" << std::string(5, '-') << "|" << std::string(20, '-') << "|" << std::string(22, '-') << "|"
              << std::string(17, '-') << "|" << std::string(20, '-') << "|\n";

    // List sessions with index
    int index = 1;
    for (const auto& [node_id, session] : sessions)
    {
        std::string short_id = node_id.length() > 16 ? node_id.substr(0, 13) + "..." : node_id;

        std::string addr = session->peer_ip + ":" + std::to_string(session->peer_port);
        if (addr.length() > 18)
            addr = addr.substr(0, 15) + "...";
        ...
        std::cout << "| " << std::setw(3) << index << "| " << std::setw(18) << short_id << "| " << std::setw(20) << addr
                  << "| " << status << "| " << encryption << "\n";
        index++;
    }
...
bool CommandHandler::cmd_list_nodes(const std::vector<std::string>& args)
{
    ...
    if (all_nodes.empty())
    {
        std::cout << COLOR_YELLOW << "(No nodes known yet. Connect to peers to populate the routing table)"
                  << COLOR_RESET << "\n\n";
        return true;
    }
    ...
    // Separator line
    std::cout << "|" << std::string(22, '-') << "|" << std::string(27, '-') << "|" << std::string(17, '-') << "|"
              << std::string(22, '-') << "|\n";
    ...
    for (const auto& node : all_nodes)
    {
        ...
        std::string time_str;
        if (diff < 60)
            time_str = std::to_string(diff) + "s ago";
        else if (diff < 3600)
            time_str = std::to_string(diff / 60) + "m ago";
        else if (diff < 86400)
            time_str = std::to_string(diff / 3600) + "h ago";
        else
            time_str = "> 1d ago";

        std::cout << "| " << std::setw(20) << short_id << "| " << std::setw(25) << addr << "| " << std::setw(15) << type
...
bool CommandHandler::cmd_relay_mode(const std::vector<std::string>& args)
{
    ...
    {
        std::cout << COLOR_MAGENTA << "[RELAY] Current mode: " << (is_relay_mode_ ? COLOR_GREEN "ON" : COLOR_RED "OFF")
                  << COLOR_RESET << std::endl;
    }
    return true;
}
...
bool CommandHandler::cmd_ping(const std::vector<std::string>& args)
{
    ...
    std::cout << COLOR_YELLOW << "[PING] Sending to " << target_ip << ":" << target_port << "..." << COLOR_RESET
              << std::endl;
...
bool CommandHandler::cmd_me(const std::vector<std::string>& args)
{
    ...
    std::cout << COLOR_CYAN << std::left << std::setw(20) << "Node ID:" << COLOR_RESET << node_ctx_->node_id << "\n";

    // Public IP and Port (from NAT detection or config)
    std::string public_addr = node_ctx_->public_ip.empty() ? "127.0.0.1" : node_ctx_->public_ip;
    int public_port = node_ctx_->public_port > 0 ? node_ctx_->public_port : node_ctx_->port;

    std::cout << COLOR_CYAN << std::left << std::setw(20) << "Public IP:Port:" << COLOR_RESET << public_addr << ":"
              << public_port << "\n";

    // Listen Port
    std::cout << COLOR_CYAN << std::left << std::setw(20) << "Listen Port:" << COLOR_RESET << node_ctx_->port << "\n";

    // Mode (Relay or Direct)
    std::cout << COLOR_CYAN << std::left << std::setw(20) << "Mode:" << COLOR_RESET;
    std::cout << (is_relay_mode_ || node_ctx_->is_relay ? COLOR_MAGENTA "RELAY" : COLOR_GREEN "DIRECT") << COLOR_RESET
              << "\n";

    // Encryption status
    std::cout << COLOR_CYAN << std::left << std::setw(20) << "Encryption:" << COLOR_RESET << COLOR_YELLOW
              << "[ENCRYPTED]" << COLOR_RESET << " AES-256-GCM + Kyber-512\n";

    // Active Sessions
    auto sessions = session_mgr_->get_all_sessions();
    std::cout << COLOR_CYAN << std::left << std::setw(20) << "Active Sessions:" << COLOR_RESET << sessions.size()
              << "\n";
...
bool CommandHandler::cmd_put(const std::vector<std::string>& args)
{
    ...
    if (requested_filename[0] == '/' || requested_filename[0] == '\\' ||
        requested_filename.find("..") != std::string::npos || requested_filename.find(':') != std::string::npos)
    { // Block Windows absolute paths (C:\)
    ...
    try
    {
        while (!data_file.eof())
        {
            ...
            proto_mgr_->hex_string_to_node_id(current_target_,
                                              chunk_hdr.target_id); // FIXED: use hex_string_to_node_id!

            // Send encrypted chunk
            std::vector<uint8_t> encrypted_chunk =
                session_mgr_->encrypt_message(current_target_, chunk_payload, chunk_hdr.packet_nonce);
...
bool CommandHandler::cmd_myshare(const std::vector<std::string>& args)
{
    ...
    if (args.size() < 2)
    {
        ...
        std::cout << COLOR_YELLOW << "Usage: myshare <command> (e.g., myshare ls -la, myshare rm file.txt)"
                  << COLOR_RESET << std::endl;
        return true;
    }
...
// Spawn non-blocking GUI for OS
        ...
        std::string cmd = "gnome-terminal --working-directory=\"" + abs_path +
                          "\" 2>/dev/null || x-terminal-emulator -e \"bash -c 'cd \\\"" + abs_path +
    ...
    if (status != 0)
    {
        std::cout << COLOR_RED << "[MYSHARE] Command exited with status " << WEXITSTATUS(status) << COLOR_RESET
                  << std::endl;
    }
...
bool CommandHandler::cmd_mydownload(const std::vector<std::string>& args)
{
    ...
    if (args.size() < 2)
    {
        ...
        std::cout << COLOR_YELLOW << "Usage: mydownload <command> (e.g., mydownload ls -la, mydownload rm file.txt)"
                  << COLOR_RESET << std::endl;
        return true;
    }
...
// Spawn non-blocking GUI for OS
    ...
    if (status != 0)
    {
        std::cout << COLOR_RED << "[MYDOWNLOAD] Command exited with status " << WEXITSTATUS(status) << COLOR_RESET
                  << std::endl;
    }
...
bool CommandHandler::cmd_bye(const std::vector<std::string>& args)
{
    ...
    std::cout << COLOR_GREEN << "✓ Disconnected from " << current_target_.substr(0, 8) << "..." << COLOR_RESET
              << std::endl;
...
bool CommandHandler::cmd_return(const std::vector<std::string>& args)
{
    ...
    try
    {
        ...
        {
            std::cout << COLOR_RED << "✗ Session " << session_num << " not found (valid: 1-" << established.size()
                      << ")" << COLOR_RESET << std::endl;
            return false;
        }
        ...
        std::cout << COLOR_GREEN << "✓ Switched to session " << session_num << " (" << short_id << " @ "
                  << sess->peer_ip << ":" << sess->peer_port << ")" << COLOR_RESET << std::endl;
        return true;
    }
    catch (const std::exception& e)
    {
...
bool CommandHandler::cmd_myid(const std::vector<std::string>& args)
{
    ...
    clipboard_cmd = "echo -n '" + node_ctx_->node_id + "' | xclip -selection clipboard 2>/dev/null || echo -n '" +
                    node_ctx_->node_id + "' | xsel -b 2>/dev/null";
#endif
