...
ProtocolManager::ProtocolManager(std::shared_ptr<SessionManager> session_mgr,
                                 std::shared_ptr<VerificationManager> verify_mgr,
                                 std::shared_ptr<RelayManager> relay_mgr, std::shared_ptr<Web3Manager> web3_mgr,
                                 FileManager* file_mgr, std::shared_ptr<CommandHandler> cmd_handler,
                                 NetworkEngine& net_engine, RoutingTable& route_table, NodeContext* my_node)
    : session_mgr_(session_mgr), verify_mgr_(verify_mgr), relay_mgr_(relay_mgr), web3_mgr_(web3_mgr),
      file_mgr_(file_mgr), cmd_handler_(cmd_handler), net_engine_(net_engine), route_table_(route_table),
      my_node_(my_node)
{
    //     std::cout << "[PROTOCOL] ProtocolManager initialized" << std::endl;
}

void ProtocolManager::dispatch_packet(PacketHeader& hdr, std::vector<uint8_t>& payload, const sockaddr_in& sender_addr)
{
    ...
    if (!verify_nonce(hdr.sender_id, hdr.packet_nonce))
    {
        ...
        std::cout << COLOR_RED << "[REPLAY-DROP] Packet from " << sender_id_str.substr(0, 8) << "... (OpCode=0x"
                  << std::hex << (int)hdr.op_code << std::dec << ", nonce=" << hdr.packet_nonce << ")" << COLOR_RESET
                  << std::endl;
        return;
    }
...
void ProtocolManager::handle_hello(PacketHeader& hdr, std::vector<uint8_t>& payload, const sockaddr_in& sender)
{
    ...
    // std::cout << "\n[HANDSHAKE-2] === RESPOND: Received HELLO from " << peer_id_str.substr(0, 8) << "... ===" <<
    // std::endl;

    if (payload.size() < 832)
    { // 800B Kyber + 32B X25519 (NO port field!)
    ...
    if (session->temp_ecdh_priv.empty())
    {
        ...
        // std::cout << "[HANDSHAKE-2] Generated new X25519 Pub (as responder): " << bytes_to_hex_short(our_x25519_pub)
        // << "..." << std::endl;
    }
    else
    {
        ...
        // std::cout << "[HANDSHAKE-2] Reusing X25519 Pub (already initiated): " << bytes_to_hex_short(our_x25519_pub)
        // << "..." << std::endl;
    }
    ...
    welcome_payload.insert(welcome_payload.end(), (uint8_t*)kyber_ct.data(),
                           (uint8_t*)kyber_ct.data() + kyber_ct.size());
    welcome_payload.insert(welcome_payload.end(), (uint8_t*)kyber_ss.data(),
                           (uint8_t*)kyber_ss.data() + kyber_ss.size()); // CRITICAL: Include our Kyber SS
    welcome_payload.insert(welcome_payload.end(), (uint8_t*)our_x25519_pub.data(),
                           (uint8_t*)our_x25519_pub.data() + 32);

    // Send WELCOME
    PacketHeader hdr_welcome;
    hdr_welcome.magic[0] = 'S';
    hdr_welcome.magic[1] = 'M';
...
{
    std::string peer_id_str = sender_id_to_string(hdr.sender_id);
    // std::cout << "\n[HANDSHAKE-3] === FINALIZE: Received WELCOME from " << peer_id_str.substr(0, 8) << "... ===" <<
    // std::endl;
    ...
    // std::cout << "[HANDSHAKE-3] Stored peer's Kyber SS from WELCOME: " << bytes_to_hex_short(peer_kyber_ss) << "..."
    // << std::endl;
    ...
    auth_payload.insert(auth_payload.end(), (uint8_t*)session->temp_kyber_ss_initiator.data(),
                        (uint8_t*)session->temp_kyber_ss_initiator.data() + 32);
...
void ProtocolManager::handle_auth(PacketHeader& hdr, std::vector<uint8_t>& payload, const sockaddr_in& sender)
{
    ...
    if (!session->temp_ecdh_priv.empty() && session->temp_ecdh_priv_initiator.empty())
    {
        ...
        {
            std::cout << "[PROTOCOL] AUTH payload too small to contain Kyber SS (got " << payload.size() << ", need 32)"
                      << std::endl;
            return;
        }
    }
...
{
    std::string sender_id_str = sender_id_to_string(hdr.sender_id);
    //     std::cout << "[PROTOCOL] DATA from " << sender_id_str << " (encrypted payload, size=" << payload.size() <<
    //     ")" << std::endl;
    ...
    std::vector<uint8_t> plaintext =
        session_mgr_->decrypt_message(sender_id_str, ciphertext_with_tag, hdr.packet_nonce);
    if (plaintext.empty())
    {
...
void ProtocolManager::handle_fs_list_req(PacketHeader& hdr, std::vector<uint8_t>& payload, const sockaddr_in& sender)
{
    ...
    if (!session)
    {
        std::cout << COLOR_RED << "[SECURITY] FS_LIST_REQ from unknown session: " << sender_id_str << COLOR_RESET
                  << std::endl;
        return;
    }
    ...
    if (session->state != SessionManager::SessionState::ESTABLISHED)
    {
        std::cout << COLOR_RED << "[SECURITY] FS_LIST_REQ from non-established session: " << sender_id_str
...
void ProtocolManager::handle_fs_list_res(PacketHeader& hdr, std::vector<uint8_t>& payload, const sockaddr_in& sender)
{
    ...
    if (!session)
    {
        std::cout << COLOR_RED << "✗ [ERROR] FS_LIST_RES from unknown session: " << sender_id_str << COLOR_RESET
                  << std::endl;
        return;
    }
    ...
    std::cout << COLOR_BRIGHT_CYAN << "║  Files in " << dir_name << std::string(padding > 0 ? padding : 0, ' ') << "║"
              << COLOR_RESET << std::endl;
    ...
    else
    {
        ...
        while (std::getline(ss, entry, ';') && parsed_count < file_count)
        {
            ...
            {
                std::cout << COLOR_RED << "✗ [ERROR] Malformed directory list format: " << entry << COLOR_RESET
                          << std::endl;
                continue;
            }
            ...
            // Print formatted row
            std::cout << color << "  " << std::left << std::setw(25) << filename << "  " << std::right << std::setw(10)
                      << size_display << "  [" << display_type << "]" << COLOR_RESET << std::endl;
            parsed_count++;
        }
        ...
        {
            std::cout << COLOR_YELLOW << "[WARNING] Parsed " << parsed_count << " files but expected " << file_count
                      << COLOR_RESET << std::endl;
        }
    }
...
void ProtocolManager::handle_fs_meta_req(PacketHeader& hdr, std::vector<uint8_t>& payload, const sockaddr_in& sender)
{
    ...
    if (!session)
    {
        std::cout << COLOR_RED << "[SECURITY] FS_META_REQ from unknown session: " << sender_id_str << COLOR_RESET
                  << std::endl;
        return;
    }
    ...
    if (session->state != SessionManager::SessionState::ESTABLISHED)
    {
        std::cout << COLOR_RED << "[SECURITY] FS_META_REQ from non-established session: " << sender_id_str
                  << COLOR_RESET << std::endl;
        return;
    }
...
void ProtocolManager::handle_fs_meta_res(PacketHeader& hdr, std::vector<uint8_t>& payload, const sockaddr_in& sender)
{
    ...
    if (session->pending_cd_request)
    {
        if (status == 1)
        {
            ...
            std::cout << COLOR_GREEN << "✓ Changed to "
                      << (session->current_remote_dir.empty() ? "/" : session->current_remote_dir) << COLOR_RESET
                      << std::endl;
        }
        else
        {
            // Directory not found
            std::cout << COLOR_RED << "✗ Directory not found: "
                      << (session->pending_cd_path.empty() ? "/" : session->pending_cd_path) << COLOR_RESET
                      << std::endl;
        }
...
void ProtocolManager::handle_fs_data_req(PacketHeader& hdr, std::vector<uint8_t>& payload, const sockaddr_in& sender)
{
    ...
    if (!session)
    {
        std::cout << COLOR_RED << "[SECURITY] FS_DATA_REQ from unknown session: " << sender_id_str << COLOR_RESET
                  << std::endl;
        return;
    }
    ...
    if (session->state != SessionManager::SessionState::ESTABLISHED)
    {
        std::cout << COLOR_RED << "[SECURITY] FS_DATA_REQ from non-established session: " << sender_id_str
                  << COLOR_RESET << std::endl;
        return;
    }
    ...
    try
    {
        ...
        if (canonical_str.length() < sandbox_str.length() ||
            canonical_str.substr(0, sandbox_str.length()) != sandbox_str)
        {
            std::cout << COLOR_RED << "[SECURITY] Path escape attempt blocked: " << filename << COLOR_RESET
                      << std::endl;
    ...
    try
    {
        ...
        else
        {
            ...
            {
                std::cout << COLOR_YELLOW << "[WARNING] FS_DATA_REQ: File " << filename << " is large (" << file_size
                          << " bytes), sent " << file_data.size() << " bytes in first chunk" << COLOR_RESET
                          << std::endl;
            }
        }
    }
...
void ProtocolManager::handle_fs_data_res(PacketHeader& hdr, std::vector<uint8_t>& payload, const sockaddr_in& sender)
{
    ...
    if (!session)
    {
        std::cout << COLOR_RED << "✗ [ERROR] FS_DATA_RES from unknown session: " << sender_id_str << COLOR_RESET
                  << std::endl;
        return;
    }
    ...
    if (session->state != SessionManager::SessionState::ESTABLISHED)
    {
        std::cout << COLOR_RED << "✗ [ERROR] FS_DATA_RES from non-established session: " << sender_id_str << COLOR_RESET
                  << std::endl;
        return;
    }
    ...
    if (plaintext.size() < 2)
    {
        std::cout << COLOR_RED << "✗ [ERROR] FS_DATA_RES payload too small (need at least 2 bytes for filename_len)"
                  << COLOR_RESET << std::endl;
        return;
    }
    ...
    {
        std::cout << COLOR_RED << "✗ [ERROR] FS_DATA_RES payload truncated (need " << (2 + filename_len + 8)
                  << " bytes, got " << plaintext.size() << ")" << COLOR_RESET << std::endl;
        return;
    }
    ...
    if (!SecurityUtils::is_safe_path(filename))
    {
        std::cout << COLOR_RED << "✗ [ERROR] Invalid filename with path traversal attempt: " << filename << COLOR_RESET
                  << std::endl;
        return;
    }
    ...
    try
    {
        ...
        if (!file.is_open())
        {
            std::cout << COLOR_RED << "✗ [ERROR] Failed to open file for writing: " << full_path << COLOR_RESET
                      << std::endl;
            return;
        }
        ...
        std::cout << COLOR_GREEN << "[SUCCESS] Received chunk at offset " << chunk_offset << " (" << chunk_data.size()
                  << " bytes). Saved to: " << full_path << COLOR_RESET << std::endl;
    }
    catch (const std::exception& e)
    {
        std::cout << COLOR_RED << "✗ [ERROR] Exception while saving file chunk: " << e.what() << COLOR_RESET
                  << std::endl;
        return;
    }
}

void ProtocolManager::handle_fs_put_meta_req(PacketHeader& hdr, std::vector<uint8_t>& payload,
                                             const sockaddr_in& sender)
{
    ...
    if (!session)
    {
        std::cout << COLOR_RED << "[SECURITY] FS_PUT_META_REQ from unknown session: " << sender_id_str << COLOR_RESET
                  << std::endl;
        return;
    }
    ...
    if (session->state != SessionManager::SessionState::ESTABLISHED)
    {
        std::cout << COLOR_RED << "[SECURITY] FS_PUT_META_REQ from non-established session: " << sender_id_str
                  << COLOR_RESET << std::endl;
        return;
    }
...
void ProtocolManager::handle_fs_put_meta_res(PacketHeader& hdr, std::vector<uint8_t>& payload,
                                             const sockaddr_in& sender)
{
    // Decode sender ID
    std::string sender_id_str = sender_id_to_string(hdr.sender_id);
    ...
    if (!session)
    {
        std::cout << COLOR_RED << "[FS] PUT_META_RES from unknown session: " << sender_id_str << COLOR_RESET
                  << std::endl;
        return;
    }
...
void ProtocolManager::handle_fs_put_data(PacketHeader& hdr, std::vector<uint8_t>& payload, const sockaddr_in& sender)
{
    ...
    if (!session)
    {
        std::cout << COLOR_RED << "[SECURITY] FS_PUT_DATA from unknown session: " << sender_id_str << COLOR_RESET
                  << std::endl;
        return;
    }
    ...
    if (session->state != SessionManager::SessionState::ESTABLISHED)
    {
        std::cout << COLOR_RED << "[SECURITY] FS_PUT_DATA from non-established session: " << sender_id_str
                  << COLOR_RESET << std::endl;
        return;
    }
    ...
    try
    {
        ...
        if (!file.is_open())
        {
            std::cout << COLOR_RED << "[SECURITY] Failed to open file for writing: " << full_path << COLOR_RESET
                      << std::endl;
            return;
        }
...
void ProtocolManager::handle_p2p_find_node_req(PacketHeader& hdr, std::vector<uint8_t>& payload,
                                               const sockaddr_in& sender)
{
    //     std::cout << "[PROTOCOL] P2P_FIND_NODE_REQ" << std::endl;
}

void ProtocolManager::handle_p2p_find_node_res(PacketHeader& hdr, std::vector<uint8_t>& payload,
                                               const sockaddr_in& sender)
{
...
void ProtocolManager::send_packet(const uint8_t* target_id, uint8_t opcode, uint8_t flags,
                                  const std::vector<uint8_t>& payload, const std::string& target_ip, int target_port)
{
...
bool ProtocolManager::initiate_handshake(const std::string& target_ip, int target_port)
{
    ...
    hello_payload.insert(hello_payload.end(), (uint8_t*)my_node_->kyber_pub.data(),
                         (uint8_t*)my_node_->kyber_pub.data() + my_node_->kyber_pub.size());
...
bool ProtocolManager::send_ping(const std::string& target_ip, int target_port)
{
    ...
    //      std::cout << "[PROTOCOL] PING sent to " << target_ip << ":" << target_port << " (ts=" << ping_ts << ")" <<
    //      std::endl;
    return true;
}
...
void ProtocolManager::send_ping_ack(const std::string& node_id)
{
    ...
    hdr_ping_ack.magic[0] = 'S';
    hdr_ping_ack.magic[1] = 'M';
