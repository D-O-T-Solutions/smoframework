...
void SessionManager::set_handshake_state(const std::string& node_id, const std::string& kyber_ss,
...
bool SessionManager::establish_session(const std::string& node_id)
{
    ...
    else
    {
        ...
        std::cout << "  Our priv:    " << bytes_to_hex_short(session->temp_ecdh_priv)
                  << "... (len=" << session->temp_ecdh_priv.size() << ")" << std::endl;
    ...
    else
    {
        ...
        combined_kyber_ss = session->temp_kyber_ss_responder.empty() ? session->temp_kyber_ss_initiator
    ...
    // std::cout << "  AES Key: " << bytes_to_hex_short_vec(session->aes_key) << "... (len=" << session->aes_key.size()
    // << ")" << std::endl; std::cout << "  AES IV:  " << bytes_to_hex_short_vec(session->aes_iv) << " (len=" <<
    // session->aes_iv.size() << ")" << std::endl;
...
std::vector<uint8_t> SessionManager::encrypt_message(const std::string& node_id, const std::vector<uint8_t>& plaintext,
...
std::vector<uint8_t> SessionManager::decrypt_message(const std::string& node_id,
...
void SessionManager::cleanup_expired_sessions(uint64_t timeout_ms)
{
    ...
    for (auto& [node_id, session] : sessions_)
    {
        ...
        if ((session->state == SessionState::INITIATED || session->state == SessionState::WELCOME_RECEIVED ||
...
void SessionManager::kdf_derive_keys(const std::string& kyber_ss, const std::string& ecdh_ss,
                                     std::vector<uint8_t>& out_key, std::vector<uint8_t>& out_iv)
{
...
bool SessionManager::migrate_session(const std::string& placeholder_id, const std::string& real_id)
{
    ...
    {
        // std::cout << "[SESSION]   Final - Key: " << key.substr(0, 8) << "..., Session ID: " <<
        // session->node_id.substr(0, 8) << "..., ptr=" << (void*)session.get() << std::endl;
    }

    // Verify migration by trying to get the new session
    auto migrated = get_session(real_id);
    // std::cout << "[SESSION] After migration, get_session(" << real_id.substr(0, 8) << "...) returns ptr=" <<
    // (void*)migrated.get() << std::endl;
