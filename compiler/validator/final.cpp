...
namespace smo {
    ...
    static uint32_t max_depth_dfs(const std::string& node_id,
    ...
    Result<ExecutionGraph> FinalValidator::validate(const ExecutionGraph& graph, const Config& cfg)
    {
        if (graph.nodes.empty())
        {
            return SMO_ERR_COMPILER(10, Error, NoRetry, None, "DAG has no nodes");
        }

        if (graph.nodes.size() > cfg.max_nodes)
        {
            return SMO_ERR_COMPILER(11, Error, NoRetry, None, "DAG exceeds max nodes");
        }

        if (cfg.reject_cycles && has_cycle(graph))
        {
            return SMO_ERR_COMPILER(12, Error, NoRetry, None, "DAG contains a cycle");
        }
        ...
        for (auto& node : graph.nodes)
        {
            ...
            {
                return SMO_ERR_COMPILER(13, Error, NoRetry, None, "DAG exceeds max depth");
            }
        }
