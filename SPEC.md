...
| MeshID | Deterministic hash derived at mesh creation: SuiteHash(RootPublicKey \|\| CreatedAt \|\| Random) — BLAKE3-256 (Suite 2/3) or SHA-256 (Suite 1) |
...
| Epoch | Mesh-wide counter incremented on major capability changes. Credentials authorized under older Epoch are rejected. Epoch is the coarse-grained revocation primitive; the Revocation Set is fine-grained (RFC 0006 AMEND-2) |
| Join Token | Issuer-signed: `SMO-JOIN-<base64url(CBOR || issuer_signature)>` (RFC 0007 AMEND-5). Signed by Authority/Root; contains mesh_id, epoch, suite, endpoints, role, expiry, nonce, issuer_key_id, issuer_suite_id. Single-use or time-limited |
| Recovery Package | Recovery domain: Argon2id + AES-256-GCM (salt 32B, nonce 12B). Contains encrypted Root Private Key. Exported at mesh creation, password-protected. Recovery ≠ resurrection |
...
│   ├── suite1_classical/    Suite 1: Ed25519 + X25519 + XChaCha20-Poly1305 + SHA-256
│   ├── suite2_modern/       Suite 2: Ed25519 + X25519 + XChaCha20-Poly1305 + BLAKE3-256
│   ├── suite3_purepqc/      Suite 3: ML-DSA-65 + ML-KEM-768 + XChaCha20-Poly1305 + BLAKE3-256
...
```
Suite 1 (Classical — max compatibility)
...
> **LOCKED 2026-08-14 (RFC 0024 canonical, DISCUSSION_0046 §12):** Suite 1 hash = SHA-256 (NOT BLAKE3). Suite 2 = Ed25519 + X25519 (NOT ML-DSA). Suite 3 KEM = ML-KEM-768 (NOT 1024). Recovery encryption is a dedicated domain (Argon2id + AES-256-GCM) and does NOT inherit these suites.
...
2. **Old handshake mixed Kyber KEM + X25519 ECDH + Falcon signature in one flow.** This combined key exchange with identity verification with session establishment. In SMO:
   ...
   - Identity verification → Session layer (post-connection, using suite-selected signer: Ed25519 Suite 1/2, ML-DSA-65 Suite 3)
...
↓
Generate Mesh Root Keypair (Suite-selected; default genesis = Suite 3 / ML-DSA-65)
↓
Compute MeshID = SuiteHash(RootPK || time || random)   (SHA-256 for Suite 1, BLAKE3-256 for Suite 2/3)
↓
Generate First Authority Keypair (Suite-selected — same suite as Root)
↓
...
↓
Export Recovery Package (Recovery domain: Argon2id + AES-256-GCM, salt 32B, nonce 12B, versioned)
↓
...
MembershipCertificate {
    NodeID:           SuiteHash(NodePublicKey)  (SHA-256 Suite 1 / BLAKE3-256 Suite 2-3)
    NodePublicKey:    suite public key bytes (Ed25519 Suite 1-2 / ML-DSA-65 Suite 3)
    MeshID:           SuiteHash identifying the mesh
...
↓
Generate node keypair (Suite-selected from mesh: Ed25519 for Suite 1/2, ML-DSA-65 for Suite 3)

↓
Propose display_name and send Certificate Signing Request (CSR):
  NodeID             SuiteHash(NodePublicKey)  (BLAKE3-256 for Suite 2/3, SHA-256 for Suite 1)
...
**The Join Token (v2, issuer-signed — AMENDED 2026-08-14, RFC 0007 AMEND-5):**
- Generated by Authority: `SMO-JOIN-<base64url(CBOR || issuer_signature)>`; CBOR contains `version, mesh_id, mesh_epoch, cipher_suite_id, bootstrap_endpoints[], role, expiry, nonce, issuer_key_id, issuer_suite_id`
- Signed with the issuing Authority/Root key; verified via `issuer_key_id + issuer_suite_id` (NO HMAC, NO hmac_secret — deprecated)
...
CREATE TABLE nodes (
    ...
    node_id_hex     TEXT NOT NULL,             -- SuiteHash(NodePublicKey), 64 hex chars
...
**Recovery Package format (Recovery domain — dedicated, NOT mesh suite; AMEND-3 2026-08-14):**
...
recovery.smo {
    ...
    MeshID
    CreatedAt
    KDF: Argon2id              (memory-hard password KDF)
    Salt: 32 bytes
    Argon2Params: {memory, iterations, parallelism}
    Nonce: 12 bytes            (AES-GCM nonce size)
    Tag: 16 bytes              (AEAD auth tag)
}
```

> **LOCKED 2026-08-14 (RFC 0006 AMEND-3, DISCUSSION_0046 §25.1):** Recovery is a dedicated cryptographic domain — Argon2id (KDF) + AES-256-GCM (AEAD, nonce 12B, salt 32B). It does NOT inherit the mesh Crypto Suite AEAD (XChaCha20). Argon2id makes offline passphrase-guessing expensive; AES-256-GCM provides confidentiality + integrity. **Recovery ≠ resurrection:** reconstructing shares recovers key material offline; a live authority requires a governance-authorized rekey/election + new epoch.
...
│ 2. MeshID = SuiteHash(RootPK || time || random)                  │
...
│ 8. Capability Epoch + Revocation Set as revocation.             │
│    → Epoch: coarse global invalidation; Revocation Set: fine.   │
│    → No CRL-only. Offline ≠ compromised (governance revokes).   │
│ 9. Roles are presets. Runtime only knows capabilities.          │
│ 10. Multi-mesh nodes: one Identity, multiple Memberships.        │
│ 11. Recovery Package (Recovery domain: Argon2id + AES-256-GCM)  │
│    → instead of "save 5 keys"; recovery ≠ resurrection.         │
└─────────────────────────────────────────────────────────────────┘
```
...
✅ Crypto Suite 1 (Ed25519, X25519, XChaCha20-Poly1305, SHA-256)
...
| MeshID | SuiteHash(RootPK \|\| CreatedAtMs \|\| Random32) |
| NodeID | SuiteHash(NodePublicKey) |
