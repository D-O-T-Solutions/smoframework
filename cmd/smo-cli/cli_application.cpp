...
namespace smo {
    ...
    public:
        ...
        static constexpr const char* kCommands_[] = {
            "ls",      "cd",      "pwd",      "info",    "exec",       "ping",     "ps",      "kill",
        ...
        Result<void> init(const std::string& data_dir) { return context_.initialize(data_dir); }
    ...
    private:
        ...
        Result<int> execute_intent(const Intent& intent)
        {
            ...
            case IntentType::Help:
                return handle_help(intent);
            case IntentType::Exit:
                return 0;
            case IntentType::Select:
                return handle_select(intent);
            case IntentType::Execute:
                return handle_exec(intent);
            case IntentType::Transfer:
                return handle_transfer(intent);
            case IntentType::Filesystem:
                return handle_filesystem(intent);
            case IntentType::Process:
                return handle_process(intent);
            case IntentType::Deploy:
                return handle_deploy(intent);
            case IntentType::Undeploy:
                return handle_undeploy(intent);
            case IntentType::Status:
                return handle_status(intent);
            case IntentType::Policy:
                return handle_policy(intent);
            case IntentType::Control:
                return handle_control(intent);
            case IntentType::Context:
                return handle_context(intent);
            case IntentType::Mesh:
                return handle_mesh(intent);
            case IntentType::Genesis:
                return handle_genesis(intent);
            case IntentType::Governance:
                return handle_governance(intent);
            case IntentType::Recovery:
                return handle_recovery(intent);
            case IntentType::Connect:
                return handle_connect(intent);
            case IntentType::Disconnect:
                return handle_disconnect(intent);
            case IntentType::Discover:
                return handle_discover(intent);
            case IntentType::Export:
                return handle_export(intent);
            case IntentType::Session:
                return handle_session(intent);
        ...
        Result<int> handle_select(const Intent& intent)
        {
            ...
            if (intent.trust_min)
                sel.trust_min = std::stof(intent.trust_min.value());
            if (intent.trust_max)
                sel.trust_max = std::stof(intent.trust_max.value());
            ...
            std::string sel_desc;
            if (!sel.node_names.empty())
                sel_desc = sel.node_names.front();
            else if (!sel.roles.empty())
                sel_desc = "role:" + sel.roles.front();
            else if (!sel.tags.empty())
                sel_desc = "tag:" + sel.tags.front();
            else
                sel_desc = "*";
            std::cout << "Selection active: " << sel_desc << " (" << (sel.roles.empty() ? "any" : sel.roles.front())
                      << ")"
                      << "\n";
            return 0;
        }
        ...
        Result<int> handle_status(const Intent& intent)
        {
            if (intent.args.empty())
            {
                ...
                std::cout << "  Mesh:     "
                          << (context_.get_current_mesh() ? context_.get_current_mesh().value() : "(none)") << "\n";
        ...
        Result<int> handle_control(const Intent& intent)
        {
            if (intent.flags.count("level"))
            {
                ...
                if (l == "safe")
                    context_.set_control_level(ControlLevel::Safe);
                else if (l == "normal")
                    context_.set_control_level(ControlLevel::Normal);
                else if (l == "force")
                    context_.set_control_level(ControlLevel::Force);
                else if (l == "emergency")
                    context_.set_control_level(ControlLevel::Emergency);
                else
                {
            ...
            if (intent.flags.count("scope"))
            {
                ...
                if (s == "single")
                    context_.set_scope(ExecutionScope::Single);
                else if (s == "mesh")
                    context_.set_scope(ExecutionScope::Mesh);
                else if (s == "quorum")
                    context_.set_scope(ExecutionScope::Quorum);
                else if (s == "witness")
                    context_.set_scope(ExecutionScope::Witness);
                else
                {
            ...
            auto level_to_str = [](ControlLevel l) -> const char* {
                ...
                case ControlLevel::Normal:
                    return "normal";
                case ControlLevel::Elevated:
                    return "elevated";
                case ControlLevel::Force:
                    return "force";
                case ControlLevel::Emergency:
                    return "emergency";
                case ControlLevel::Privileged:
                    return "privileged";
                default:
                    return "unknown";
                }
            ...
            std::cout << "Control: level=" << level_to_str(context_.get_control_level())
                      << " scope=" << scope_to_str(context_.get_scope()) << " timeout=" << context_.get_timeout()
                      << "ms"
        ...
        Result<int> handle_mesh(const Intent& intent)
        {
            ...
            if (intent.flags.count("list") || intent.flags.empty())
            {
                ...
                if (std::filesystem::is_directory(meshes_dir))
                {
                    for (const auto& entry : std::filesystem::directory_iterator(meshes_dir))
                    {
                        ...
                        if (!role.empty())
                            std::cout << " (" << role << ")";
                        if (!mesh_id.empty() && mesh_id != name)
                            std::cout << " [" << mesh_id.substr(0, 8) << "]";
                        std::cout << "\n";
                    }
                }
                else
            ...
            if (intent.flags.count("create"))
            {
                ...
                auto hash_result =
                    crypto->hash.hash(BytesView(reinterpret_cast<const uint8_t*>(canonical.data()), canonical.size()));
                if (!hash_result)
                {
            ...
            if (intent.flags.count("publish"))
            {
                ...
                if (!current)
                {
                    std::cerr << current.error().message << "\n";
                    return 1;
                }
            ...
            if (intent.flags.count("join"))
            {
                ...
                else
                {
                    ...
                    std::string mesh_name = jr.mesh_id.size() >= 8 ? jr.mesh_id.substr(0, 8) : jr.mesh_id;
            ...
            if (intent.flags.count("health"))
            {
                ...
                auto health = smo::compute_health(total, online, m.authorities.minimum, m.authorities.preferred,
        ...
        Result<int> handle_genesis(const Intent& intent)
        {
            ...
            if (intent.flags.count("create"))
            {
                ...
                if (intent.flags.count("authorities"))
                {
                    try
                    {
                        authorities = std::stoul(intent.flags.at("authorities"));
                    }
                    catch (...)
                    {
                        std::cerr << "Invalid authorities count\n";
                        return 1;
                    }
                }
            ...
            if (intent.flags.count("status"))
            {
                ...
                std::cout << "  Authorities: min=" << m.authorities.minimum << " preferred=" << m.authorities.preferred
        ...
        Result<int> handle_recovery(const Intent& intent)
        {
            ...
            if (intent.flags.count("restore"))
            {
                ...
                std::string passphrase = intent.flags.count("passphrase") ? intent.flags.at("passphrase") : "";
                ...
                auto session = engine.start_soft(current.value(), "root-node", 1, passphrase, 0);
            ...
            if (intent.flags.count("force"))
            {
                ...
                auto session = engine.start_hard(current.value(), "root-node", 1, passphrase, 0);
        ...
        Result<int> handle_history(const Intent& intent)
        {
            ...
            if (intent.flags.count("limit"))
            {
                try
                {
                    limit = std::stoul(intent.flags.at("limit"));
                }
                catch (...)
                {
                }
