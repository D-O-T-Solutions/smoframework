...
namespace smo {
    ...
    public:
        ...
        Impl()
        {
            ...
            auto add2 = [this](const std::string& name, std::string desc, IntentType t,
                               std::vector<std::string> aliases, std::vector<std::string> required,
                               std::vector<std::pair<std::string, std::string>> flags) {
                CommandDef def;
                def.description = std::move(desc);
                def.type = t;
            ...
            add("ls", "List directory contents", IntentType::Filesystem, {"path"},
                {{"recursive", "Recursive listing"}, {"long", "Long format"}});
            add("pwd", "Print working directory", IntentType::Filesystem, {}, {});
            add("mkdir", "Create directory", IntentType::Filesystem, {"path"},
                {{"parents", "Create parent directories"}, {"mode", "Permission mode"}});
            add("rm", "Remove file or directory", IntentType::Filesystem, {"path"},
                {{"recursive", "Recursive removal"}, {"force", "Force removal"}});
            add("cp", "Copy file or directory", IntentType::Filesystem, {"src", "dst"},
                {{"recursive", "Recursive copy"}, {"preserve", "Preserve attributes"}});
            add("mv", "Move/rename file or directory", IntentType::Filesystem, {"src", "dst"},
                {{"force", "Force overwrite"}});
            add("cat", "Display file contents", IntentType::Filesystem, {"path"},
            ...
            add("put", "Put file to node", IntentType::Transfer, {"local", "remote"},
                {{"overwrite", "Overwrite remote"}, {"resume", "Resume transfer"}});
            add("sync", "Synchronize directories", IntentType::Transfer, {"local", "remote"},
                {{"delete", "Delete extra files"}, {"dry-run", "Dry run"}});
            add2("exec", "Execute command on node", IntentType::Execute, {}, {"command"},
                 {{"args", "Command arguments"}, {"timeout", "Timeout ms"}, {"shell", "Use shell"}});
            add("ping", "Ping node", IntentType::Execute, {}, {{"count", "Ping count"}, {"interval", "Interval ms"}});
            add("ps", "List processes", IntentType::Process, {},
                {{"all", "All processes"}, {"user", "Filter by user"}});
            add("kill", "Kill process", IntentType::Process, {"pid"},
            ...
            add("history", "Show history", IntentType::History, {},
            ...
            add2("policy", "Policy management", IntentType::Policy, {}, {},
                 {{"preset", "Policy preset"},
                  {"custom", "Custom policy file"},
                  {"list", "List policies"},
                  {"show", "Show policy details"}});
            add2("control", "Control level", IntentType::Control, {}, {},
                 {{"level", "Control level: safe|normal|force|emergency"},
                  {"scope", "Scope: single|mesh|quorum|witness"},
                  {"timeout", "Timeout ms"},
                  {"retry", "Retry count"}});
            add2("mesh", "Mesh management", IntentType::Mesh, {}, {},
                 {{"list", "List meshes"},
                  {"use", "Use mesh"},
                  {"create", "Create mesh"},
                  {"health", "Show mesh health"},
                  {"join", "Join mesh"},
                  {"leave", "Leave mesh"},
                  {"remove", "Remove mesh"},
                  {"invite", "Generate Join Token"},
                  {"expire", "Token expiry duration (e.g. 30m, 1h)"},
                  {"role", "Identity role: Authority, Member, Contributor, Observer"},
                  {"profile", "Setup profile: server, desktop, embedded, gateway"}});
            add2("genesis", "Genesis management", IntentType::Genesis, {}, {},
                 {{"create", "Create genesis mesh"},
                  {"status", "Show genesis status"},
                  {"manifest", "Show genesis manifest"}});
            add2("governance", "Governance management", IntentType::Governance, {}, {},
                 {{"propose", "Submit governance proposal"},
                  {"sign", "Sign a proposal"},
                  {"commit", "Commit a proposal"},
                  {"list", "List proposals"},
                  {"status", "Show governance status"}});
            add2("recovery", "Mesh recovery operations", IntentType::Recovery, {}, {},
            ...
            add2("context", "Context management", IntentType::Context, {}, {},
                 {{"list", "List contexts"},
                  {"save", "Save context"},
                  {"load", "Load context"},
                  {"clear", "Clear context"}});
    ...
    Result<ParsedCommand> IntentParser::parse(const std::vector<std::string>& args) const
    {
        ...
        // Parse special flags
        if (intent.flags.count("nearest"))
            intent.nearest = true;
        if (intent.flags.count("random"))
        {
            try
            {
                intent.random_n = std::stoi(intent.flags["random"]);
            }
            catch (...)
            {
            }
        ...
        if (intent.flags.count("top"))
        {
            try
            {
                intent.top_n = std::stoi(intent.flags["top"]);
            }
            catch (...)
            {
            }
        ...
        if (intent.flags.count("scope"))
            intent.scope = intent.flags["scope"];
        if (intent.flags.count("control"))
            intent.control = intent.flags["control"];
        if (intent.flags.count("timeout"))
        {
            try
            {
                intent.timeout_ms = std::stoi(intent.flags["timeout"]);
            }
            catch (...)
            {
            }
        ...
        if (intent.flags.count("retry"))
        {
            try
            {
                intent.retry_count = std::stoi(intent.flags["retry"]);
            }
            catch (...)
            {
            }
        }
        if (intent.flags.count("dry-run"))
            intent.dry_run = true;
        ...
        if (intent.flags.count("role"))
            intent.roles.push_back(intent.flags["role"]);
        if (intent.flags.count("tag"))
            intent.tags.push_back(intent.flags["tag"]);
        if (intent.flags.count("where"))
            intent.where_expression = intent.flags["where"];
        if (intent.flags.count("mesh"))
            intent.mesh_filter = intent.flags["mesh"];
        if (intent.flags.count("os"))
            intent.os_filter = intent.flags["os"];
        if (intent.flags.count("arch"))
            intent.arch_filter = intent.flags["arch"];
        if (intent.flags.count("version"))
            intent.version_filter = intent.flags["version"];
        if (intent.flags.count("trust-min"))
            intent.trust_min = intent.flags["trust-min"];
        if (intent.flags.count("trust-max"))
            intent.trust_max = intent.flags["trust-max"];

        if (intent.flags.count("name"))
            intent.node_names.push_back(intent.flags["name"]);
        ...
        return ParsedCommand{std::move(intent),
