...
int main(int argc, char* argv[])
{
    ...
    if (!node_name.empty())
        std::printf("[smo-node] Name: %s\n", node_name.c_str());
    if (!seed_addr.empty())
        std::printf("[smo-node] Seed: %s\n", seed_addr.c_str());
    ...
    std::printf("[smo-node] Local NodeID: %s (state: %s)\n", local_id_hex.c_str(),
                smo::to_string(local_identity.state()));
    ...
    smo::runtime::RuntimeKernel runtime_kernel(event_bus, output_mgr, runtime_dispatcher, plan_resolver);
    ...
    sync_service.on_delta("crl", [&](const std::string&) -> smo::Result<void> {
        ...
        {
            if (e.epoch > last_crl_epoch)
                last_crl_epoch = e.epoch;
        }
    ...
    sync_service.on_delta("policy", [&](const std::string&) -> smo::Result<void> {
        ...
        if (current <= last_policy_version)
            return {};
    ...
    sync_service.on_delta("manifest", [&](const std::string&) -> smo::Result<void> {
        if (!manifest_store.is_open())
            return {};
        auto latest = manifest_store.latest_epoch();
        if (!latest || latest.value() <= last_manifest_epoch)
            return {};
    ...
    runtime_dispatcher.register_contract("system.echo", std::make_unique<smo::runtime::EchoContract>());

    // BootstrapContract: mesh bootstrap snapshots
    runtime_dispatcher.register_contract("system.bootstrap", std::make_unique<smo::runtime::BootstrapContract>(
                                                                 mesh_manager, authority, &governance_engine,
    ...
    // JoinContract: node enrollment
    {
        ...
        runtime_dispatcher.register_contract(
            "system.join", std::make_unique<smo::runtime::JoinContract>(crypto->hash, crypto->signer, rng));
    }

    // GovernanceContract: proposals, voting, commit
    runtime_dispatcher.register_contract(
        "system.governance", std::make_unique<smo::runtime::GovernanceContract>(governance_engine, authority));

    // RecoveryContract: recovery sessions, CRL
    runtime_dispatcher.register_contract(
        "system.recovery", std::make_unique<smo::runtime::RecoveryContract>(recovery_engine, &crl, governance_engine));

    // FileContract: filesystem operations
    runtime_dispatcher.register_contract("system.file", std::make_unique<smo::runtime::FileContract>());
    ...
    smo::runtime::RuntimeBridge runtime_bridge(runtime_kernel, runtime_dispatcher);

    // Register routes for Echo
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::ECHO), "system.echo", "echo");

    // Register routes for BootstrapContract
    runtime_bridge.register_route(smo::bootstrap::kOpcodeBootstrapRequest, "system.bootstrap", "request");
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::BOOTSTRAP_SNAPSHOT), "system.bootstrap",
                                  "snapshot");
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::BOOTSTRAP_INFO), "system.bootstrap", "info");
    runtime_bridge.register_route(smo::join::kOpcodeBootstrapSyncReq, "system.bootstrap", "bootstrap_sync");

    // Register routes for JoinContract
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::JOIN), "system.join", "join");
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::LEAVE), "system.join", "leave");
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::JOIN_INFO), "system.join", "info");

    // Register routes for GovernanceContract
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::GOV_PROPOSE), "system.governance", "propose");
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::GOV_VOTE), "system.governance", "vote");
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::GOV_COMMIT), "system.governance", "commit");
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::GOV_LIST), "system.governance", "list");
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::GOV_STATUS), "system.governance", "status");
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::GOV_INFO), "system.governance", "info");

    // Register routes for RecoveryContract (single opcode, method in payload)
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::RECOVERY), "system.recovery", "invoke");

    // Register routes for FileContract (single opcode, method in payload)
    runtime_bridge.register_route(static_cast<uint32_t>(smo::Opcode::FILE_OP), "system.file", "invoke");
    ...
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::ECHO), runtime_handler);
    dispatcher.register_handler(smo::bootstrap::kOpcodeBootstrapRequest, runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::BOOTSTRAP_SNAPSHOT), runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::BOOTSTRAP_INFO), runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::JOIN), runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::LEAVE), runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::JOIN_INFO), runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::GOV_PROPOSE), runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::GOV_VOTE), runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::GOV_COMMIT), runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::GOV_LIST), runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::GOV_STATUS), runtime_handler);
    dispatcher.register_handler(static_cast<uint32_t>(smo::Opcode::GOV_INFO), runtime_handler);
    ...
    event_bus.subscribe(smo::runtime::EventType::RecoveryApproved, [&](const smo::runtime::Event& ev) {
    ...
    event_bus.subscribe(smo::runtime::EventType::SecurityAlert, [&](const smo::runtime::Event& ev) {
        std::printf("[smo-node] AUDIT: Trust score change - %s\n", ev.details.c_str());
    });

    // Session disconnect → Discovery membership update
    event_bus.subscribe(smo::runtime::EventType::NodeDisconnected, [&](const smo::runtime::Event& ev) {
        std::printf("[smo-node] Discovery: Node disconnected - %s\n", ev.details.c_str());
        // Membership sync will pick up on next tick
    });

    // Trust score change → Audit log
    event_bus.subscribe(smo::runtime::EventType::AuditLogged, [&](const smo::runtime::Event& ev) {
        std::printf("[smo-node] AUDIT: %s\n", ev.details.c_str());
    });

    // Governance proposal updates → all nodes
    event_bus.subscribe(smo::runtime::EventType::ProposalCreated, [&](const smo::runtime::Event& ev) {
        std::printf("[smo-node] GOVERNANCE: Proposal created - %s\n", ev.details.c_str());
    });
    event_bus.subscribe(smo::runtime::EventType::ProposalVoted, [&](const smo::runtime::Event& ev) {
        std::printf("[smo-node] GOVERNANCE: Vote cast - %s\n", ev.details.c_str());
    });
    event_bus.subscribe(smo::runtime::EventType::ProposalCommitted, [&](const smo::runtime::Event& ev) {
        std::printf("[smo-node] GOVERNANCE: Proposal committed - %s\n", ev.details.c_str());
    });
    event_bus.subscribe(smo::runtime::EventType::ProposalRejected, [&](const smo::runtime::Event& ev) {
        std::printf("[smo-node] GOVERNANCE: Proposal rejected - %s\n", ev.details.c_str());
    });

    // Recovery events
    event_bus.subscribe(smo::runtime::EventType::RecoveryStarted, [&](const smo::runtime::Event& ev) {
        std::printf("[smo-node] RECOVERY: Started - %s\n", ev.details.c_str());
    });
    event_bus.subscribe(smo::runtime::EventType::RecoveryCompleted, [&](const smo::runtime::Event& ev) {
        std::printf("[smo-node] RECOVERY: Completed - %s\n", ev.details.c_str());
    });
    event_bus.subscribe(smo::runtime::EventType::RecoveryFailed, [&](const smo::runtime::Event& ev) {
    ...
    smo::sync::AntiEntropyService anti_entropy(membership, gossip_engine, *sync_backend, ae_config);
    anti_entropy.start();
    ...
    while (g_running)
    {
        ...
        if (now_ns - last_tick > 5000000000LL)
        {
            ...
            if (!daemon_ready_logged && uptime_ns > 30'000'000'000LL)
            {
                ...
                if (hb_active && gossip_tx && gossip_rx)
                {
                    LOG.info("node READY — " + std::to_string(membership.count()) +
                             " peer(s), gossip tx=" + std::to_string(gossip_engine.gossip_sent_count()) +
                             " rx=" + std::to_string(gossip_engine.gossip_received_count()));
            ...
            telemetry.set_gauge("smo_connected_peers", static_cast<double>(membership.count()), "");
            telemetry.set_gauge("smo_membership_epoch", static_cast<double>(now_ns % 1'000'000), "");
            telemetry.set_gauge("smo_anti_entropy_repairs_total", static_cast<double>(anti_entropy.repairs_done()), "");
        ...
        if (udp_listener)
        {
            while (true)
            {
                ...
                if (recv_data)
                {
                    ...
                    (void)smo::dispatch_discovery_datagram(recv_data.value(), discovery_engine, from, now_ns);
                }
        ...
        if (tcp_session)
        {
            ...
            {
                auto dispatch_res = dispatcher.dispatch_session(*tcp_ses, remote_ep);
                if (!dispatch_res)
                {
                    LOG.warn("dispatch failed: " + dispatch_res.error().message + " from " + remote_str);
                }
